...
#define GDICT_CONTEXT(obj) \
  (G_TYPE_CHECK_INSTANCE_CAST((obj), GDICT_TYPE_CONTEXT, GdictContext))
#define GDICT_IS_CONTEXT(obj) \
  (G_TYPE_CHECK_INSTANCE_TYPE((obj), GDICT_TYPE_CONTEXT))
#define GDICT_CONTEXT_GET_IFACE(inst) \
  (G_TYPE_INSTANCE_GET_INTERFACE((inst), GDICT_TYPE_CONTEXT, GdictContextIface))
...
struct _GdictContextIface {
  ...
  /*< public >*/
  /* methods, not signals */
  gboolean (*get_databases)(GdictContext *context, GError **error);
  gboolean (*get_strategies)(GdictContext *context, GError **error);
  gboolean (*match_word)(GdictContext *context, const gchar *database,
                         const gchar *strategy, const gchar *word,
                         GError **error);
  gboolean (*define_word)(GdictContext *context, const gchar *database,
  ...
  void (*database_found)(GdictContext *context, GdictDatabase *database);
  void (*strategy_found)(GdictContext *context, GdictStrategy *strategy);
  void (*match_found)(GdictContext *context, GdictMatch *match);
  void (*definition_found)(GdictContext *context, GdictDefinition *definition);

  /* fired each time there's an error; the GError is owned
   * by the context, and should never be modified or freed
   */
  void (*error)(GdictContext *context, const GError *error);
};

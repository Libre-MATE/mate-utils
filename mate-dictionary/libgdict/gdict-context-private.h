...
#error \
    "You are trying to access Gdict's internals outside Gdict.  The API of these internal functions is not fixed."
#endif
...
#define GDICT_DEFINE_BOXED_TYPE(TypeName, type_name)          \
  ...
  GType type_name##_get_type(void) {                          \
    ...
    if (G_UNLIKELY(gdict_define_boxed_type == 0))             \
      gdict_define_boxed_type = g_boxed_type_register_static( \
          #TypeName, (GBoxedCopyFunc)type_name##_intern_ref,  \
          (GBoxedFreeFunc)type_name##_intern_unref);          \

...
static const GDebugKey gdict_debug_keys[] = {
    {"misc", GDICT_DEBUG_MISC},     {"context", GDICT_DEBUG_CONTEXT},
    {"dict", GDICT_DEBUG_DICT},     {"source", GDICT_DEBUG_SOURCE},
    {"loader", GDICT_DEBUG_LOADER}, {"chooser", GDICT_DEBUG_CHOOSER},
    {"defbux", GDICT_DEBUG_DEFBOX}, {"speller", GDICT_DEBUG_SPELLER},
};
...
#ifdef GDICT_ENABLE_DEBUG
static gboolean gdict_arg_debug_cb(const char *key, const char *value,
                                   gpointer user_data) {
  gdict_debug_flags |= g_parse_debug_string(value, gdict_debug_keys,
                                            G_N_ELEMENTS(gdict_debug_keys));
  return TRUE;
}

static gboolean gdict_arg_no_debug_cb(const char *key, const char *value,
                                      gpointer user_data) {
  gdict_debug_flags &= ~g_parse_debug_string(value, gdict_debug_keys,
                                             G_N_ELEMENTS(gdict_debug_keys));
  return TRUE;
...
static gboolean pre_parse_hook(GOptionContext *context, GOptionGroup *group,
                               gpointer data, GError **error) {
  if (gdict_is_initialized) return TRUE;

#ifdef GDICT_ENABLE_DEBUG
  const char *env_string = g_getenv("GDICT_DEBUG");
  if (env_string != NULL) {
    gdict_debug_flags = g_parse_debug_string(env_string, gdict_debug_keys,
                                             G_N_ELEMENTS(gdict_debug_keys));
  }
#endif /* GDICT_ENABLE_DEBUG */
...
static gboolean post_parse_hook(GOptionContext *context, GOptionGroup *group,
                                gpointer data, GError **error) {
...
GOptionGroup *gdict_get_option_group(void) {
  ...
  group = g_option_group_new("gdict", _("GDict Options"),
                             _("Show GDict Options"), NULL, NULL);
 ...
 * Since: 0.12
 */
void gdict_debug_init(gint *argc, gchar ***argv) {
  GOptionContext *option_context;
  GOptionGroup *gdict_group;
  GError *error = NULL;

  if (gdict_is_initialized) return;
...
gboolean gdict_check_version(guint required_major, guint required_minor,
                             guint required_micro) {
  gint gdict_effective_micro = 100 * GDICT_MINOR_VERSION + GDICT_MICRO_VERSION;
  gint required_effective_micro = 100 * required_minor + required_micro;

  if (required_major > GDICT_MAJOR_VERSION) return FALSE;

  if (required_major < GDICT_MAJOR_VERSION) return FALSE;

  if (required_effective_micro < gdict_effective_micro) return FALSE;
...
static GtkWindow *get_toplevel_window(GtkWidget *widget) {
  GtkWidget *toplevel;

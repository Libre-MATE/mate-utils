...
enum { DATABASE_NAME, DATABASE_ERROR } DBType;
...
G_DEFINE_TYPE_WITH_PRIVATE(GdictDatabaseChooser, gdict_database_chooser,
                           GTK_TYPE_BOX);

static void set_gdict_context(GdictDatabaseChooser *chooser,
                              GdictContext *context) {
...
static void gdict_database_chooser_finalize(GObject *gobject) {
...
static void gdict_database_chooser_dispose(GObject *gobject) {
  GdictDatabaseChooser *chooser = GDICT_DATABASE_CHOOSER(gobject);
  GdictDatabaseChooserPrivate *priv = chooser->priv;
...
static void gdict_database_chooser_set_property(GObject *gobject, guint prop_id,
                                                const GValue *value,
                                                GParamSpec *pspec) {
  GdictDatabaseChooser *chooser = GDICT_DATABASE_CHOOSER(gobject);
...
static void gdict_database_chooser_get_property(GObject *gobject, guint prop_id,
                                                GValue *value,
                                                GParamSpec *pspec) {
  GdictDatabaseChooser *chooser = GDICT_DATABASE_CHOOSER(gobject);
...
static void row_activated_cb(GtkTreeView *treeview, GtkTreePath *path,
  ...
  gtk_tree_model_get(GTK_TREE_MODEL(priv->store), &iter, DB_COLUMN_NAME,
                     &db_name, DB_COLUMN_DESCRIPTION, &db_desc, -1);
  if (db_name && db_desc) {
    g_free(priv->current_db);
    priv->current_db = g_strdup(db_name);

    g_signal_emit(chooser, db_chooser_signals[DATABASE_ACTIVATED], 0, db_name,
                  db_desc);
  } else {
...
static void refresh_button_clicked_cb(GtkWidget *widget, gpointer user_data) {
  GdictDatabaseChooser *chooser = GDICT_DATABASE_CHOOSER(user_data);

  gdict_database_chooser_refresh(chooser);
}

static void clear_button_clicked_cb(GtkWidget *widget, gpointer user_data) {
  GdictDatabaseChooser *chooser = GDICT_DATABASE_CHOOSER(user_data);
...
static GObject *gdict_database_chooser_constructor(
    GType type, guint n_params, GObjectConstructParam *params) {
  ...
  column = gtk_tree_view_column_new_with_attributes(
      "databases", renderer, "text", DB_COLUMN_DESCRIPTION, "weight",
      DB_COLUMN_CURRENT, NULL);
...
static void gdict_database_chooser_class_init(
    GdictDatabaseChooserClass *klass) {
  ...
  g_object_class_install_property(
      ...
      g_param_spec_object(
          ...
          GDICT_TYPE_CONTEXT, (G_PARAM_READWRITE | G_PARAM_CONSTRUCT)));
  /**
  ...
  g_object_class_install_property(
      ...
      g_param_spec_int("count", "Count", "The number of available databases",
   ...
   * Since: 0.10
   */
  db_chooser_signals[DATABASE_ACTIVATED] = g_signal_new(
      "database-activated", G_OBJECT_CLASS_TYPE(gobject_class),
      G_SIGNAL_RUN_LAST,
      G_STRUCT_OFFSET(GdictDatabaseChooserClass, database_activated), NULL,
      NULL, gdict_marshal_VOID__STRING_STRING, G_TYPE_NONE, 2, G_TYPE_STRING,
      G_TYPE_STRING);
  /**
   ...
   * Since: 0.12
   */
  db_chooser_signals[SELECTION_CHANGED] = g_signal_new(
      "selection-changed", G_OBJECT_CLASS_TYPE(gobject_class),
      G_SIGNAL_RUN_LAST,
      G_STRUCT_OFFSET(GdictDatabaseChooserClass, selection_changed), NULL, NULL,
      gdict_marshal_VOID__VOID, G_TYPE_NONE, 0);
}

static void gdict_database_chooser_init(GdictDatabaseChooser *chooser) {
  GdictDatabaseChooserPrivate *priv;
  ...
  priv->store = gtk_list_store_new(DB_N_COLUMNS, G_TYPE_INT, /* db_type */
...
GtkWidget *gdict_database_chooser_new_with_context(GdictContext *context) {
  ...
  return g_object_new(GDICT_TYPE_DATABASE_CHOOSER, "context", context, NULL);
}
...
static void lookup_start_cb(GdictContext *context, gpointer user_data) {
  ...
  if (gtk_widget_get_window(GTK_WIDGET(chooser)))
    gdk_window_set_cursor(gtk_widget_get_window(GTK_WIDGET(chooser)),
                          priv->busy_cursor);
...
static void database_found_cb(GdictContext *context, GdictDatabase *database,
                              gpointer user_data) {
  GdictDatabaseChooser *chooser = GDICT_DATABASE_CHOOSER(user_data);
  GdictDatabaseChooserPrivate *priv = chooser->priv;
  GtkTreeIter iter;
  ...
  GDICT_NOTE(CHOOSER, "DATABASE: `%s' (`%s')", name, full_name);

  gtk_list_store_append(priv->store, &iter);
  gtk_list_store_set(priv->store, &iter, DB_COLUMN_TYPE, DATABASE_NAME,
                     DB_COLUMN_NAME, name, DB_COLUMN_DESCRIPTION, full_name,
                     DB_COLUMN_CURRENT, weight, -1);
...
void gdict_database_chooser_refresh(GdictDatabaseChooser *chooser) {
  ...
  if (!priv->context) {
    g_warning(
        "Attempting to retrieve the available databases, but "
        "no GdictContext has been set.  Use "
        "gdict_database_chooser_set_context() "
  ...
  if (!priv->start_id) {
    priv->start_id = g_signal_connect(priv->context, "lookup-start",
                                      G_CALLBACK(lookup_start_cb), chooser);
    priv->match_id = g_signal_connect(priv->context, "database-found",
                                      G_CALLBACK(database_found_cb), chooser);
    priv->end_id = g_signal_connect(priv->context, "lookup-end",
                                    G_CALLBACK(lookup_end_cb), chooser);
  }

  if (!priv->error_id)
    priv->error_id =
        g_signal_connect(priv->context, "error", G_CALLBACK(error_cb), chooser);
  ...
  if (db_error) {
    ...
    gtk_list_store_append(priv->store, &iter);
    gtk_list_store_set(priv->store, &iter, DB_COLUMN_TYPE, DATABASE_ERROR,
                       DB_COLUMN_NAME, _("Error while matching"),
                       DB_COLUMN_DESCRIPTION, NULL, -1);

    g_warning("Error while looking for databases: %s", db_error->message);
...
static gboolean scan_for_db_name(GtkTreeModel *model, GtkTreePath *path,
  ...
  if (strcmp(db_name, select_data->db_name) == 0) {
    ...
    if (select_data->do_activate) {
      ...
      gtk_list_store_set(GTK_LIST_STORE(model), iter, DB_COLUMN_CURRENT,
                         PANGO_WEIGHT_BOLD, -1);
...
gboolean gdict_database_chooser_select_database(GdictDatabaseChooser *chooser,
  ...
  gtk_tree_model_foreach(GTK_TREE_MODEL(priv->store), scan_for_db_name, &data);

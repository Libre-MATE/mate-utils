...
typedef enum { MATCH_DB, MATCH_WORD, MATCH_ERROR } MatchType;
...
static void set_gdict_context(GdictSpeller *speller, GdictContext *context) {
...
static void gdict_speller_finalize(GObject *gobject) {
  GdictSpeller *speller = GDICT_SPELLER(gobject);
  GdictSpellerPrivate *priv = speller->priv;

  if (priv->context) set_gdict_context(speller, NULL);

  if (priv->busy_cursor) g_object_unref(priv->busy_cursor);

  g_free(priv->strategy);
  g_free(priv->database);
  g_free(priv->word);

  if (priv->store) g_object_unref(priv->store);

  G_OBJECT_CLASS(gdict_speller_parent_class)->finalize(gobject);
}

static void gdict_speller_set_property(GObject *gobject, guint prop_id,
...
static void gdict_speller_get_property(GObject *gobject, guint prop_id,
...
static void row_activated_cb(GtkTreeView *treeview, GtkTreePath *path,
  ...
  gtk_tree_model_get(GTK_TREE_MODEL(priv->store), &iter, MATCH_COLUMN_WORD,
                     &word, MATCH_COLUMN_DB_NAME, &db_name, -1);
  if (word)
    g_signal_emit(speller, speller_signals[WORD_ACTIVATED], 0, word, db_name);
  else {
...
static GObject *gdict_speller_constructor(GType type, guint n_params,
  ...
  object = G_OBJECT_CLASS(gdict_speller_parent_class)
  ...
  column = gtk_tree_view_column_new_with_attributes("matches", renderer, "text",
  ...
  g_signal_connect(priv->clear_button, "clicked",
                   G_CALLBACK(clear_button_clicked_cb), speller);
...
static void gdict_speller_class_init(GdictSpellerClass *klass) {
  ...
  speller_signals[WORD_ACTIVATED] = g_signal_new(
      "word-activated", G_OBJECT_CLASS_TYPE(gobject_class), G_SIGNAL_RUN_LAST,
      G_STRUCT_OFFSET(GdictSpellerClass, word_activated), NULL, NULL,
      gdict_marshal_VOID__STRING_STRING, G_TYPE_NONE, 2, G_TYPE_STRING,
      G_TYPE_STRING);
}
...
static void gdict_speller_init(GdictSpeller *speller) {
  ...
  gtk_orientable_set_orientation(GTK_ORIENTABLE(speller),
  ...
  priv->store = gtk_list_store_new(MATCH_N_COLUMNS, G_TYPE_INT, /* MatchType */
...
GtkWidget *gdict_speller_new_with_context(GdictContext *context) {
  ...
  return g_object_new(GDICT_TYPE_SPELLER, "context", context, NULL);
}
...
void gdict_speller_set_database(GdictSpeller *speller, const gchar *database) {
  ...
  if (!database || database[0] == '\0') database = GDICT_DEFAULT_DATABASE;
...
void gdict_speller_set_strategy(GdictSpeller *speller, const gchar *strategy) {
  ...
  if (!strategy || strategy[0] == '\0') strategy = GDICT_DEFAULT_STRATEGY;
...
static void lookup_start_cb(GdictContext *context, gpointer user_data) {
  ...
  if (gtk_widget_get_window(GTK_WIDGET(speller)))
    gdk_window_set_cursor(gtk_widget_get_window(GTK_WIDGET(speller)),
                          priv->busy_cursor);
...
static void match_found_cb(GdictContext *context, GdictMatch *match,
                           gpointer user_data) {
  GdictSpeller *speller = GDICT_SPELLER(user_data);
  GdictSpellerPrivate *priv = speller->priv;
  GtkTreeIter iter;

  GDICT_NOTE(SPELLER, "MATCH: `%s' (from `%s')", gdict_match_get_word(match),
             gdict_match_get_database(match));

  gtk_list_store_append(priv->store, &iter);
  gtk_list_store_set(priv->store, &iter, MATCH_COLUMN_TYPE, MATCH_WORD,
                     MATCH_COLUMN_DB_NAME, gdict_match_get_database(match),
                     MATCH_COLUMN_WORD, gdict_match_get_word(match), -1);
...
void gdict_speller_match(GdictSpeller *speller, const gchar *word) {
  ...
  if (!priv->context) {
    g_warning(
        "Attempting to match `%s', but no GdictContext "
  ...
  if (priv->is_searching) {
    _gdict_show_error_dialog(NULL, _("Another search is in progress"),
  ...
  if (!priv->start_id) {
    priv->start_id = g_signal_connect(priv->context, "lookup-start",
                                      G_CALLBACK(lookup_start_cb), speller);
    priv->match_id = g_signal_connect(priv->context, "match-found",
                                      G_CALLBACK(match_found_cb), speller);
    priv->end_id = g_signal_connect(priv->context, "lookup-end",
                                    G_CALLBACK(lookup_end_cb), speller);
  }

  if (!priv->error_id)
    priv->error_id =
        g_signal_connect(priv->context, "error", G_CALLBACK(error_cb), speller);

  g_free(priv->word);
  priv->word = g_strdup(word);

  match_error = NULL;
  gdict_context_match_word(priv->context, priv->database, priv->strategy,
                           priv->word, &match_error);
  if (match_error) {
    GtkTreeIter iter;

    gtk_list_store_append(priv->store, &iter);
    gtk_list_store_set(priv->store, &iter, MATCH_COLUMN_TYPE, MATCH_ERROR,
                       MATCH_COLUMN_DB_NAME, _("Error while matching"),
                       MATCH_COLUMN_WORD, NULL, -1);

    g_warning("Error while matching `%s': %s", priv->word,
              match_error->message);

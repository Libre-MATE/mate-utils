...
static const gchar *dict_command_strings[] = {
    "CLIENT", "SHOW DB", "SHOW STRAT",  "SHOW INFO", "SHOW SERVER", "MATCH",
    "DEFINE", "STATUS",  "OPTION MIME", "AUTH",      "HELP",        "QUIT",
...
G_DEFINE_TYPE_WITH_CODE(GdictClientContext, gdict_client_context, G_TYPE_OBJECT,
                        G_ADD_PRIVATE(GdictClientContext) G_IMPLEMENT_INTERFACE(
                            GDICT_TYPE_CONTEXT,
                            gdict_client_context_iface_init));

/* GObject methods */
static void gdict_client_context_set_property(GObject *object, guint prop_id,
                                              const GValue *value,
                                              GParamSpec *pspec);
static void gdict_client_context_get_property(GObject *object, guint prop_id,
...
static void gdict_client_context_class_init(GdictClientContextClass *klass) {
  ...
  g_object_class_override_property(gobject_class, GDICT_CONTEXT_PROP_LOCAL_ONLY,
                                   "local-only");
  ...
  g_object_class_install_property(
      gobject_class, PROP_CLIENT_NAME,
      g_param_spec_string("client-name", _("Client Name"),
  ...
  g_object_class_install_property(
      ...
      g_param_spec_string(
          "hostname", _("Hostname"),
          _("The hostname of the dictionary server to connect to"), NULL,
  ...
  g_object_class_install_property(
      ...
      g_param_spec_uint("port", _("Port"),
                        _("The port of the dictionary server to connect to"), 0,
                        65535, GDICT_DEFAULT_PORT,
                        (G_PARAM_READABLE | G_PARAM_WRITABLE)));
  /**
  ...
  g_object_class_install_property(
      ...
      g_param_spec_enum(
          ...
          GDICT_TYPE_STATUS_CODE, GDICT_STATUS_INVALID, G_PARAM_READABLE));
   ...
   * Since: 1.0
   */
  gdict_client_context_signals[CONNECTED] = g_signal_new(
      "connected", G_OBJECT_CLASS_TYPE(gobject_class), G_SIGNAL_RUN_LAST,
      G_STRUCT_OFFSET(GdictClientContextClass, connected), NULL, NULL,
   ...
   * Since: 1.0
   */
  gdict_client_context_signals[DISCONNECTED] = g_signal_new(
      "disconnected", G_OBJECT_CLASS_TYPE(gobject_class), G_SIGNAL_RUN_LAST,
      G_STRUCT_OFFSET(GdictClientContextClass, disconnected), NULL, NULL,
      gdict_marshal_VOID__VOID, G_TYPE_NONE, 0);
...
static void gdict_client_context_init(GdictClientContext *context) {
...
static void gdict_client_context_set_property(GObject *object, guint prop_id,
                                              ...
                                              GParamSpec *pspec) {
  GdictClientContextPrivate *priv =
      gdict_client_context_get_instance_private(GDICT_CLIENT_CONTEXT(object));
...
static void gdict_client_context_finalize(GObject *object) {
  ...
  if (priv->commands_queue) {
    g_queue_free_full(priv->commands_queue, (GDestroyNotify)gdict_command_free);
...
#ifdef ENABLE_IPV6
  if (priv->host6info) freeaddrinfo(priv->host6info);
#endif
...
GdictContext *gdict_client_context_new(const gchar *hostname, gint port) {
  return g_object_new(GDICT_TYPE_CLIENT_CONTEXT, "hostname",
                      (hostname != NULL ? hostname : GDICT_DEFAULT_HOSTNAME),
                      "port", (port != -1 ? port : GDICT_DEFAULT_PORT),
                      "client-name", GDICT_DEFAULT_CLIENT, NULL);
}
...
void gdict_client_context_set_hostname(GdictClientContext *context,
  ...
  g_object_set(G_OBJECT(context), "hostname",
               (hostname != NULL ? hostname : GDICT_DEFAULT_HOSTNAME), NULL);
}
...
void gdict_client_context_set_port(GdictClientContext *context, gint port) {
  ...
  g_object_set(G_OBJECT(context), "port",
               (port != -1 ? port : GDICT_DEFAULT_PORT), NULL);
}
...
void gdict_client_context_set_client(GdictClientContext *context,
  ...
  g_object_set(G_OBJECT(context), "client-name",
               (client != NULL ? client : GDICT_DEFAULT_CLIENT), NULL);
}
...
static void gdict_command_free(GdictCommand *cmd) {
  ...
  if (cmd->buffer) g_string_free(cmd->buffer, TRUE);

  if (cmd->data_destroy) cmd->data_destroy(cmd->data);
 ...
 * tail.
 */
static gboolean gdict_client_context_push_command(GdictClientContext *context,
                                                  GdictCommand *command) {
  GdictClientContextPrivate *priv;
  ...
  if (g_queue_find(priv->commands_queue, command)) {
    g_warning(
        "gdict_client_context_push_command() called on a command already in "
        "queue\n");
    return FALSE;
  }
...
static GdictCommand *gdict_client_context_pop_command(
    GdictClientContext *context) {
  GdictClientContextPrivate *priv;
  GdictCommand *retval;
...
static gboolean gdict_client_context_send_command(GdictClientContext *context,
                                                  GdictCommand *command,
                                                  GError **error) {
  GdictClientContextPrivate *priv;
  GError *write_error;
  ...
  write_error = NULL;
  res = g_io_channel_write_chars(priv->channel, command->cmd_string, -1,
                                 &written_bytes, &write_error);
  ...
  GDICT_NOTE(DICT, "Wrote %" G_GSIZE_FORMAT " bytes to the channel",
             written_bytes);
 ...
 * server.
 */
static gboolean gdict_client_context_run_command(GdictClientContext *context,
                                                 GdictCommand *command,
                                                 GError **error) {
  GdictClientContextPrivate *priv;
  gchar *payload;
  ...
  switch (command->cmd_type) {
    case CMD_CLIENT:
      ...
      command->cmd_string = g_strdup_printf(
          "%s %s\r\n", dict_command_strings[CMD_CLIENT], payload);
      g_free(payload);
      break;
    case CMD_QUIT:
      command->cmd_string =
    ...
    case CMD_MATCH:
      ...
      command->cmd_string = g_strdup_printf(
          ...
          (command->strategy != NULL ? command->strategy : "*"), payload);
      g_free(payload);
      break;
    case CMD_DEFINE:
      g_assert(command->word);
      ...
      command->cmd_string = g_strdup_printf(
          ...
          (command->database != NULL ? command->database : "!"), payload);
      g_free(payload);
      break;
    default:
...
static void clear_command_queue(GdictClientContext *context) {
  GdictClientContextPrivate *priv = context->priv;
...
#ifdef ENABLE_IPV6
  if (!priv->host6info) return;
#endif

  if (!priv->hostinfo) return;
...
static gboolean gdict_client_context_lookup_server(GdictClientContext *context,
  ...
  time(&now);
  if (now >= (priv->last_lookup + HOSTNAME_LOOKUP_EXPIRE)) is_expired = TRUE;

    /* we already have resolved the hostname */
#ifdef ENABLE_IPV6
  if (priv->host6info && !is_expired) return TRUE;
#endif

  if (priv->hostinfo && !is_expired) return TRUE;
  ...
  if (_gdict_has_ipv6()) {
    ...
    if (getaddrinfo(priv->hostname, NULL, &hints, &(priv->host6info)) == 0) {
      for (res = priv->host6info; res; res = res->ai_next)
        if (res->ai_family == AF_INET6 || res->ai_family == AF_INET) break;
      ...
      if (!res) {
        ...
        GDICT_NOTE(DICT, "Hostname '%s' found (using IPv6)", priv->hostname);
      ...
      g_set_error(error, GDICT_CLIENT_CONTEXT_ERROR,
                  GDICT_CLIENT_CONTEXT_ERROR_LOOKUP,
                  _("Lookup failed for host '%s': %s"), priv->hostname,
                  gai_strerror(errno));
    ...
    if (priv->hostinfo) {
      memcpy(&((struct sockaddr_in *)&(priv->sockaddr))->sin_addr,
             priv->hostinfo->h_addr, priv->hostinfo->h_length);

      GDICT_NOTE(DICT, "Hostname '%s' found (using IPv4)", priv->hostname);
      ...
      g_set_error(
          error, GDICT_CLIENT_CONTEXT_ERROR, GDICT_CLIENT_CONTEXT_ERROR_LOOKUP,
          _("Lookup failed for host '%s': host not found"), priv->hostname);
...
static gboolean gdict_client_context_parse_line(GdictClientContext *context,
    ...
    if (priv->status_code == GDICT_STATUS_CONNECT) {
      ...
      g_set_error(&server_error, GDICT_CLIENT_CONTEXT_ERROR,
                  ...
                  _("Unable to connect to the dictionary server "
                    ...
                    "code %d (server down)"),
                  priv->hostname, priv->port, priv->status_code);
      ...
      g_set_error(&parse_error, GDICT_CONTEXT_ERROR, GDICT_CONTEXT_ERROR_PARSE,
  ...
  switch (priv->status_code) {
    case GDICT_STATUS_NO_MATCH:
      g_set_error(&server_error, GDICT_CONTEXT_ERROR,
                  ...
                  _("No definitions found for '%s'"), priv->command->word);
    ...
    case GDICT_STATUS_BAD_DATABASE:
      g_set_error(&server_error, GDICT_CONTEXT_ERROR,
                  ...
                  _("Invalid database '%s'"), priv->command->database);
    ...
    case GDICT_STATUS_BAD_STRATEGY:
      g_set_error(&server_error, GDICT_CONTEXT_ERROR,
                  ...
                  _("Invalid strategy '%s'"), priv->command->strategy);
    ...
    case GDICT_STATUS_BAD_COMMAND:
      g_set_error(&server_error, GDICT_CONTEXT_ERROR,
                  GDICT_CONTEXT_ERROR_INVALID_COMMAND, _("Bad command '%s'"),
    ...
    case GDICT_STATUS_NO_DATABASES_PRESENT:
      g_set_error(
          &server_error, GDICT_CONTEXT_ERROR, GDICT_CONTEXT_ERROR_NO_DATABASES,
          _("No databases found on dictionary server at '%s'"), priv->hostname);
  ...
  GDICT_NOTE(DICT, "check command %d ('%s')[state:%d]", priv->command->cmd_type,
             dict_command_strings[priv->command->cmd_type],
             priv->command->state);
  ...
  switch (priv->command->cmd_type) {
    ...
    case CMD_SHOW_DB:
      if (priv->status_code == GDICT_STATUS_N_DATABASES_PRESENT) {
        ...
        if (full[0] == '\"') full = g_utf8_next_char(full);
    ...
    case CMD_SHOW_STRAT:
      if (priv->status_code == GDICT_STATUS_N_STRATEGIES_PRESENT) {
        ...
        if (desc[0] == '\"') desc = g_utf8_next_char(desc);
    ...
    case CMD_DEFINE:
      if (priv->status_code == GDICT_STATUS_N_DEFINITIONS_RETRIEVED) {
        ...
        if (word[0] == '\"') word = g_utf8_next_char(word);
        ...
        if (db_full[0] == '\"') db_full = g_utf8_next_char(db_full);
        ...
        if (!priv->command->buffer) priv->command->buffer = g_string_new(NULL);
...
static gint get_status_code(const gchar *line, gint old_status) {
  gchar *status;
...
static gboolean gdict_client_context_io_watch_cb(GIOChannel *channel,
                                                 GIOCondition condition,
                                                 GdictClientContext *context) {
  GdictClientContextPrivate *priv;
  ...
  while (1) {
    ...
    if (res == G_IO_STATUS_ERROR) {
      if (read_err) {
        ...
        g_set_error(
            &err, GDICT_CLIENT_CONTEXT_ERROR, GDICT_CLIENT_CONTEXT_ERROR_SOCKET,
...
static gboolean check_for_connection(gpointer data) {
  ...
  if (context->priv->is_connecting) {
    ...
    g_set_error(&err, GDICT_CLIENT_CONTEXT_ERROR,
                ...
                context->priv->hostname, context->priv->port);
...
static gboolean gdict_client_context_connect(GdictClientContext *context,
                                             GError **error) {
  GdictClientContextPrivate *priv;
  ...
  if (!priv->port) priv->port = GDICT_DEFAULT_PORT;
  ...
  priv->timeout_id = g_timeout_add_seconds(CONNECTION_TIMEOUT_SEC,
                                           check_for_connection, context);

  /* XXX - remember that g_io_add_watch() increases the reference count
   * of the GIOChannel we are using.
   */
  priv->source_id =
      g_io_add_watch(priv->channel, (G_IO_IN | G_IO_ERR),
                     (GIOFunc)gdict_client_context_io_watch_cb, context);
...
static gboolean gdict_client_context_is_connected(GdictClientContext *context) {
  g_assert(GDICT_IS_CLIENT_CONTEXT(context));

  /* we are in the middle of a connection attempt */
  if (context->priv->is_connecting) return TRUE;
...
static gboolean gdict_client_context_define_word(GdictContext *context,
  ...
  cmd->database =
      g_strdup((database != NULL ? database : GDICT_DEFAULT_DATABASE));
...
static gboolean gdict_client_context_match_word(GdictContext *context,
  ...
  cmd->database =
      g_strdup((database != NULL ? database : GDICT_DEFAULT_DATABASE));
  cmd->strategy =
      g_strdup((strategy != NULL ? strategy : GDICT_DEFAULT_STRATEGY));
  cmd->word = g_utf8_normalize(word, -1, G_NORMALIZE_NFC);

...
#define IS_VALID_TRANSPORT(t)               \
  (((t) >= GDICT_SOURCE_TRANSPORT_DICTD) && \
...
static void gdict_source_set_property(GObject *object, guint prop_id,
...
static void gdict_source_get_property(GObject *object, guint prop_id,
...
static void gdict_source_finalize(GObject *object) {
  GdictSourcePrivate *priv =
      gdict_source_get_instance_private(GDICT_SOURCE(object));

  g_free(priv->filename);

  if (priv->keyfile) g_key_file_free(priv->keyfile);
  ...
  if (priv->context) g_object_unref(priv->context);

  G_OBJECT_CLASS(gdict_source_parent_class)->finalize(object);
}

static void gdict_source_class_init(GdictSourceClass *klass) {
  ...
  g_object_class_install_property(
      ...
      g_param_spec_string("name", _("Name"),
                          _("The display name of this dictionary source"), NULL,
  ...
  g_object_class_install_property(
      ...
      g_param_spec_string("description", _("Description"),
                          _("The description of this dictionary source"), NULL,
                          (G_PARAM_READABLE | G_PARAM_WRITABLE)));
  /**
  ...
  g_object_class_install_property(
      ...
      g_param_spec_string("database", _("Database"),
   ...
   */
  g_object_class_install_property(
      gobject_class, PROP_STRATEGY,
      g_param_spec_string("strategy", _("Strategy"),
   ...
   */
  g_object_class_install_property(
      gobject_class, PROP_TRANSPORT,
      g_param_spec_enum(
          "transport", _("Transport"),
          _("The transport mechanism used by this dictionary source"),
          GDICT_TYPE_SOURCE_TRANSPORT, GDICT_SOURCE_TRANSPORT_INVALID,
          (G_PARAM_READABLE | G_PARAM_WRITABLE)));
  /**
   * GdictSource:context
   *
  ...
  g_object_class_install_property(
      ...
      g_param_spec_object("context", _("Context"),
                          ...
                          GDICT_TYPE_CONTEXT, G_PARAM_READABLE));
}

static void gdict_source_init(GdictSource *source) {
  GdictSourcePrivate *priv;
...
static GdictSourceTransport gdict_source_resolve_transport(
    const gchar *transport) {
  if (!transport) return GDICT_SOURCE_TRANSPORT_INVALID;
...
static GdictContext *gdict_source_create_context(GdictSource *source,
                                                 GdictSourceTransport transport,
                                                 GError **error) {
  GdictSourcePrivate *priv;
  GdictContext *context;
  ...
  switch (transport) {
    case GDICT_SOURCE_TRANSPORT_DICTD: {
      ...
      hostname = g_key_file_get_string(priv->keyfile, SOURCE_GROUP,
                                       SOURCE_KEY_HOSTNAME, NULL);

      port = g_key_file_get_integer(priv->keyfile, SOURCE_GROUP,
                                    SOURCE_KEY_PORT, NULL);
      if (!port) port = -1;
    ...
    default:
      g_set_error(error, GDICT_SOURCE_ERROR, GDICT_SOURCE_ERROR_PARSE,
                  _("Invalid transport type '%d'"), transport);
      return NULL;
  }

  g_assert(context != NULL);

  if (priv->transport != transport) priv->transport = transport;

  return context;
}

static gboolean gdict_source_parse(GdictSource *source, GError **error) {
  GdictSourcePrivate *priv;
  GError *parse_error;
  gchar *transport;
  ...
  if (!g_key_file_has_group(priv->keyfile, SOURCE_GROUP)) {
    g_set_error(
        error, GDICT_SOURCE_ERROR, GDICT_SOURCE_ERROR_PARSE,
  ...
  parse_error = NULL;
  priv->name = g_key_file_get_string(priv->keyfile, SOURCE_GROUP,
                                     SOURCE_KEY_NAME, &parse_error);
  if (parse_error) {
    g_set_error(error, GDICT_SOURCE_ERROR, GDICT_SOURCE_ERROR_PARSE,
                _("Unable to get the '%s' key inside the dictionary "
                  "source definition: %s"),
                SOURCE_KEY_NAME, parse_error->message);
  ...
  if (g_key_file_has_key(priv->keyfile, SOURCE_GROUP, SOURCE_KEY_DESCRIPTION,
                         NULL)) {
    priv->description = g_key_file_get_locale_string(
        priv->keyfile, SOURCE_GROUP, SOURCE_KEY_DESCRIPTION, NULL,
        &parse_error);
    if (parse_error) {
      g_set_error(error, GDICT_SOURCE_ERROR, GDICT_SOURCE_ERROR_PARSE,
                  _("Unable to get the '%s' key inside the dictionary "
                    "source definition: %s"),
                  SOURCE_KEY_DESCRIPTION, parse_error->message);
  ...
  if (g_key_file_has_key(priv->keyfile, SOURCE_GROUP, SOURCE_KEY_DATABASE,
                         NULL)) {
    priv->database = g_key_file_get_string(priv->keyfile, SOURCE_GROUP,
                                           SOURCE_KEY_DATABASE, &parse_error);
    if (parse_error) {
      g_set_error(error, GDICT_SOURCE_ERROR, GDICT_SOURCE_ERROR_PARSE,
                  _("Unable to get the '%s' key inside the dictionary "
                    "source definition: %s"),
                  SOURCE_KEY_DATABASE, parse_error->message);
  ...
  if (g_key_file_has_key(priv->keyfile, SOURCE_GROUP, SOURCE_KEY_STRATEGY,
                         NULL)) {
    priv->strategy = g_key_file_get_string(priv->keyfile, SOURCE_GROUP,
                                           SOURCE_KEY_STRATEGY, &parse_error);
    if (parse_error) {
      g_set_error(error, GDICT_SOURCE_ERROR, GDICT_SOURCE_ERROR_PARSE,
                  _("Unable to get the '%s' key inside the dictionary "
                    "source definition: %s"),
                  SOURCE_KEY_STRATEGY, parse_error->message);
  ...
  transport = g_key_file_get_string(priv->keyfile, SOURCE_GROUP,
                                    SOURCE_KEY_TRANSPORT, &parse_error);
  if (parse_error) {
    g_set_error(error, GDICT_SOURCE_ERROR, GDICT_SOURCE_ERROR_PARSE,
                _("Unable to get the '%s' key inside the dictionary "
                  "source definition file: %s"),
                SOURCE_KEY_TRANSPORT, parse_error->message);
...
gboolean gdict_source_load_from_file(GdictSource *source, const gchar *filename,
  ...
  if (!priv->keyfile) priv->keyfile = g_key_file_new();

  read_error = NULL;
  g_key_file_load_from_file(priv->keyfile, filename,
                            G_KEY_FILE_KEEP_TRANSLATIONS, &read_error);
  if (read_error) {
...
gboolean gdict_source_load_from_data(GdictSource *source, const gchar *data,
  ...
  read_error = NULL;
  g_key_file_load_from_data(priv->keyfile, data, length,
                            G_KEY_FILE_KEEP_TRANSLATIONS, &read_error);
  if (read_error) {
    g_propagate_error(error, read_error);
...
gchar *gdict_source_to_data(GdictSource *source, gsize *length,
  ...
  if (!priv->name) {
    g_set_error(error, GDICT_SOURCE_ERROR, GDICT_SOURCE_ERROR_INVALID_NAME,
  ...
  if (!IS_VALID_TRANSPORT(priv->transport)) {
    g_set_error(error, GDICT_SOURCE_ERROR, GDICT_SOURCE_ERROR_INVALID_TRANSPORT,
                _("Dictionary source '%s' has invalid transport '%s'"),
                priv->name, valid_transports[priv->transport]);
  ...
  if (priv->keyfile) {
    ...
    retval = g_key_file_to_data(priv->keyfile, length, &write_error);
    if (write_error) g_propagate_error(error, write_error);
  }
...
void gdict_source_set_name(GdictSource *source, const gchar *name) {
  ...
  if (!source->priv->keyfile) source->priv->keyfile = g_key_file_new();

  g_key_file_set_string(source->priv->keyfile, SOURCE_GROUP, SOURCE_KEY_NAME,
                        name);
}
...
void gdict_source_set_description(GdictSource *source,
  ...
  if (description && description[0] != '\0') {
    ...
    g_key_file_set_string(source->priv->keyfile, SOURCE_GROUP,
                          SOURCE_KEY_DESCRIPTION, description);
  } else {
    if (g_key_file_has_key(source->priv->keyfile, SOURCE_GROUP,
                           SOURCE_KEY_DESCRIPTION, NULL))
      g_key_file_remove_key(source->priv->keyfile, SOURCE_GROUP,
                            SOURCE_KEY_DESCRIPTION, NULL);
  }
}
...
void gdict_source_set_database(GdictSource *source, const gchar *database) {
  ...
  if (database && database[0] != '\0') {
    ...
    g_key_file_set_string(source->priv->keyfile, SOURCE_GROUP,
                          SOURCE_KEY_DATABASE, database);
  } else {
    if (g_key_file_has_key(source->priv->keyfile, SOURCE_GROUP,
                           SOURCE_KEY_DATABASE, NULL))
      g_key_file_remove_key(source->priv->keyfile, SOURCE_GROUP,
                            SOURCE_KEY_DATABASE, NULL);
  }
}
...
void gdict_source_set_strategy(GdictSource *source, const gchar *strategy) {
  ...
  if (strategy && strategy[0] != '\0') {
    ...
    g_key_file_set_string(source->priv->keyfile, SOURCE_GROUP,
                          SOURCE_KEY_STRATEGY, strategy);
  } else {
    if (g_key_file_has_key(source->priv->keyfile, SOURCE_GROUP,
                           SOURCE_KEY_STRATEGY, NULL))
      g_key_file_remove_key(source->priv->keyfile, SOURCE_GROUP,
                            SOURCE_KEY_STRATEGY, NULL);
  }
}
...
void gdict_source_set_transportv(GdictSource *source,
  ...
  switch (priv->transport) {
    case GDICT_SOURCE_TRANSPORT_DICTD:
      ...
      g_object_set_valist(G_OBJECT(priv->context), first_transport_property,
                          var_args);
  ...
  g_key_file_set_string(priv->keyfile, SOURCE_GROUP, SOURCE_KEY_TRANSPORT,
                        valid_transports[transport]);

  switch (priv->transport) {
    case GDICT_SOURCE_TRANSPORT_DICTD:
      g_key_file_set_string(priv->keyfile, SOURCE_GROUP, SOURCE_KEY_HOSTNAME,
                            gdict_client_context_get_hostname(
                                GDICT_CLIENT_CONTEXT(priv->context)));
...
void gdict_source_set_transport(GdictSource *source,
  ...
  gdict_source_set_transportv(source, transport, first_transport_property,
                              args);
...
GdictContext *gdict_source_get_context(GdictSource *source) {
  ...
  retval = gdict_source_create_context(source, source->priv->transport, NULL);
...
GdictContext *gdict_source_peek_context(GdictSource *source) {
  ...
  if (!source->priv->context)
    source->priv->context =
        gdict_source_create_context(source, source->priv->transport, NULL);

...
static gboolean select_active_source_name(GtkTreeModel *model,
                                          GtkTreePath *path, GtkTreeIter *iter,
                                          gpointer data) {
  GdictPrefDialog *dialog = GDICT_PREF_DIALOG(data);
  gboolean is_active;

  gtk_tree_model_get(model, iter, SOURCES_ACTIVE_COLUMN, &is_active, -1);
  if (is_active) {
    GtkTreeSelection *selection;

    selection =
        gtk_tree_view_get_selection(GTK_TREE_VIEW(dialog->sources_view));
...
static void update_sources_view(GdictPrefDialog *dialog) {
  ...
  for (l = sources; l != NULL; l = l->next) {
    ...
    if (strcmp(name, dialog->active_source) == 0) is_active = TRUE;

    gtk_list_store_append(dialog->sources_list, &iter);
    gtk_list_store_set(dialog->sources_list, &iter, SOURCES_ACTIVE_COLUMN,
                       is_active, SOURCES_NAME_COLUMN, name,
                       SOURCES_DESCRIPTION_COLUMN, description, -1);
  }
  ...
  gtk_tree_model_foreach(GTK_TREE_MODEL(dialog->sources_list),
                         select_active_source_name, dialog);
}

static void source_renderer_toggled_cb(GtkCellRendererToggle *renderer,
                                       const gchar *path,
  ...
  res = gtk_tree_model_get_iter(GTK_TREE_MODEL(dialog->sources_list), &iter,
                                treepath);
  if (!res) {
  ...
  gtk_tree_model_get(GTK_TREE_MODEL(dialog->sources_list), &iter,
                     SOURCES_NAME_COLUMN, &name, SOURCES_ACTIVE_COLUMN,
                     &is_active, -1);
  if (!is_active && name != NULL) {
    g_free(dialog->active_source);
    dialog->active_source = g_strdup(name);

    g_settings_set_string(dialog->settings, GDICT_SETTINGS_SOURCE_KEY,
...
static void sources_view_row_activated_cb(GtkTreeView *tree_view,
  ...
  if (!gtk_tree_model_get_iter(model, &iter, tree_path)) return;

  gtk_tree_model_get(model, &iter, SOURCES_NAME_COLUMN, &source_name, -1);
  if (!source_name) return;

  edit_dialog = gdict_source_dialog_new(
      GTK_WINDOW(dialog), _("Edit Dictionary Source"), GDICT_SOURCE_DIALOG_EDIT,
      dialog->loader, source_name);
...
static void build_sources_view(GdictPrefDialog *dialog) {
  GtkTreeViewColumn *column;
  GtkCellRenderer *renderer;

  if (dialog->sources_list) return;

  dialog->sources_list =
      gtk_list_store_new(SOURCES_N_COLUMNS, G_TYPE_BOOLEAN, /* active */
                         G_TYPE_STRING,                     /* name */
  ...
  g_signal_connect(renderer, "toggled", G_CALLBACK(source_renderer_toggled_cb),
                   dialog);

  column = gtk_tree_view_column_new_with_attributes(
      "active", renderer, "active", SOURCES_ACTIVE_COLUMN, NULL);
  gtk_tree_view_append_column(GTK_TREE_VIEW(dialog->sources_view), column);

  renderer = gtk_cell_renderer_text_new();
  column = gtk_tree_view_column_new_with_attributes(
      "description", renderer, "text", SOURCES_DESCRIPTION_COLUMN, NULL);
  gtk_tree_view_append_column(GTK_TREE_VIEW(dialog->sources_view), column);
...
static void source_add_clicked_cb(GtkWidget *widget, GdictPrefDialog *dialog) {
  GtkWidget *add_dialog;

  add_dialog =
      gdict_source_dialog_new(GTK_WINDOW(dialog), _("Add Dictionary Source"),
                              GDICT_SOURCE_DIALOG_CREATE, dialog->loader, NULL);
...
static void source_remove_clicked_cb(GtkWidget *widget,
                                     GdictPrefDialog *dialog) {
  GtkTreeSelection *selection;
  GtkTreeModel *model;
  GtkTreeIter iter;
  ...
  gtk_tree_model_get(model, &iter, SOURCES_NAME_COLUMN, &name,
                     SOURCES_DESCRIPTION_COLUMN, &description, -1);
  if (!name)
    return;
  else {
    ...
    confirm_dialog = gtk_message_dialog_new(
        GTK_WINDOW(dialog), GTK_DIALOG_DESTROY_WITH_PARENT, GTK_MESSAGE_WARNING,
        GTK_BUTTONS_NONE, _("Remove \"%s\"?"), description);
    gtk_message_dialog_format_secondary_text(
        GTK_MESSAGE_DIALOG(confirm_dialog),
        _("This will permanently remove the "
          "dictionary source from the list."));

    gtk_dialog_add_button(GTK_DIALOG(confirm_dialog), "gtk-cancel",
                          GTK_RESPONSE_CANCEL);
    gtk_dialog_add_button(GTK_DIALOG(confirm_dialog), "gtk-remove",
                          GTK_RESPONSE_OK);
  ...
  if (gdict_source_loader_remove_source(dialog->loader, name))
    ...
    message = g_strdup_printf(_("Unable to remove source '%s'"), description);
...
static void source_edit_clicked_cb(GtkButton *button, GdictPrefDialog *dialog) {
  GtkTreeSelection *selection;
  GtkTreeModel *model;
  GtkTreeIter iter;
  gboolean is_selected;
  ...
  if (!name)
    ...
    edit_dialog =
        gdict_source_dialog_new(GTK_WINDOW(dialog), _("Edit Dictionary Source"),
                                GDICT_SOURCE_DIALOG_EDIT, dialog->loader, name);
    gtk_dialog_run(GTK_DIALOG(edit_dialog));
...
static void set_source_loader(GdictPrefDialog *dialog,
                              GdictSourceLoader *loader) {
  if (!dialog->sources_list) return;

  if (dialog->loader) g_object_unref(dialog->loader);
...
static void font_button_font_set_cb(GtkWidget *font_button,
                                    GdictPrefDialog *dialog) {
  gchar *font;
  ...
  g_settings_set_string(dialog->settings, GDICT_SETTINGS_PRINT_FONT_KEY,
                        dialog->print_font);
  ...
  switch (response_id) {
    case GTK_RESPONSE_HELP:
      ...
      if (err) {
        ...
        message =
            g_strdup_printf(_("There was an error while displaying help"));
        error_dialog = gtk_message_dialog_new(
            GTK_WINDOW(dialog), GTK_DIALOG_DESTROY_WITH_PARENT,
            GTK_MESSAGE_ERROR, GTK_BUTTONS_OK, "%s", message);
        gtk_message_dialog_format_secondary_text(
            GTK_MESSAGE_DIALOG(error_dialog), "%s", err->message);
        gtk_window_set_title(GTK_WINDOW(error_dialog), "");
...
static void gdict_pref_dialog_finalize(GObject *object) {
  ...
  if (dialog->settings) g_object_unref(dialog->settings);

  if (dialog->builder) g_object_unref(dialog->builder);

  if (dialog->active_source) g_free(dialog->active_source);
...
static void gdict_pref_dialog_set_property(GObject *object, guint prop_id,
                                           const GValue *value,
                                           GParamSpec *pspec) {
  GdictPrefDialog *dialog = GDICT_PREF_DIALOG(object);
...
static void gdict_pref_dialog_get_property(GObject *object, guint prop_id,
...
static gboolean on_preferences_notebook_scroll_event(GtkWidget *widget,
...
static void gdict_pref_dialog_class_init(GdictPrefDialogClass *klass) {
  ...
  g_object_class_install_property(
      gobject_class, PROP_SOURCE_LOADER,
      g_param_spec_object(
...
static void gdict_pref_dialog_init(GdictPrefDialog *dialog) {
  gchar *font;
  GError *error = NULL;

  gtk_window_set_default_size(GTK_WINDOW(dialog), DEFAULT_MIN_WIDTH,
                              DEFAULT_MIN_HEIGHT);
  ...
  /* add buttons */
  gtk_dialog_add_button(GTK_DIALOG(dialog), "gtk-help", GTK_RESPONSE_HELP);
  gtk_dialog_add_button(GTK_DIALOG(dialog), "gtk-close", GTK_RESPONSE_ACCEPT);
  ...
  if (error) {
    g_critical("Unable to load the preferences user interface: %s",
               error->message);
    g_error_free(error);
  ...
  g_signal_connect(dialog->notebook, "scroll-event",
                   G_CALLBACK(on_preferences_notebook_scroll_event), NULL);

  dialog->sources_view = GET_WIDGET("sources_treeview");
  build_sources_view(dialog);

  dialog->active_source =
      g_settings_get_string(dialog->settings, GDICT_SETTINGS_SOURCE_KEY);
  ...
  gtk_widget_set_tooltip_text(
      dialog->sources_edit, _("Edit the currently selected dictionary source"));
  ...
  gtk_widget_set_tooltip_text(
      dialog->font_button, _("Set the font used for printing the definitions"));
...
void gdict_show_pref_dialog(GtkWidget *parent, const gchar *title,
                            GdictSourceLoader *loader) {
  GtkWidget *dialog;
  ...
  if (dialog == NULL) {
    dialog = g_object_new(GDICT_TYPE_PREF_DIALOG, "source-loader", loader,
                          "title", title, NULL);
    ...
    if (parent != NULL && GTK_IS_WINDOW(parent)) {
      ...
      g_object_set_data_full(G_OBJECT(parent), "gdict-pref-dialog", dialog,
                             g_object_unref);
    } else

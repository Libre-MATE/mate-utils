...
static SidebarPage *sidebar_page_new(const gchar *id, const gchar *name,
                                     GtkWidget *widget) {
  SidebarPage *page;
...
static void sidebar_page_free(SidebarPage *page) {
  if (G_LIKELY(page)) {
    g_free(page->name);
    g_free(page->id);
...
static void gdict_sidebar_finalize(GObject *object) {
  GdictSidebar *sidebar = GDICT_SIDEBAR(object);
  GdictSidebarPrivate *priv = sidebar->priv;

  if (priv->pages_by_id) g_hash_table_destroy(priv->pages_by_id);
...
static void gdict_sidebar_dispose(GObject *object) {
  GdictSidebar *sidebar = GDICT_SIDEBAR(object);
...
static gboolean gdict_sidebar_select_button_press_cb(GtkWidget *widget,
  ...
  if (event->button == 1) {
    ...
    gtk_widget_set_size_request(sidebar->priv->menu, MAX(width, req.width), -1);
    gtk_widget_grab_focus(widget);

    gtk_toggle_button_set_active(GTK_TOGGLE_BUTTON(widget), TRUE);
    gtk_menu_popup_at_widget(GTK_MENU(sidebar->priv->menu), widget,
...
static gboolean gdict_sidebar_select_key_press_cb(GtkWidget *widget,
                                                  GdkEventKey *event,
                                                  gpointer user_data) {
  GdictSidebar *sidebar = GDICT_SIDEBAR(user_data);
...
static void gdict_sidebar_close_clicked_cb(GtkWidget *widget,
                                           gpointer user_data) {
  GdictSidebar *sidebar = GDICT_SIDEBAR(user_data);
...
static void gdict_sidebar_menu_deactivate_cb(GtkWidget *widget,
                                             gpointer user_data) {
  GdictSidebar *sidebar = GDICT_SIDEBAR(user_data);
  GdictSidebarPrivate *priv = sidebar->priv;
...
static void gdict_sidebar_menu_detach_cb(GtkWidget *widget, GtkMenu *menu) {
  GdictSidebar *sidebar = GDICT_SIDEBAR(widget);

  sidebar->priv->menu = NULL;
}

static void gdict_sidebar_menu_item_activate(GtkWidget *widget,
                                             gpointer user_data) {
  GdictSidebar *sidebar = GDICT_SIDEBAR(user_data);
  GdictSidebarPrivate *priv = sidebar->priv;
  GtkWidget *menu_item;
  ...
  if (current_index == page->index) return;
...
static void gdict_sidebar_class_init(GdictSidebarClass *klass) {
  ...
  sidebar_signals[PAGE_CHANGED] = g_signal_new(
      "page-changed", G_TYPE_FROM_CLASS(gobject_class), G_SIGNAL_RUN_LAST,
      G_STRUCT_OFFSET(GdictSidebarClass, page_changed), NULL, NULL,
      g_cclosure_marshal_VOID__VOID, G_TYPE_NONE, 0);
  sidebar_signals[CLOSED] = g_signal_new(
      "closed", G_TYPE_FROM_CLASS(gobject_class), G_SIGNAL_RUN_LAST,
      G_STRUCT_OFFSET(GdictSidebarClass, closed), NULL, NULL,
      g_cclosure_marshal_VOID__VOID, G_TYPE_NONE, 0);
}
...
static void gdict_sidebar_init(GdictSidebar *sidebar) {
  ...
  g_signal_connect(select_button, "button-press-event",
                   G_CALLBACK(gdict_sidebar_select_button_press_cb), sidebar);
  ...
  gtk_button_set_image(GTK_BUTTON(close_button),
                       gtk_image_new_from_icon_name(
                           "window-close", GTK_ICON_SIZE_SMALL_TOOLBAR));
  g_signal_connect(close_button, "clicked",
                   G_CALLBACK(gdict_sidebar_close_clicked_cb), sidebar);
  ...
  gtk_menu_attach_to_widget(GTK_MENU(sidebar->priv->menu), GTK_WIDGET(sidebar),
...
void gdict_sidebar_add_page(GdictSidebar *sidebar, const gchar *page_id,
  ...
  if (g_hash_table_lookup(priv->pages_by_id, page_id)) {
    g_warning(
        "Attempting to add a page to the sidebar with "
  ...
  page->index =
      gtk_notebook_append_page(GTK_NOTEBOOK(priv->notebook), page_widget, NULL);

  /* add the menu item for the page */
  menu_item = gtk_image_menu_item_new_with_label(page_name);
  g_object_set_qdata_full(G_OBJECT(menu_item), sidebar_page_id_quark,
...
void gdict_sidebar_remove_page(GdictSidebar *sidebar, const gchar *page_id) {
  GdictSidebarPrivate *priv;
  SidebarPage *page;
  GList *children, *l;
  ...
  if ((page = g_hash_table_lookup(priv->pages_by_id, page_id)) == NULL) {
    g_warning(
        "Attempting to remove a page from the sidebar with "
...
void gdict_sidebar_view_page(GdictSidebar *sidebar, const gchar *page_id) {
  GdictSidebarPrivate *priv;
  SidebarPage *page;
...
gchar **gdict_sidebar_list_pages(GdictSidebar *sidebar, gsize *length) {
  GdictSidebarPrivate *priv;
  gchar **retval;
  gint i;
  GSList *l;

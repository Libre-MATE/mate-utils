...
static void gdict_aligned_window_get_property(GObject *object, guint prop_id,
                                              GValue *value, GParamSpec *pspec);
static void gdict_aligned_window_set_property(GObject *object, guint prop_id,
                                              const GValue *value,
                                              GParamSpec *pspec);
...
static gboolean gdict_aligned_window_motion_notify_cb(
    GtkWidget *widget, GdkEventMotion *event,
    GdictAlignedWindow *aligned_window);

G_DEFINE_TYPE_WITH_PRIVATE(GdictAlignedWindow, gdict_aligned_window,
                           GTK_TYPE_WINDOW);

static void gdict_aligned_window_class_init(GdictAlignedWindowClass *klass) {
  ...
  g_object_class_install_property(
      gobject_class, PROP_ALIGN_WIDGET,
      g_param_spec_object("align-widget", "Align Widget",
                          "The widget the window should align to",
                          GTK_TYPE_WIDGET, G_PARAM_READWRITE));
}

static void gdict_aligned_window_init(GdictAlignedWindow *aligned_window) {
  GdictAlignedWindowPrivate *priv =
      gdict_aligned_window_get_instance_private(aligned_window);
...
static void gdict_aligned_window_position(GdictAlignedWindow *window) {
  GdictAlignedWindowPrivate *priv;
  GtkWidget *align_widget;
  ...
  gdk_window_get_geometry(gtk_widget_get_window(GTK_WIDGET(window)), NULL, NULL,
  ...
  if (entry_y + entry_height + our_height <
...
GtkWidget *gdict_aligned_window_new(GtkWidget *align_widget) {
  return g_object_new(GDICT_TYPE_ALIGNED_WINDOW, "align-widget", align_widget,
                      NULL);
}
...
void gdict_aligned_window_set_widget(GdictAlignedWindow *aligned_window,
  ...
  priv->motion_id = g_signal_connect(
      priv->align_widget, "motion-notify-event",
      G_CALLBACK(gdict_aligned_window_motion_notify_cb), aligned_window);
}

...
#define GDICT_APPLET_CLASS(klass) \
  (G_TYPE_CHECK_CLASS_CAST((klass), GDICT_TYPE_APPLET, GdictAppletClass))
#define GDICT_APPLET_GET_CLASS(obj) \
  (G_TYPE_INSTANCE_GET_CLASS((obj), GDICT_TYPE_APPLET, GdictAppletClass))
...
static void set_atk_name_description(GtkWidget *widget, const char *name,
...
static void set_window_default_size(GdictApplet *applet) {
  ...
  font_size =
      pango_font_description_get_size(gtk_widget_get_style(defbox)->font_desc);
  ...
  monitor_num =
      gdk_display_get_monitor_at_window(display, gtk_widget_get_window(widget));
...
static void clear_cb(GtkWidget *widget, GdictApplet *applet) {
  GdictAppletPrivate *priv = applet->priv;

  gtk_entry_set_text(GTK_ENTRY(priv->entry), "");

  if (!priv->defbox) return;

  gdict_defbox_clear(GDICT_DEFBOX(priv->defbox));
}

static void print_cb(GtkWidget *widget, GdictApplet *applet) {
  GdictAppletPrivate *priv = applet->priv;

  if (!priv->defbox) return;

  gdict_show_print_dialog(GTK_WINDOW(priv->window), GDICT_DEFBOX(priv->defbox));
}

static void save_cb(GtkWidget *widget, GdictApplet *applet) {
  GdictAppletPrivate *priv = applet->priv;
  GtkWidget *dialog;
  ...
  dialog = gtk_file_chooser_dialog_new(
      _("Save a Copy"), GTK_WINDOW(priv->window), GTK_FILE_CHOOSER_ACTION_SAVE,
  ...
  if (gtk_dialog_run(GTK_DIALOG(dialog)) == GTK_RESPONSE_ACCEPT) {
    ...
    if (write_error) {
      ...
      gdict_show_error_dialog(GTK_WINDOW(priv->window), message,
                              write_error->message);
...
static void gdict_applet_set_menu_items_sensitive(GdictApplet *applet,
...
static gboolean window_key_press_event_cb(GtkWidget *widget, GdkEventKey *event,
                                          gpointer user_data) {
  GdictApplet *applet = GDICT_APPLET(user_data);

  if (event->keyval == GDK_KEY_Escape) {
    gtk_toggle_button_set_active(GTK_TOGGLE_BUTTON(applet->priv->toggle),
                                 FALSE);
...
static void window_show_cb(GtkWidget *window, GdictApplet *applet) {
  set_window_default_size(applet);
}

static void gdict_applet_build_window(GdictApplet *applet) {
  GdictAppletPrivate *priv = applet->priv;
  GtkWidget *window;
  GtkWidget *frame;
  GtkWidget *vbox;
  ...
  g_signal_connect(window, "key-press-event",
                   G_CALLBACK(window_key_press_event_cb), applet);
  g_signal_connect(window, "show", G_CALLBACK(window_show_cb), applet);
  ...
  set_atk_name_description(button, _("Clear definition"),
  ...
  button =
      GTK_WIDGET(g_object_new(GTK_TYPE_BUTTON, "label", "gtk-print",
                              "use-stock", TRUE, "use-underline", TRUE, NULL));

  gtk_widget_set_tooltip_text(button, _("Print the definitions found"));
  set_atk_name_description(button, _("Print definition"),
  ...
  button =
      GTK_WIDGET(g_object_new(GTK_TYPE_BUTTON, "label", "gtk-save", "use-stock",
                              TRUE, "use-underline", TRUE, NULL));

  gtk_widget_set_tooltip_text(button, _("Save the definitions found"));
  set_atk_name_description(button, _("Save definition"),
...
static gboolean gdict_applet_icon_toggled_cb(GtkWidget *widget,
                                             GdictApplet *applet) {
  GdictAppletPrivate *priv = applet->priv;

  if (!priv->window) gdict_applet_build_window(applet);
...
static void gdict_applet_entry_activate_cb(GtkWidget *widget,
                                           GdictApplet *applet) {
  GdictAppletPrivate *priv = applet->priv;
  gchar *text;
...
static gboolean gdict_applet_entry_key_press_cb(GtkWidget *widget,
                                                GdkEventKey *event,
                                                gpointer user_data) {
  ...
  if (event->keyval == GDK_KEY_Escape) {
    ...
    if (priv->is_window_showing) gtk_widget_grab_focus(priv->defbox);
  }

  return FALSE;
}

static gboolean gdict_applet_icon_button_press_event_cb(GtkWidget *widget,
...
static gboolean gdict_applet_entry_button_press_event_cb(GtkWidget *widget,
                                                         GdkEventButton *event,
                                                         GdictApplet *applet) {
...
static gboolean gdict_applet_draw(GdictApplet *applet) {
  GdictAppletPrivate *priv = applet->priv;
  GtkWidget *box;
  GtkWidget *hbox;
  gchar *text = NULL;
  ...
  if (priv->box) gtk_widget_destroy(priv->box);
  ...
  gtk_widget_set_tooltip_text(priv->toggle,
                              _("Click to view the dictionary window"));
  set_atk_name_description(priv->toggle, _("Toggle dictionary window"),
                           _("Show or hide the definition window"));

  gtk_button_set_relief(GTK_BUTTON(priv->toggle), GTK_RELIEF_NONE);
  g_signal_connect(priv->toggle, "toggled",
                   G_CALLBACK(gdict_applet_icon_toggled_cb), applet);
  g_signal_connect(priv->toggle, "button-press-event",
                   G_CALLBACK(gdict_applet_icon_button_press_event_cb), applet);
  ...
  if (priv->icon) {
    ...
    scaled = gdk_pixbuf_scale_simple(priv->icon, priv->size - 5, priv->size - 5,
                                     GDK_INTERP_BILINEAR);
    ...
    gtk_image_set_pixel_size(GTK_IMAGE(priv->image), priv->size - 10);
    gtk_image_set_from_icon_name(GTK_IMAGE(priv->image), "image-missing", -1);
  ...
  gtk_widget_set_tooltip_text(priv->entry,
                              _("Type the word you want to look up"));
  set_atk_name_description(priv->entry, _("Dictionary entry"),
...
static void gdict_applet_queue_draw(GdictApplet *applet) {
  if (!applet->priv->idle_draw_id)
    applet->priv->idle_draw_id =
        g_idle_add((GSourceFunc)gdict_applet_draw, applet);
}

static void gdict_applet_lookup_start_cb(GdictContext *context,
                                         GdictApplet *applet) {
  GdictAppletPrivate *priv = applet->priv;
...
static void gdict_applet_lookup_end_cb(GdictContext *context,
                                       GdictApplet *applet) {
...
static void gdict_applet_error_cb(GdictContext *context, const GError *error,
                                  GdictApplet *applet) {
  /* disable menu items */
  gdict_applet_set_menu_items_sensitive(applet, FALSE);
}

static void gdict_applet_cmd_lookup(GtkAction *action, GdictApplet *applet) {
  GdictAppletPrivate *priv = applet->priv;
  gchar *text = NULL;
  ;
...
static void gdict_applet_cmd_clear(GtkAction *action, GdictApplet *applet) {
  clear_cb(NULL, applet);
}

static void gdict_applet_cmd_print(GtkAction *action, GdictApplet *applet) {
  print_cb(NULL, applet);
}

static void gdict_applet_cmd_save(GtkAction *action, GdictApplet *applet) {
  save_cb(NULL, applet);
}

static void gdict_applet_cmd_preferences(GtkAction *action,
                                         GdictApplet *applet) {
  gdict_show_pref_dialog(GTK_WIDGET(applet), _("Dictionary Preferences"),
                         applet->priv->loader);
}

static void gdict_applet_cmd_about(GtkAction *action, GdictApplet *applet) {
  gdict_show_about_dialog(GTK_WIDGET(applet));
}

static void gdict_applet_cmd_help(GtkAction *action, GdictApplet *applet) {
  GError *err = NULL;

  gtk_show_uri_on_window(NULL, "help:mate-dictionary/mate-dictionary-applet",
                         gtk_get_current_event_time(), &err);

  if (err) {
    gdict_show_error_dialog(NULL, _("There was an error while displaying help"),
                            err->message);
    g_error_free(err);
  }
}

static void gdict_applet_change_orient(MatePanelApplet *applet,
  ...
  if (new_size != priv->size) priv->size = new_size;

  gdict_applet_queue_draw(GDICT_APPLET(applet));

  if (MATE_PANEL_APPLET_CLASS(gdict_applet_parent_class)->change_orient)
    MATE_PANEL_APPLET_CLASS(gdict_applet_parent_class)
        ->change_orient(applet, orient);
}

static void gdict_applet_size_allocate(GtkWidget *widget,
  ...
  if (priv->size != new_size) {
    ...
    if (priv->icon) {
      ...
      scaled = gdk_pixbuf_scale_simple(priv->icon, priv->size - 5,
                                       priv->size - 5, GDK_INTERP_BILINEAR);
  ...
  if (GTK_WIDGET_CLASS(gdict_applet_parent_class)->size_allocate)
    GTK_WIDGET_CLASS(gdict_applet_parent_class)
...
static void gdict_applet_set_database(GdictApplet *applet,
  ...
  if (database != NULL && *database != '\0')
    ...
    priv->database =
        g_settings_get_string(priv->settings, GDICT_SETTINGS_DATABASE_KEY);
  if (priv->defbox)
    gdict_defbox_set_database(GDICT_DEFBOX(priv->defbox), priv->database);
}

static void gdict_applet_set_strategy(GdictApplet *applet,
  ...
  if (strategy != NULL && *strategy != '\0')
    ...
    priv->strategy =
        g_settings_get_string(priv->settings, GDICT_SETTINGS_STRATEGY_KEY);
}

static GdictContext *get_context_from_loader(GdictApplet *applet) {
  GdictAppletPrivate *priv = applet->priv;
  GdictSource *source;
  GdictContext *retval;
  ...
  if (!source) {
    ...
    gdict_show_error_dialog(NULL, _("Unable to find dictionary source"),
                            detail);
  ...
  if (!retval) {
    ...
    gdict_show_error_dialog(NULL, _("Unable to create a context"), detail);
...
static void gdict_applet_set_print_font(GdictApplet *applet,
  ...
  if (print_font != NULL && *print_font != '\0')
    ...
    priv->print_font =
        g_settings_get_string(priv->settings, GDICT_SETTINGS_PRINT_FONT_KEY);
}

static void gdict_applet_set_defbox_font(GdictApplet *applet,
  ...
  if (defbox_font != NULL && *defbox_font != '\0')
    ...
    priv->defbox_font =
        g_settings_get_string(priv->desktop_settings, DOCUMENT_FONT_KEY);

  if (priv->defbox)
    gdict_defbox_set_font_name(GDICT_DEFBOX(priv->defbox), priv->defbox_font);
}

static void gdict_applet_set_context(GdictApplet *applet,
                                     GdictContext *context) {
  GdictAppletPrivate *priv = applet->priv;
  ...
  priv->lookup_start_id =
      g_signal_connect(context, "lookup-start",
                       G_CALLBACK(gdict_applet_lookup_start_cb), applet);
  priv->lookup_end_id = g_signal_connect(
      context, "lookup-end", G_CALLBACK(gdict_applet_lookup_end_cb), applet);
  priv->error_id = g_signal_connect(context, "error",
                                    G_CALLBACK(gdict_applet_error_cb), applet);

  priv->context = context;
}

static void gdict_applet_set_source_name(GdictApplet *applet,
  ...
  if (source_name != NULL && *source_name != '\0')
    ...
    priv->source_name =
        g_settings_get_string(priv->settings, GDICT_SETTINGS_SOURCE_KEY);

  context = get_context_from_loader(applet);
  gdict_applet_set_context(applet, context);
}

static void gdict_applet_settings_changed_cb(GSettings *settings,
                                             const gchar *key,
                                             GdictApplet *applet) {
...
static void gdict_applet_finalize(GObject *object) {
  GdictApplet *applet = GDICT_APPLET(object);
  GdictAppletPrivate *priv = applet->priv;

  if (priv->idle_draw_id) g_source_remove(priv->idle_draw_id);
...
static void gdict_applet_class_init(GdictAppletClass *klass) {
...
static void gdict_applet_init(GdictApplet *applet) {
  ...
  if (!priv->loader) priv->loader = gdict_source_loader_new();
  ...
  priv->icon = gtk_icon_theme_load_icon(gtk_icon_theme_get_default(),
                                        "accessories-dictionary", 48, 0, NULL);
...
static const GtkActionEntry gdict_applet_menu_actions[] = {
    {"DictionaryLookup", "edit-find", N_("_Look Up Selected Text"), NULL, NULL,
     G_CALLBACK(gdict_applet_cmd_lookup)},
    {"DictionaryClear", "edit-clear", N_("Cl_ear"), NULL, NULL,
     G_CALLBACK(gdict_applet_cmd_clear)},
    {"DictionaryPrint", "document-print", N_("_Print"), NULL, NULL,
     G_CALLBACK(gdict_applet_cmd_print)},
    {"DictionarySave", "document-save", N_("_Save"), NULL, NULL,
     G_CALLBACK(gdict_applet_cmd_save)},
    {"DictionaryPreferences", "document-properties", N_("Preferences"), NULL,
     NULL, G_CALLBACK(gdict_applet_cmd_preferences)},
    {"DictionaryHelp", "help-browser", N_("_Help"), NULL, NULL,
     G_CALLBACK(gdict_applet_cmd_help)},
    {"DictionaryAbout", "help-about", N_("_About"), NULL, NULL,
     G_CALLBACK(gdict_applet_cmd_about)},
};

static gboolean gdict_applet_factory(MatePanelApplet *applet, const gchar *iid,
                                     gpointer data) {
  ...
  if (((!strcmp(iid, "DictionaryApplet"))) && gdict_create_data_dir()) {
    ...
    priv->context_menu_action_group =
        gtk_action_group_new("Dictionary Applet Actions");
#ifdef ENABLE_NLS
    gtk_action_group_set_translation_domain(priv->context_menu_action_group,
                                            GETTEXT_PACKAGE);
#endif /* ENABLE_NLS */
    gtk_action_group_add_actions(
        priv->context_menu_action_group, gdict_applet_menu_actions,
        G_N_ELEMENTS(gdict_applet_menu_actions), applet);
    mate_panel_applet_setup_menu_from_file(
        applet, PKGDATADIR "/dictionary-applet-menu.xml",
        priv->context_menu_action_group);

...
static void gdict_app_finalize(GObject *object) {
  GdictApp *app = GDICT_APP(object);

  if (app->loader) g_object_unref(app->loader);
...
static void gdict_app_class_init(GdictAppClass *klass) {
  GObjectClass *gobject_class = G_OBJECT_CLASS(klass);

  gobject_class->finalize = gdict_app_finalize;
}

static void gdict_app_init(GdictApp *app) {
  app->windows = NULL;
  app->current_window = NULL;
}

static void gdict_window_destroy_cb(GtkWidget *widget, gpointer user_data) {
  ...
  if (app->windows == NULL) gtk_main_quit();
}

static void gdict_window_created_cb(GdictWindow *parent,
   ...
   */
  g_signal_connect(new_window, "created", G_CALLBACK(gdict_window_created_cb),
                   app);
  g_signal_connect(new_window, "destroy", G_CALLBACK(gdict_window_destroy_cb),
                   app);
...
static void gdict_create_window(GdictApp *app) {
  GSList *l;

  if (!singleton->lookup_words && !singleton->match_words) {
    GtkWidget *window;

    window = gdict_window_new(GDICT_WINDOW_ACTION_CLEAR, singleton->loader,
                              singleton->source_name, NULL);
    g_signal_connect(window, "created", G_CALLBACK(gdict_window_created_cb),
                     app);
    g_signal_connect(window, "destroy", G_CALLBACK(gdict_window_destroy_cb),
                     app);
  ...
  for (l = singleton->lookup_words; l != NULL; l = l->next) {
    ...
    window = gdict_window_new(GDICT_WINDOW_ACTION_LOOKUP, singleton->loader,
                              singleton->source_name, word);
  ...
  for (l = singleton->match_words; l != NULL; l = l->next) {
    ...
    window = gdict_window_new(GDICT_WINDOW_ACTION_MATCH, singleton->loader,
                              singleton->source_name, word);
...
static void lookup_end_cb(GdictContext *context, gpointer user_data) {
  ...
  if (app->remaining_words == 0) gtk_main_quit();
}

static void gdict_look_up_word_and_quit(GdictApp *app) {
  GdictSource *source;
  ...
  if (app->source_name)
    ...
    source =
        gdict_source_loader_get_source(app->loader, GDICT_DEFAULT_SOURCE_NAME);
  ...
  g_signal_connect(context, "definition-found", G_CALLBACK(definition_found_cb),
                   app);
  g_signal_connect(context, "error", G_CALLBACK(error_cb), app);
  g_signal_connect(context, "lookup-end", G_CALLBACK(lookup_end_cb), app);
  ...
  for (l = app->lookup_words; l != NULL; l = l->next) {
    ...
    gdict_context_define_word(context, app->database, word, &err);
...
void gdict_init(int *argc, char ***argv) {
...
#endif /* ENABLE_NLS */
  g_option_context_add_main_entries(context, gdict_app_goptions,
                                    GETTEXT_PACKAGE);
  ...
  if (lookup_words) {
    ...
    for (i = 0; i < length; i++)
      singleton->lookup_words =
          g_slist_prepend(singleton->lookup_words, g_strdup(lookup_words[i]));
  }
  ...
  if (match_words) {
    ...
    for (i = 0; i < length; i++)
      singleton->match_words =
          g_slist_prepend(singleton->match_words, g_strdup(match_words[i]));
  }

  if (database) singleton->database = g_strdup(database);

  if (source_name) singleton->source_name = g_strdup(source_name);

  if (no_window) singleton->no_window = TRUE;
}

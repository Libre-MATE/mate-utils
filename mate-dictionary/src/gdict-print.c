...
static void begin_print(GtkPrintOperation *operation, GtkPrintContext *context,
  ...
  height =
      gtk_print_context_get_height(context) - HEADER_HEIGHT(10) - HEADER_GAP(3);
...
static void draw_page(GtkPrintOperation *operation, GtkPrintContext *context,
                      gint page_number, gpointer user_data) {
  GdictPrintData *data = user_data;
  cairo_t *cr;
  PangoLayout *layout;
...
static gchar *get_print_font(void) {
  static GSettings *settings = NULL;

  if (settings == NULL) settings = g_settings_new("org.mate.dictionary");

  return g_settings_get_string(settings, "print-font");
}

void gdict_show_print_preview(GtkWindow *parent, GdictDefbox *defbox) {
  ...
  data->font_size =
      pango_font_description_get_size(data->font_desc) / PANGO_SCALE;
  g_free(print_font);
  ...
  error = NULL;
  gtk_print_operation_run(operation, GTK_PRINT_OPERATION_ACTION_PREVIEW, parent,
                          &error);
...
void gdict_show_print_dialog(GtkWindow *parent, GdictDefbox *defbox) {
  GtkPrintOperation *operation;
  GdictPrintData *data;
  gchar *print_font;
  gchar *word;

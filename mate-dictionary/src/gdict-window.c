...
static GParamSpec *gdict_window_properties[LAST_PROP] = {
    NULL,
};
...
static void gdict_window_finalize(GObject *gobject) {
...
static void gdict_window_dispose(GObject *gobject) {
  GdictWindow *window = GDICT_WINDOW(gobject);
...
static void gdict_window_ensure_menu_state(GdictWindow *window) {
  gint i;
  gboolean is_sensitive;

  g_assert(GDICT_IS_WINDOW(window));

  if (!window->ui_manager) return;
...
static void gdict_window_set_sidebar_visible(GdictWindow *window,
                                             gboolean is_visible) {
  g_assert(GDICT_IS_WINDOW(window));
...
static void gdict_window_definition_cb(GdictContext *context,
...
static void gdict_window_lookup_start_cb(GdictContext *context,
                                         GdictWindow *window) {
  GdkDisplay *display;
  gchar *message;
  ...
  if (window->progress) gtk_widget_show(window->progress);
...
static void gdict_window_lookup_end_cb(GdictContext *context,
                                       GdictWindow *window) {
  gchar *message;
  gint count;
  GtkTreeIter iter;
  ...
  if (count == 0)
    ...
    message = g_strdup_printf(
        ngettext("A definition found", "%d definitions found", count), count);

  if (window->status && window->statusbar_visible)
    gtk_statusbar_push(GTK_STATUSBAR(window->status), 0, message);

  if (window->progress) gtk_widget_hide(window->progress);
  ...
  if (count != 0 && window->word) {
    ...
    gtk_list_store_set(window->completion_model, &iter, COMPLETION_TEXT_COLUMN,
                       window->word, -1);
  }
...
static void gdict_window_error_cb(GdictContext *context, const GError *error,
                                  GdictWindow *window) {
  gdk_window_set_cursor(gtk_widget_get_window(GTK_WIDGET(window)), NULL);
  ...
  if (error->code == GDICT_CONTEXT_ERROR_NO_MATCH) {
     ...
     */
    source =
        gdict_source_loader_get_source(window->loader, window->source_name);
...
static void gdict_window_set_database(GdictWindow *window,
                                      const gchar *database) {
  if (g_strcmp0(window->database, database) == 0) return;
  ...
  if (database != NULL && *database != '\0')
    ...
    window->database =
        g_settings_get_string(window->settings, GDICT_SETTINGS_DATABASE_KEY);

  if (window->defbox)
    gdict_defbox_set_database(GDICT_DEFBOX(window->defbox), window->database);

  if (window->db_chooser)
    gdict_database_chooser_set_current_database(
        GDICT_DATABASE_CHOOSER(window->db_chooser), window->database);

  g_object_notify_by_pspec(G_OBJECT(window),
                           gdict_window_properties[PROP_DATABASE]);
}

static void gdict_window_set_strategy(GdictWindow *window,
                                      const gchar *strategy) {
  if (g_strcmp0(window->strategy, strategy) == 0) return;
  ...
  if (strategy != NULL && *strategy != '\0')
    ...
    window->strategy =
        g_settings_get_string(window->settings, GDICT_SETTINGS_STRATEGY_KEY);
  ...
  if (window->strat_chooser)
    gdict_strategy_chooser_set_current_strategy(
        GDICT_STRATEGY_CHOOSER(window->strat_chooser), window->strategy);

  g_object_notify_by_pspec(G_OBJECT(window),
                           gdict_window_properties[PROP_STRATEGY]);
}

static GdictContext *get_context_from_loader(GdictWindow *window) {
  GdictSource *source;
  GdictContext *retval;
  ...
  source = gdict_source_loader_get_source(window->loader, window->source_name);
  if (!source && strcmp(window->source_name, GDICT_DEFAULT_SOURCE_NAME) != 0) {
    g_free(window->source_name);
    window->source_name = g_strdup(GDICT_DEFAULT_SOURCE_NAME);
  ...
  if (!retval) {
    ...
    gdict_show_error_dialog(GTK_WINDOW(window), _("Unable to create a context"),
                            detail);
...
static void gdict_window_set_defbox_font(GdictWindow *window,
  ...
  if (defbox_font != NULL && *defbox_font != '\0')
    ...
    window->defbox_font =
        g_settings_get_string(window->desktop_settings, DOCUMENT_FONT_KEY);

  gdict_defbox_set_font_name(GDICT_DEFBOX(window->defbox), window->defbox_font);
}

static void gdict_window_set_print_font(GdictWindow *window,
  ...
  if (print_font != NULL && *print_font != '\0')
    ...
    window->print_font =
        g_settings_get_string(window->settings, GDICT_SETTINGS_PRINT_FONT_KEY);
}

static void gdict_window_set_word(GdictWindow *window, const gchar *word,
                                  const gchar *database) {
  gchar *title;
  ...
  if (!database || database[0] == '\0') database = window->database;
...
static void gdict_window_set_context(GdictWindow *window,
  ...
  if (window->db_chooser)
    gdict_database_chooser_set_context(
        GDICT_DATABASE_CHOOSER(window->db_chooser), context);

  if (window->strat_chooser)
    gdict_strategy_chooser_set_context(
        GDICT_STRATEGY_CHOOSER(window->strat_chooser), context);
  ...
  window->definition_id =
      g_signal_connect(context, "definition-found",
                       G_CALLBACK(gdict_window_definition_cb), window);
  window->lookup_start_id =
      g_signal_connect(context, "lookup-start",
                       G_CALLBACK(gdict_window_lookup_start_cb), window);
  window->lookup_end_id = g_signal_connect(
      context, "lookup-end", G_CALLBACK(gdict_window_lookup_end_cb), window);
  window->error_id = g_signal_connect(
      context, "error", G_CALLBACK(gdict_window_error_cb), window);

  window->context = context;
}

static void gdict_window_set_source_name(GdictWindow *window,
  ...
  if (source_name != NULL && *source_name != '\0')
    ...
    window->source_name =
        g_settings_get_string(window->settings, GDICT_SETTINGS_SOURCE_KEY);
  ...
  if (window->source_chooser)
    gdict_source_chooser_set_current_source(
        GDICT_SOURCE_CHOOSER(window->source_chooser), window->source_name);

  g_object_notify_by_pspec(G_OBJECT(window),
                           gdict_window_properties[PROP_SOURCE_NAME]);
}

static void gdict_window_set_property(GObject *object, guint prop_id,
  ...
  switch (prop_id) {
    ...
    case PROP_SOURCE_LOADER:
      if (window->loader) g_object_unref(window->loader);
...
static void gdict_window_get_property(GObject *object, guint prop_id,
...
static void gdict_window_store_state(GdictWindow *window) {
  ...
  state_dir = g_build_filename(g_get_user_config_dir(), "mate",
                               "mate-dictionary", NULL);
   ...
   */
  g_key_file_set_integer(state_key, "WindowState", "Width",
                         window->current_width);
  g_key_file_set_integer(state_key, "WindowState", "Height",
                         window->current_height);
  g_key_file_set_boolean(state_key, "WindowState", "IsMaximized",
                         window->is_maximized);
  g_key_file_set_boolean(state_key, "WindowState", "SidebarVisible",
                         window->sidebar_visible);
  g_key_file_set_boolean(state_key, "WindowState", "StatusbarVisible",
                         window->statusbar_visible);
  g_key_file_set_integer(state_key, "WindowState", "SidebarWidth",
                         window->sidebar_width);

  page_id = gdict_sidebar_current_page(GDICT_SIDEBAR(window->sidebar));
  if (page_id == NULL) page_id = GDICT_SIDEBAR_SPELLER_PAGE;
...
static void gdict_window_load_state(GdictWindow *window) {
  gchar *state_file;
  GKeyFile *state_key;
  GError *error;

  state_file = g_build_filename(g_get_user_config_dir(), "mate",
                                "mate-dictionary", "window.ini", NULL);
  state_key = g_key_file_new();
  ...
  window->default_width =
      g_key_file_get_integer(state_key, "WindowState", "Width", &error);
  if (error != NULL) {
    g_clear_error(&error);
  ...
  window->default_height =
      g_key_file_get_integer(state_key, "WindowState", "Height", &error);
  if (error != NULL) {
    g_clear_error(&error);
  ...
  window->is_maximized =
      (g_key_file_get_boolean(state_key, "WindowState", "IsMaximized",
                              &error) != FALSE);
  if (error != NULL) {
    g_clear_error(&error);
  ...
  window->sidebar_visible =
      (g_key_file_get_boolean(state_key, "WindowState", "SidebarVisible",
                              &error) != FALSE);
  if (error != NULL) {
    g_clear_error(&error);
  ...
  window->statusbar_visible =
      (g_key_file_get_boolean(state_key, "WindowState", "StatusbarVisible",
                              &error) != FALSE);
  if (error != NULL) {
    g_clear_error(&error);
  ...
  window->sidebar_width =
      g_key_file_get_integer(state_key, "WindowState", "SidebarWidth", &error);
  if (error != NULL) {
    g_clear_error(&error);
  ...
  window->sidebar_page =
      g_key_file_get_string(state_key, "WindowState", "SidebarPage", &error);
  if (error != NULL) {
    g_clear_error(&error);
...
static void gdict_window_cmd_file_new(GtkAction *action, GdictWindow *window) {
  ...
  if (word) {
    new_window = gdict_window_new(GDICT_WINDOW_ACTION_LOOKUP, window->loader,
                                  NULL, word);
    g_free(word);
  } else
    new_window =
        gdict_window_new(GDICT_WINDOW_ACTION_CLEAR, window->loader, NULL, NULL);
...
static void gdict_window_cmd_save_as(GtkAction *action, GdictWindow *window) {
  GtkWidget *dialog;

  g_assert(GDICT_IS_WINDOW(window));

  dialog = gtk_file_chooser_dialog_new(
      _("Save a Copy"), GTK_WINDOW(window), GTK_FILE_CHOOSER_ACTION_SAVE,
  ...
  if (gtk_dialog_run(GTK_DIALOG(dialog)) == GTK_RESPONSE_ACCEPT) {
    ...
    if (write_error) {
      ...
      gdict_show_gerror_dialog(GTK_WINDOW(window), message, write_error);
...
static void gdict_window_cmd_file_preview(GtkAction *action,
                                          GdictWindow *window) {
  g_assert(GDICT_IS_WINDOW(window));

  gdict_show_print_preview(GTK_WINDOW(window), GDICT_DEFBOX(window->defbox));
}

static void gdict_window_cmd_file_print(GtkAction *action,
                                        GdictWindow *window) {
  g_assert(GDICT_IS_WINDOW(window));

  gdict_show_print_dialog(GTK_WINDOW(window), GDICT_DEFBOX(window->defbox));
}

static void gdict_window_cmd_file_close_window(GtkAction *action,
                                               GdictWindow *window) {
  g_assert(GDICT_IS_WINDOW(window));
   ...
   * will destroy itself.
   */
  if (action) gtk_widget_destroy(GTK_WIDGET(window));
}

static void gdict_window_cmd_edit_copy(GtkAction *action, GdictWindow *window) {
  g_assert(GDICT_IS_WINDOW(window));
...
static void gdict_window_cmd_edit_select_all(GtkAction *action,
                                             GdictWindow *window) {
  g_assert(GDICT_IS_WINDOW(window));
...
static void gdict_window_cmd_edit_find(GtkAction *action, GdictWindow *window) {
  g_assert(GDICT_IS_WINDOW(window));

  gdict_defbox_set_show_find(GDICT_DEFBOX(window->defbox), TRUE);
}

static void gdict_window_cmd_edit_find_next(GtkAction *action,
                                            GdictWindow *window) {
  g_assert(GDICT_IS_WINDOW(window));
...
static void gdict_window_cmd_edit_find_previous(GtkAction *action,
                                                GdictWindow *window) {
  g_assert(GDICT_IS_WINDOW(window));
...
static void gdict_window_cmd_edit_preferences(GtkAction *action,
                                              GdictWindow *window) {
  g_assert(GDICT_IS_WINDOW(window));

  gdict_show_pref_dialog(GTK_WIDGET(window), _("Dictionary Preferences"),
                         window->loader);
}

static void gdict_window_cmd_view_sidebar(GtkToggleAction *action,
                                          GdictWindow *window) {
  g_assert(GDICT_IS_WINDOW(window));
...
static void gdict_window_cmd_view_statusbar(GtkToggleAction *action,
                                            GdictWindow *window) {
  g_assert(GDICT_IS_WINDOW(window));
...
static void gdict_window_cmd_view_speller(GtkAction *action,
                                          GdictWindow *window) {
  g_assert(GDICT_IS_WINDOW(window));
...
static void gdict_window_cmd_view_databases(GtkAction *action,
                                            GdictWindow *window) {
  g_assert(GDICT_IS_WINDOW(window));
...
static void gdict_window_cmd_view_strategies(GtkAction *action,
                                             GdictWindow *window) {
  g_assert(GDICT_IS_WINDOW(window));
...
static void gdict_window_cmd_view_sources(GtkAction *action,
                                          GdictWindow *window) {
  g_assert(GDICT_IS_WINDOW(window));
...
static void gdict_window_cmd_go_first_def(GtkAction *action,
                                          GdictWindow *window) {
  g_assert(GDICT_IS_WINDOW(window));
...
static void gdict_window_cmd_go_previous_def(GtkAction *action,
                                             GdictWindow *window) {
  g_assert(GDICT_IS_WINDOW(window));

  if (window->last_definition == 0) return;
...
static void gdict_window_cmd_go_next_def(GtkAction *action,
                                         GdictWindow *window) {
  g_assert(GDICT_IS_WINDOW(window));

  if (window->max_definition == -1)
    window->max_definition =
        gdict_defbox_count_definitions(GDICT_DEFBOX(window->defbox)) - 1;

  if (window->last_definition == window->max_definition) return;
...
static void gdict_window_cmd_go_last_def(GtkAction *action,
                                         GdictWindow *window) {
  g_assert(GDICT_IS_WINDOW(window));
...
static void gdict_window_cmd_help_contents(GtkAction *action,
                                           GdictWindow *window) {
  GError *err = NULL;

  g_return_if_fail(GDICT_IS_WINDOW(window));

  gtk_show_uri_on_window(GTK_WINDOW(window), "help:mate-dictionary",
                         gtk_get_current_event_time(), &err);
  if (err) {
    gdict_show_gerror_dialog(
        GTK_WINDOW(window), _("There was an error while displaying help"), err);
  }
}

static void gdict_window_cmd_help_about(GtkAction *action,
                                        GdictWindow *window) {
  g_assert(GDICT_IS_WINDOW(window));
...
static void gdict_window_cmd_lookup(GtkAction *action, GdictWindow *window) {
  g_assert(GDICT_IS_WINDOW(window));

  gtk_widget_grab_focus(window->entry);
}

static void gdict_window_cmd_escape(GtkAction *action, GdictWindow *window) {
  g_assert(GDICT_IS_WINDOW(window));
...
static const GtkActionEntry entries[] = {
    ...
    /* File menu */
    {"FileNew", "document-new", N_("_New"), "<control>N", N_("New look up"),
     G_CALLBACK(gdict_window_cmd_file_new)},
    ...
    /* Go menu */
    {"GoPreviousDef", "go-previous", N_("_Previous Definition"),
     "<control>Page_Up", N_("Go to the previous definition"),
     G_CALLBACK(gdict_window_cmd_go_previous_def)},
    ...
    {"GoFirstDef", "go-first", N_("_First Definition"), "<control>Home",
     N_("Go to the first definition"),
     G_CALLBACK(gdict_window_cmd_go_first_def)},
    ...
    /* Accelerators */
    {"Lookup", NULL, "", "<control>L", NULL,
     G_CALLBACK(gdict_window_cmd_lookup)},
...
static gboolean gdict_window_delete_event_cb(GtkWidget *widget, GdkEvent *event,
                                             gpointer user_data) {
...
static gboolean gdict_window_state_event_cb(GtkWidget *widget,
...
static void lookup_word(GdictWindow *window, gpointer dummy) {
  ...
  if (!entry_text || *entry_text == '\0') return;
...
static void source_activated_cb(GdictSourceChooser *chooser,
                                const gchar *source_name, GdictSource *source,
                                GdictWindow *window) {
...
static void strategy_activated_cb(GdictStrategyChooser *chooser,
...
static void database_activated_cb(GdictDatabaseChooser *chooser,
                                  const gchar *db_name, const gchar *db_desc,
                                  GdictWindow *window) {
...
static void speller_word_activated_cb(GdictSpeller *speller, const gchar *word,
                                      const gchar *db_name,
                                      GdictWindow *window) {
...
static void sidebar_page_changed_cb(GdictSidebar *sidebar,
                                    GdictWindow *window) {
  const gchar *page_id;
  const gchar *message;
  ...
  switch (page_id[0]) {
    ...
    case 'd': /* db-chooser */
      ...
      gdict_database_chooser_refresh(
          GDICT_DATABASE_CHOOSER(window->db_chooser));
      break;
    default:
      message = NULL;
      break;
...
static void sidebar_closed_cb(GdictSidebar *sidebar, GdictWindow *window) {
  gdict_window_set_sidebar_visible(window, FALSE);
}

static void gdict_window_link_clicked(GdictDefbox *defbox,
...
static void gdict_window_drag_data_received_cb(GtkWidget *widget,
...
static void gdict_window_size_allocate(GtkWidget *widget,
  ...
  if (GTK_WIDGET_CLASS(gdict_window_parent_class)->size_allocate)
    GTK_WIDGET_CLASS(gdict_window_parent_class)
        ->size_allocate(widget, allocation);
}

static void gdict_window_handle_notify_position_cb(GtkWidget *widget,
                                                   GParamSpec *pspec,
                                                   gpointer user_data) {
  GdictWindow *window = GDICT_WINDOW(user_data);
...
static GObject *gdict_window_constructor(
  ...
  object = G_OBJECT_CLASS(gdict_window_parent_class)
  ...
  if (!gtk_ui_manager_add_ui_from_file(
          window->ui_manager, PKGDATADIR "/mate-dictionary-ui.xml", &error)) {
    g_warning("Building menus failed: %s", error->message);
    g_error_free(error);
  } else {
    window->menubar =
        gtk_ui_manager_get_widget(window->ui_manager, "/MainMenu");

    gtk_box_pack_start(GTK_BOX(window->main_box), window->menubar, FALSE, FALSE,
                       0);
  ...
  g_signal_connect_swapped(button, "clicked", G_CALLBACK(lookup_word), window);
  gtk_button_set_relief(GTK_BUTTON(button), GTK_RELIEF_NONE);
  gtk_box_pack_start(GTK_BOX(hbox), button, FALSE, FALSE, 0);
  gtk_widget_show(button);

  window->completion_model =
      gtk_list_store_new(COMPLETION_N_COLUMNS, G_TYPE_STRING);
  ...
  if (window->word) gtk_entry_set_text(GTK_ENTRY(window->entry), window->word);

  gtk_entry_set_completion(GTK_ENTRY(window->entry), window->completion);
  g_signal_connect_swapped(window->entry, "activate", G_CALLBACK(lookup_word),
                           window);
  ...
  g_signal_connect(window->defbox, "link-clicked",
                   G_CALLBACK(gdict_window_link_clicked), window);

  gtk_drag_dest_set(window->defbox, GTK_DEST_DEFAULT_ALL, drop_types,
                    n_drop_types, GDK_ACTION_COPY);
  g_signal_connect(window->defbox, "drag-data-received",
                   G_CALLBACK(gdict_window_drag_data_received_cb), window);
  ...
  g_signal_connect(window->sidebar, "page-changed",
                   G_CALLBACK(sidebar_page_changed_cb), window);
  g_signal_connect(window->sidebar, "closed", G_CALLBACK(sidebar_closed_cb),
                   window);
  ...
  g_signal_connect(window->speller, "word-activated",
                   G_CALLBACK(speller_word_activated_cb), window);
  gdict_sidebar_add_page(GDICT_SIDEBAR(window->sidebar),
                         GDICT_SIDEBAR_SPELLER_PAGE, _("Similar words"),
                         window->speller);
  ...
  if (window->context)
    gdict_database_chooser_set_context(
        GDICT_DATABASE_CHOOSER(window->db_chooser), window->context);
  g_signal_connect(window->db_chooser, "database-activated",
                   G_CALLBACK(database_activated_cb), window);
  gdict_sidebar_add_page(GDICT_SIDEBAR(window->sidebar),
                         GDICT_SIDEBAR_DATABASES_PAGE,
                         _("Available dictionaries"), window->db_chooser);
  gtk_widget_show(window->db_chooser);

  /* bind the database property to the database setting */
  g_settings_bind(window->settings, GDICT_SETTINGS_DATABASE_KEY, window,
                  "database", G_SETTINGS_BIND_DEFAULT);

  /* Strategy chooser */
  if (window->context)
    gdict_strategy_chooser_set_context(
        GDICT_STRATEGY_CHOOSER(window->strat_chooser), window->context);
  g_signal_connect(window->strat_chooser, "strategy-activated",
                   G_CALLBACK(strategy_activated_cb), window);
  gdict_sidebar_add_page(GDICT_SIDEBAR(window->sidebar),
                         GDICT_SIDEBAR_STRATEGIES_PAGE,
                         _("Available strategies"), window->strat_chooser);
  gtk_widget_show(window->strat_chooser);

  /* bind the strategy property to the strategy setting */
  g_settings_bind(window->settings, GDICT_SETTINGS_STRATEGY_KEY, window,
                  "strategy", G_SETTINGS_BIND_DEFAULT);
  ...
  g_signal_connect(window->source_chooser, "source-activated",
                   G_CALLBACK(source_activated_cb), window);
  gdict_sidebar_add_page(GDICT_SIDEBAR(window->sidebar),
  ...
  g_settings_bind(window->settings, GDICT_SETTINGS_SOURCE_KEY, window,
                  "source-name", G_SETTINGS_BIND_DEFAULT);
  ...
  g_settings_bind(window->desktop_settings, DOCUMENT_FONT_KEY, window,
                  "defbox-font", G_SETTINGS_BIND_GET);
  ...
  gtk_window_set_default_size(GTK_WINDOW(window), window->default_width,
                              window->default_height);
  if (window->is_maximized) gtk_window_maximize(GTK_WINDOW(window));

  gtk_widget_get_allocation(GTK_WIDGET(window), &allocation);
  gtk_paned_set_position(GTK_PANED(handle),
                         allocation.width - window->sidebar_width);
  ...
  g_signal_connect(window, "delete-event",
                   G_CALLBACK(gdict_window_delete_event_cb), NULL);
  g_signal_connect(window, "window-state-event",
                   G_CALLBACK(gdict_window_state_event_cb), NULL);
  g_signal_connect(handle, "notify::position",
                   G_CALLBACK(gdict_window_handle_notify_position_cb), window);
...
static void gdict_window_class_init(GdictWindowClass *klass) {
  GObjectClass *gobject_class = G_OBJECT_CLASS(klass);
  GtkWidgetClass *widget_class = GTK_WIDGET_CLASS(klass);

  gdict_window_properties[PROP_ACTION] = g_param_spec_enum(
      "action", "Action", "The default action performed by the window",
      GDICT_TYPE_WINDOW_ACTION, GDICT_WINDOW_ACTION_CLEAR,
      G_PARAM_READWRITE | G_PARAM_STATIC_STRINGS | G_PARAM_CONSTRUCT_ONLY);

  gdict_window_properties[PROP_SOURCE_LOADER] = g_param_spec_object(
      "source-loader", "Source Loader",
      "The GdictSourceLoader to be used to load dictionary sources",
      GDICT_TYPE_SOURCE_LOADER,
      G_PARAM_READWRITE | G_PARAM_STATIC_STRINGS | G_PARAM_CONSTRUCT_ONLY);

  gdict_window_properties[PROP_SOURCE_NAME] = g_param_spec_string(
      "source-name", "Source Name", "The name of the GdictSource to be used",
      GDICT_DEFAULT_SOURCE_NAME, G_PARAM_READWRITE | G_PARAM_STATIC_STRINGS);

  gdict_window_properties[PROP_DATABASE] = g_param_spec_string(
      "database", "Database", "The name of the database to search",
      GDICT_DEFAULT_DATABASE, G_PARAM_READWRITE | G_PARAM_STATIC_STRINGS);

  gdict_window_properties[PROP_STRATEGY] = g_param_spec_string(
      "strategy", "Strategy", "The name of the strategy",
      GDICT_DEFAULT_STRATEGY, G_PARAM_READWRITE | G_PARAM_STATIC_STRINGS);

  gdict_window_properties[PROP_PRINT_FONT] = g_param_spec_string(
      "print-font", "Print Font", "The font name to be used when printing",
      GDICT_DEFAULT_PRINT_FONT, G_PARAM_READWRITE | G_PARAM_STATIC_STRINGS);

  gdict_window_properties[PROP_DEFBOX_FONT] = g_param_spec_string(
      "defbox-font", "Defbox Font",
      "The font name to be used by the defbox widget",
      GDICT_DEFAULT_DEFBOX_FONT, G_PARAM_READWRITE | G_PARAM_STATIC_STRINGS);

  gdict_window_properties[PROP_WORD] =
      g_param_spec_string("word", "Word", "The word to search", NULL,
                          G_PARAM_READWRITE | G_PARAM_STATIC_STRINGS);

  gdict_window_properties[PROP_WINDOW_ID] = g_param_spec_uint(
      "window-id", "Window ID", "The unique identifier for this window", 0,
      G_MAXUINT, 0, G_PARAM_READABLE | G_PARAM_STATIC_STRINGS);

  gdict_window_signals[CREATED] = g_signal_new(
      "created", G_OBJECT_CLASS_TYPE(gobject_class), G_SIGNAL_RUN_LAST,
      G_STRUCT_OFFSET(GdictWindowClass, created), NULL, NULL,
      g_cclosure_marshal_VOID__OBJECT, G_TYPE_NONE, 1, GDICT_TYPE_WINDOW);
...
static void gdict_window_init(GdictWindow *window) {
...
GtkWidget *gdict_window_new(GdictWindowAction action, GdictSourceLoader *loader,
  ...
  retval = g_object_new(GDICT_TYPE_WINDOW, "action", action, "source-loader",
                        loader, "source-name", source_name, NULL);
...
GType gdict_window_action_get_type(void) {

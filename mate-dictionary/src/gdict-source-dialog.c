...
#define GET_EDITABLE(x) \
  (GTK_EDITABLE(gtk_builder_get_object(dialog->builder, (x))))
...
static void set_source_loader(GdictSourceDialog *dialog,
                              GdictSourceLoader *loader) {
...
static void transport_combo_changed_cb(GtkWidget *widget, gpointer user_data) {
  GdictSourceDialog *dialog = GDICT_SOURCE_DIALOG(user_data);
  gint transport;

  transport = gtk_combo_box_get_active(GTK_COMBO_BOX(widget));
  if (transport == dialog->transport) return;
...
static gchar *get_text_from_entry(GdictSourceDialog *dialog,
...
static void set_text_to_entry(GdictSourceDialog *dialog,
                              const gchar *entry_name, const gchar *text) {
  GtkWidget *entry;
...
static void set_transport_settings(GdictSourceDialog *dialog) {
...
static void update_dialog_ui(GdictSourceDialog *dialog) {
  GdictSource *source;
  ...
  switch (dialog->action) {
    ...
    case GDICT_SOURCE_DIALOG_EDIT:
      if (!dialog->source_name) {
        g_warning(
            "Attempting to retrieve source, but no "
            "source name has been defined.  Aborting...");
        return;
      }

      source =
          gdict_source_loader_get_source(dialog->loader, dialog->source_name);
      if (!source) {
        g_warning(
            "Attempting to retrieve source, but no "
      ...
      if (!dialog->context) {
        g_warning(
            "Attempting to retrieve the context, but "
      ...
      gdict_database_chooser_set_context(
          GDICT_DATABASE_CHOOSER(dialog->db_chooser), dialog->context);
      gdict_database_chooser_refresh(
          GDICT_DATABASE_CHOOSER(dialog->db_chooser));
      gdict_strategy_chooser_set_context(
          GDICT_STRATEGY_CHOOSER(dialog->strat_chooser), dialog->context);
      gdict_strategy_chooser_refresh(
          GDICT_STRATEGY_CHOOSER(dialog->strat_chooser));
      break;
...
static void build_new_source(GdictSourceDialog *dialog) {
  GdictSource *source;
  gchar *name, *text;
  GdictSourceTransport transport;
  gchar *host, *port;
  ...
  name = g_strdup_printf("source-%lu-%u", (gulong)time(NULL), (guint)getpid());
  ...
  switch (transport) {
    case GDICT_SOURCE_TRANSPORT_DICTD:
      ...
      gdict_source_set_transport(source, GDICT_SOURCE_TRANSPORT_DICTD,
                                 "hostname", host, "port", atoi(port), NULL);
  ...
  if (error) {
    gdict_show_gerror_dialog(GTK_WINDOW(dialog),
                             _("Unable to create a source file"), error);
  ...
  filename = g_build_filename(g_get_user_config_dir(), "mate",
                              "mate-dictionary", name, NULL);
  g_free(name);

  g_file_set_contents(filename, data, length, &error);
  if (error)
    gdict_show_gerror_dialog(GTK_WINDOW(dialog),
                             _("Unable to save source file"), error);
...
static void save_source(GdictSourceDialog *dialog) {
  GdictSource *source;
  GdictDatabaseChooser *db_chooser;
  GdictStrategyChooser *strat_chooser;
  gchar *name, *text;
  ...
  if (!source) {
    g_warning(
        "Attempting to save source `%s', but no "
...
static void gdict_source_dialog_response_cb(GtkDialog *dialog, gint response_id,
                                            gpointer user_data) {
  GError *err = NULL;
...
static void gdict_source_dialog_finalize(GObject *object) {
  ...
  if (dialog->source_name) g_free(dialog->source_name);

  if (dialog->source) g_object_unref(dialog->source);
...
static void gdict_source_dialog_set_property(GObject *object, guint prop_id,
                                             const GValue *value,
                                             GParamSpec *pspec) {
  GdictSourceDialog *dialog = GDICT_SOURCE_DIALOG(object);
...
static void gdict_source_dialog_get_property(GObject *object, guint prop_id,
...
static GObject *gdict_source_dialog_constructor(
  ...
  object = G_OBJECT_CLASS(gdict_source_dialog_parent_class)
  ...
  g_signal_connect(dialog->transport_combo, "changed",
                   G_CALLBACK(transport_combo_changed_cb), dialog);
  ...
  gtk_box_pack_start(GTK_BOX(GET_WIDGET("db-box")), dialog->db_chooser, TRUE,
                     TRUE, 0);
  gtk_widget_show(dialog->db_chooser);

  dialog->strat_chooser = gdict_strategy_chooser_new();
  gtk_box_pack_start(GTK_BOX(GET_WIDGET("strat-box")), dialog->strat_chooser,
                     TRUE, TRUE, 0);
  ...
  switch (dialog->action) {
    case GDICT_SOURCE_DIALOG_VIEW:
      ...
      dialog->close_button = gtk_dialog_add_button(
          GTK_DIALOG(dialog), "gtk-close", GTK_RESPONSE_CLOSE);
      break;
    case GDICT_SOURCE_DIALOG_CREATE:
      dialog->cancel_button = gtk_dialog_add_button(
          GTK_DIALOG(dialog), "gtk-cancel", GTK_RESPONSE_CANCEL);
      dialog->add_button = gtk_dialog_add_button(GTK_DIALOG(dialog), "gtk-add",
                                                 GTK_RESPONSE_ACCEPT);
...
static void gdict_source_dialog_class_init(GdictSourceDialogClass *klass) {
  ...
  g_object_class_install_property(
      gobject_class, PROP_SOURCE_NAME,
      g_param_spec_string(
          "source-name", "Source Name", "The source name", NULL,
          (G_PARAM_READABLE | G_PARAM_WRITABLE | G_PARAM_CONSTRUCT)));
  g_object_class_install_property(
      gobject_class, PROP_ACTION,
      g_param_spec_int(
          "action", "Action", "The action the source dialog should perform", -1,
          GDICT_SOURCE_DIALOG_EDIT, GDICT_SOURCE_DIALOG_VIEW,
          (G_PARAM_READABLE | G_PARAM_WRITABLE | G_PARAM_CONSTRUCT_ONLY)));
}

static void gdict_source_dialog_init(GdictSourceDialog *dialog) {
  ...
  g_signal_connect(dialog, "response",
                   G_CALLBACK(gdict_source_dialog_response_cb), NULL);
}

GtkWidget *gdict_source_dialog_new(GtkWindow *parent, const gchar *title,
                                   GdictSourceDialogAction action,
                                   GdictSourceLoader *loader,
  ...
  retval = g_object_new(GDICT_TYPE_SOURCE_DIALOG, "source-loader", loader,
                        "source-name", source_name, "action", action, "title",
                        title, NULL);

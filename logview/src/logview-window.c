...
static void findbar_close_cb(LogviewFindbar *findbar, gpointer user_data);
static void read_new_lines_cb(LogviewLog *log, const char **lines,
...
static void populate_style_tag_table(LogviewWindow *logview) {
...
static void _gtk_text_buffer_apply_tag_to_rectangle(
    GtkTextBuffer *buffer, int line_start, int line_end, int offset_start,
    int offset_end, char *tag_name) {
...
static void logview_update_statusbar(LogviewWindow *logview,
                                     LogviewLog *active) {
  GDateTime *date_time;
  char *statusbar_text;
  ...
  gtk_statusbar_pop(GTK_STATUSBAR(logview->priv->statusbar), 0);
  gtk_statusbar_push(GTK_STATUSBAR(logview->priv->statusbar), 0,
                     statusbar_text);
...
static void logview_set_font(LogviewWindow *logview, const char *fontname) {
  PangoFontDescription *font_desc;

  if (fontname == NULL) fontname = DEFAULT_LOGVIEW_FONT;
...
static void logview_set_fontsize(LogviewWindow *logview, gboolean store) {
...
static void logview_set_window_title(LogviewWindow *logview,
                                     const char *log_name) {
  char *window_title;
...
static void open_file_selected_cb(GtkWidget *chooser, gint response,
...
static void logview_open_log(GtkAction *action, LogviewWindow *logview) {
  ...
  if (chooser == NULL) {
    chooser = gtk_file_chooser_dialog_new(
        _("Open Log"), GTK_WINDOW(logview), GTK_FILE_CHOOSER_ACTION_OPEN,
        "gtk-cancel", GTK_RESPONSE_CANCEL, "gtk-open", GTK_RESPONSE_OK, NULL);
    gtk_dialog_set_default_response(GTK_DIALOG(chooser), GTK_RESPONSE_OK);
    gtk_window_set_modal(GTK_WINDOW(chooser), TRUE);
    g_signal_connect(chooser, "response", G_CALLBACK(open_file_selected_cb),
                     logview);
    g_signal_connect(chooser, "destroy", G_CALLBACK(gtk_widget_destroyed),
                     &chooser);
...
static void logview_help(GtkAction *action, GtkWidget *parent_window) {
  ...
  gtk_show_uri_on_window(GTK_WINDOW(parent_window), "help:mate-system-log",
...
static void logview_search_text(LogviewWindow *logview, gboolean forward) {
  GtkTextBuffer *buffer;
  ...
  if (!search_start) {
    ...
    search_start =
        gtk_text_buffer_create_mark(buffer, SEARCH_START_MARK, &search, TRUE);
    search_end =
        gtk_text_buffer_create_mark(buffer, SEARCH_END_MARK, &search, TRUE);
  } else {
    if (forward) {
...
wrap:
  ...
  if (forward) {
    res = gtk_text_iter_forward_search(
        &search, text, GTK_TEXT_SEARCH_VISIBLE_ONLY, &start_m, &end_m, NULL);
  } else {
  ...
  if (res) {
    ...
    gtk_text_view_scroll_mark_onscreen(GTK_TEXT_VIEW(logview->priv->text_view),
                                       search_end);

    if (wrapped) {
      logview_findbar_set_message(LOGVIEW_FINDBAR(logview->priv->find_bar),
                                  _("Wrapped"));
    }
  } else {
    if (wrapped) {
...
static void findbar_previous_cb(LogviewFindbar *findbar, gpointer user_data) {
  LogviewWindow *logview = user_data;

  logview_search_text(logview, FALSE);
}

static void findbar_next_cb(LogviewFindbar *findbar, gpointer user_data) {
  LogviewWindow *logview = user_data;

  logview_search_text(logview, TRUE);
}

static gboolean text_changed_timeout_cb(gpointer user_data) {
  LogviewWindow *logview = user_data;
  GtkTextMark *search_start, *search_end;
  GtkTextIter start;
  GtkTextBuffer *buffer;
...
static void findbar_text_changed_cb(LogviewFindbar *findbar,
                                    gpointer user_data) {
  LogviewWindow *logview = user_data;
  ...
  logview->priv->search_timeout_id =
      g_timeout_add(300, text_changed_timeout_cb, logview);
}
...
static void filter_buffer(LogviewWindow *logview, gint start_line) {
  GtkTextBuffer *buffer;
  GtkTextIter start, *end;
  gchar *text;
  GList *cur_filter;
  ...
  for (i = start_line; i < lines; i++) {
    ...
    for (cur_filter = logview->priv->active_filters; cur_filter != NULL;
      ...
      if (logview_filter_filter(LOGVIEW_FILTER(cur_filter->data), text)) {
        gtk_text_buffer_apply_tag(
            buffer, logview_filter_get_tag(LOGVIEW_FILTER(cur_filter->data)),
            &start, end);
        matched = TRUE;
      }
    }
    ...
    if (!matched && logview->priv->matches_only) {
      gtk_text_buffer_apply_tag_by_name(buffer, "invisible-filter", &start,
                                        end);
    } else {
      gtk_text_buffer_remove_tag_by_name(buffer, "invisible-filter", &start,
                                         end);
    }
...
static void filter_remove(LogviewWindow *logview, LogviewFilter *filter) {
  GtkTextIter start, end;
  GtkTextBuffer *buffer;
  ...
  gtk_text_buffer_remove_tag(buffer, logview_filter_get_tag(filter), &start,
                             &end);
}

static void on_filter_toggled(GtkToggleAction *action, LogviewWindow *logview) {
  ...
  if (gtk_toggle_action_get_active(action)) {
    priv->active_filters = g_list_append(
        priv->active_filters, logview_prefs_get_filter(priv->prefs, name));
    filter_buffer(logview, 0);
  } else {
    filter = logview_prefs_get_filter(priv->prefs, name);
    priv->active_filters = g_list_remove(priv->active_filters, filter);
...
static void update_filter_menu(LogviewWindow *window) {
  ...
  if (priv->filter_merge_id != 0) {
    gtk_ui_manager_remove_ui(ui, priv->filter_merge_id);
  }
  ...
  for (l = actions; l != NULL; l = g_list_next(l)) {
    tag = gtk_text_tag_table_lookup(table,
                                    gtk_action_get_name(GTK_ACTION(l->data)));
  ...
  for (l = filters; l != NULL; l = g_list_next(l)) {
    ...
    gtk_action_group_add_action(priv->filter_action_group, GTK_ACTION(action));

    g_signal_connect(action, "toggled", G_CALLBACK(on_filter_toggled), window);

    gtk_ui_manager_add_ui(ui, id, FILTER_PLACEHOLDER, name, name,
...
static void on_logview_filter_manager_response(GtkDialog *dialog, gint response,
                                               LogviewWindow *logview) {
...
static void logview_manage_filters(GtkAction *action, LogviewWindow *logview) {
  GtkWidget *manager;

  manager = logview_filter_manager_new();

  g_signal_connect(manager, "response",
                   G_CALLBACK(on_logview_filter_manager_response), logview);

  gtk_window_set_transient_for(GTK_WINDOW(manager), GTK_WINDOW(logview));
  gtk_widget_show(GTK_WIDGET(manager));
}

static void logview_about(GtkWidget *widget, GtkWidget *window) {
  g_return_if_fail(GTK_IS_WINDOW(window));

  char *license_trans =
      g_strjoin("\n\n", _(logview_about_license[0]),
                _(logview_about_license[1]), _(logview_about_license[2]), NULL);

#ifdef ENABLE_NLS
  const char **p;
  for (p = logview_about_documenters; *p; ++p) *p = _(*p);
#endif
  ...
  gtk_show_about_dialog(
      ...
      logview_about_documenters, "translator_credits",
      strcmp(logview_about_translator_credits, "translator-credits") != 0
          ? logview_about_translator_credits
          : NULL,
      "logo_icon_name", "mate-system-log", NULL);
  g_free(license_trans);

  return;
}

static void logview_toggle_statusbar(GtkAction *action,
                                     LogviewWindow *logview) {
...
static void logview_toggle_match_filters(GtkToggleAction *action,
                                         LogviewWindow *logview) {
...
static GtkActionEntry entries[] = {
    ...
    {"OpenLog", "document-open", N_("_Open..."), "<control>O",
     N_("Open a log from file"), G_CALLBACK(logview_open_log)},
    {"CloseLog", "window-close", N_("_Close"), "<control>W",
     N_("Close this log"), G_CALLBACK(logview_close_log)},
    {"Quit", "application-exit", N_("_Quit"), "<control>Q",
     N_("Quit the log viewer"), G_CALLBACK(gtk_main_quit)},

    {"Copy", "edit-copy", N_("_Copy"), "<control>C", N_("Copy the selection"),
     G_CALLBACK(logview_copy)},
    {"SelectAll", NULL, N_("Select _All"), "<Control>A",
     N_("Select the entire log"), G_CALLBACK(logview_select_all)},
    {"Search", "edit-find", N_("_Find..."), "<control>F",
     N_("Find a word or phrase in the log"), G_CALLBACK(logview_search)},

    {"ViewZoomIn", "zoom-in", N_("Zoom _In"), "<control>plus",
     N_("Bigger text size"), G_CALLBACK(logview_bigger_text)},
    {"ViewZoomOut", "zoom-out", N_("Zoom _Out"), "<control>minus",
     N_("Smaller text size"), G_CALLBACK(logview_smaller_text)},
    {"ViewZoom100", "zoom-original", N_("_Normal Size"), "<control>0",
     N_("Normal text size"), G_CALLBACK(logview_normal_text)},
    ...
    {"HelpContents", "help-browser", N_("_Contents"), "F1",
     N_("Open the help contents for the log viewer"), G_CALLBACK(logview_help)},
    {"AboutAction", "help-about", N_("_About"), NULL,
     N_("Show the about dialog for the log viewer"), G_CALLBACK(logview_about)},
};
...
static GtkToggleActionEntry toggle_entries[] = {
    ...
    {"FilterMatchOnly", NULL, N_("Show matches only"), NULL,
     N_("Only show lines that match one of the given filters"),
     G_CALLBACK(logview_toggle_match_filters), FALSE}};

static gboolean window_size_changed_cb(GtkWidget *widget,
                                       GdkEventConfigure *event,
                                       gpointer data) {
  LogviewWindow *window = data;

  logview_prefs_store_window_size(window->priv->prefs, event->width,
                                  event->height);

  return FALSE;
}

static void real_select_day(LogviewWindow *logview, GDate *date, int first_line,
                            int last_line) {
  GtkTextBuffer *buffer;
  ...
  gtk_text_buffer_remove_tag_by_name(buffer, "invisible", &start_iter,
                                     &end_iter);

  gtk_text_buffer_apply_tag_by_name(buffer, "invisible", &start_iter,
                                    &start_vis);
  gtk_text_buffer_apply_tag_by_name(buffer, "invisible", &end_vis, &end_iter);
...
static void loglist_day_selected_cb(LogviewLoglist *loglist, Day *day,
                                    gpointer user_data) {
  LogviewWindow *logview = user_data;
...
static void loglist_day_cleared_cb(LogviewLoglist *loglist,
                                   gpointer user_data) {
  LogviewWindow *logview = user_data;
  GtkTextBuffer *buffer;
  GtkTextIter start, end;
  ...
  /* clear all previous invisible tags */
  gtk_text_buffer_remove_tag_by_name(buffer, "invisible", &start, &end);
}

static void logview_window_schedule_log_read(LogviewWindow *window,
                                             LogviewLog *log) {
  ...
  logview_log_read_new_lines(log, window->priv->read_cancellable,
                             (LogviewNewLinesCallback)read_new_lines_cb,
                             window);
}

static void log_monitor_changed_cb(LogviewLog *log, gpointer user_data) {
...
static void paint_timestamps(GtkTextBuffer *buffer, int old_line_count,
                             GSList *days) {
  GSList *l;
  ...
  for (l = days; l; l = l->next) {
    ...
    _gtk_text_buffer_apply_tag_to_rectangle(
        buffer, old_line_count + day->first_line - 1,
        old_line_count + day->last_line, 0, day->timestamp_len, "gray");
  }
}
...
static void read_new_lines_cb(LogviewLog *log, const char **lines,
  ...
  gtk_text_view_scroll_to_iter(GTK_TEXT_VIEW(window->priv->text_view), &iter,
                               0.0, FALSE, 0.0, 0.0);

  paint_timestamps(buffer, old_line_count, new_days);

  if (window->priv->monitor_id == 0) {
    window->priv->monitor_id = g_signal_connect(
        log, "log-changed", G_CALLBACK(log_monitor_changed_cb), window);
  }
...
static void active_log_changed_cb(LogviewManager *manager, LogviewLog *log,
                                  LogviewLog *old_log, gpointer data) {
  LogviewWindow *window = data;
  const char **lines;
  GtkTextBuffer *buffer;

  findbar_close_cb(LOGVIEW_FINDBAR(window->priv->find_bar), window);
...
static void font_changed_cb(LogviewPrefs *prefs, const char *font_name,
                            gpointer user_data) {
  LogviewWindow *window = user_data;
...
static void tearoff_changed_cb(LogviewPrefs *prefs, gboolean have_tearoffs,
                               gpointer user_data) {
  LogviewWindow *window = user_data;
...
static const struct {
  ...
  const gchar *action;
} extra_keybindings[] = {{GDK_KEY_KP_Add, GDK_CONTROL_MASK, "ViewZoomIn"},
...
static void message_area_create_error_box(LogviewWindow *window,
                                          GtkWidget *message_area) {
  ...
  gtk_container_add(
      GTK_CONTAINER(gtk_info_bar_get_content_area(GTK_INFO_BAR(message_area))),
      hbox_content);
}

static void message_area_set_labels(LogviewWindow *window, const char *primary,
                                    const char *secondary) {
  char *primary_markup, *secondary_markup;

  primary_markup = g_markup_printf_escaped("<b>%s</b>", primary);
  secondary_markup = g_markup_printf_escaped("<small>%s</small>", secondary);
...
static void message_area_response_cb(GtkInfoBar *message_area, int response_id,
                                     gpointer user_data) {
  gtk_widget_hide(GTK_WIDGET(message_area));

  g_signal_handlers_disconnect_by_func(message_area, message_area_response_cb,
                                       user_data);
}

static void logview_window_finalize(GObject *object) {
...
static void logview_window_init(LogviewWindow *logview) {
  GtkActionGroup *action_group;
  ...
  gtk_action_group_add_toggle_actions(action_group, toggle_entries,
                                      G_N_ELEMENTS(toggle_entries), logview);
  ...
  res = gtk_ui_manager_add_ui_from_resource(
      priv->ui_manager, "/org/mate/system-log/logview-toolbar.xml", &error);
  ...
  gtk_info_bar_add_button(GTK_INFO_BAR(priv->message_area), "gtk-close",
                          GTK_RESPONSE_CLOSE);
  ...
  gtk_box_pack_end(GTK_BOX(priv->version_bar), priv->version_selector, FALSE,
                   FALSE, 0);
  ...
  g_signal_connect(priv->find_bar, "previous", G_CALLBACK(findbar_previous_cb),
                   logview);
  g_signal_connect(priv->find_bar, "next", G_CALLBACK(findbar_next_cb),
                   logview);
  g_signal_connect(priv->find_bar, "text_changed",
                   G_CALLBACK(findbar_text_changed_cb), logview);
  g_signal_connect(priv->find_bar, "close", G_CALLBACK(findbar_close_cb),
                   logview);
  ...
  g_signal_connect(logview, "key-press-event", G_CALLBACK(key_press_event_cb),
                   logview);
...
static void logview_window_class_init(LogviewWindowClass *klass) {

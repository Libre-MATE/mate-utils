...
enum { SYSTEM_FONT_CHANGED, HAVE_TEAROFF_CHANGED, LAST_SIGNAL };
...
static void do_finalize(GObject *obj) {
...
static void logview_prefs_class_init(LogviewPrefsClass *klass) {
  GObjectClass *oclass = G_OBJECT_CLASS(klass);

  oclass->finalize = do_finalize;

  signals[SYSTEM_FONT_CHANGED] = g_signal_new(
      "system-font-changed", G_OBJECT_CLASS_TYPE(oclass), G_SIGNAL_RUN_LAST,
      G_STRUCT_OFFSET(LogviewPrefsClass, system_font_changed), NULL, NULL,
      g_cclosure_marshal_VOID__STRING, G_TYPE_NONE, 1, G_TYPE_STRING);
  signals[HAVE_TEAROFF_CHANGED] = g_signal_new(
      "have-tearoff-changed", G_OBJECT_CLASS_TYPE(oclass), G_SIGNAL_RUN_LAST,
      G_STRUCT_OFFSET(LogviewPrefsClass, have_tearoff_changed), NULL, NULL,
      g_cclosure_marshal_VOID__BOOLEAN, G_TYPE_NONE, 1, G_TYPE_BOOLEAN);
}

static void have_tearoff_changed_cb(GSettings *settings, gchar *key,
                                    gpointer data) {
...
static void monospace_font_changed_cb(GSettings *settings, gchar *key,
                                      gpointer data) {
  LogviewPrefs *prefs = data;
  gchar *monospace_font_name;

  monospace_font_name = g_settings_get_string(settings, key);
  g_signal_emit(prefs, signals[SYSTEM_FONT_CHANGED], 0, monospace_font_name,
                NULL);
...
static void load_filters(LogviewPrefs *prefs) {
  gchar **filters;
  gchar **tokens;
  const gchar *str;
  ...
  filters = g_settings_get_strv(prefs->priv->logview_prefs, PREF_FILTERS);
  ...
  for (idx = 0; filters[idx] != NULL; idx++) {
    ...
    g_object_set(tag, "invisible", g_str_equal(tokens[FILTER_INVISIBLE], "1"),
                 NULL);

    if (strlen(tokens[FILTER_FOREGROUND])) {
      gdk_rgba_parse(&color, tokens[FILTER_FOREGROUND]);
      g_object_set(tag, "foreground-rgba", &color, "foreground-set", TRUE,
                   NULL);
    }
...
static void save_filter_foreach_func(gpointer key, gpointer value,
                                     gpointer user_data) {
  GPtrArray *filters;
  const gchar *name;
  LogviewFilter *filter;
  ...
  g_object_get(filter, "regex", &regex, "texttag", &tag, NULL);
  g_object_get(tag, "foreground-set", &foreground_set, "foreground-rgba",
               &foreground, "paragraph-background-set", &background_set,
               "paragraph-background-rgba", &background, "invisible",
               &invisible, NULL);
...
static void save_filters(LogviewPrefs *prefs) {
  GPtrArray *filters;
  gchar **filters_strv;

  filters = g_ptr_array_new();
  g_hash_table_foreach(prefs->priv->filters, save_filter_foreach_func, filters);
  g_ptr_array_add(filters, NULL);

  filters_strv = (gchar **)g_ptr_array_free(filters, FALSE);
  g_settings_set_strv(prefs->priv->logview_prefs, PREF_FILTERS,
                      (const gchar **)filters_strv);

  g_strfreev(filters_strv);
}

static void get_filters_foreach(gpointer key, gpointer value,
                                gpointer user_data) {
  GList **list;
  list = user_data;
...
static void logview_prefs_init(LogviewPrefs *self) {
...
LogviewPrefs *logview_prefs_get() {
  if (!singleton) singleton = g_object_new(LOGVIEW_TYPE_PREFS, NULL);

  return singleton;
}

void logview_prefs_store_window_size(LogviewPrefs *prefs, int width,
                                     int height) {
  g_assert(LOGVIEW_IS_PREFS(prefs));

  g_settings_set_int(prefs->priv->logview_prefs, PREF_WIDTH, width);
  g_settings_set_int(prefs->priv->logview_prefs, PREF_HEIGHT, height);
}

void logview_prefs_get_stored_window_size(LogviewPrefs *prefs, int *width,
                                          int *height) {
  g_assert(LOGVIEW_IS_PREFS(prefs));

  *width = g_settings_get_int(prefs->priv->logview_prefs, PREF_WIDTH);
  *height = g_settings_get_int(prefs->priv->logview_prefs, PREF_HEIGHT);
...
char *logview_prefs_get_monospace_font_name(LogviewPrefs *prefs) {
  ...
  return (g_settings_get_string(prefs->priv->interface_prefs,
                                MATE_MONOSPACE_FONT_NAME));
}

gboolean logview_prefs_get_have_tearoff(LogviewPrefs *prefs) {
  g_assert(LOGVIEW_IS_PREFS(prefs));

  return (g_settings_get_boolean(prefs->priv->interface_prefs,
                                 MATE_MENUS_HAVE_TEAROFF));
}
...
gchar **logview_prefs_get_stored_logfiles(LogviewPrefs *prefs) {
  ...
  return g_settings_get_strv(prefs->priv->logview_prefs, PREF_LOGFILES);
}

void logview_prefs_store_log(LogviewPrefs *prefs, GFile *file) {
  gchar **stored_logs;
  GFile *stored;
  ...
  if (!found) {
    ...
    g_settings_set_strv(prefs->priv->logview_prefs, PREF_LOGFILES,
...
void logview_prefs_remove_stored_log(LogviewPrefs *prefs, GFile *target) {
  gchar **stored_logs;
  GFile *stored;
  GPtrArray *new_value;
  gint idx;
...
void logview_prefs_store_fontsize(LogviewPrefs *prefs, int fontsize) {
  g_assert(LOGVIEW_IS_PREFS(prefs));
  g_assert(fontsize > 0);
...
void logview_prefs_store_active_logfile(LogviewPrefs *prefs,
  ...
  g_settings_set_string(prefs->priv->logview_prefs, PREF_LOGFILE, filename);
}

char *logview_prefs_get_active_logfile(LogviewPrefs *prefs) {
  g_assert(LOGVIEW_IS_PREFS(prefs));

  return g_settings_get_string(prefs->priv->logview_prefs, PREF_LOGFILE);
}
...
GList *logview_prefs_get_filters(LogviewPrefs *prefs) {
  ...
  g_hash_table_foreach(prefs->priv->filters, get_filters_foreach, &filters);

  return filters;
}

void logview_prefs_remove_filter(LogviewPrefs *prefs, const gchar *name) {
  g_assert(LOGVIEW_IS_PREFS(prefs));

  g_hash_table_remove(prefs->priv->filters, name);

  save_filters(prefs);
}

void logview_prefs_add_filter(LogviewPrefs *prefs, LogviewFilter *filter) {
  gchar *name;

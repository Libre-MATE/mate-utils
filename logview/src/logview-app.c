...
static gboolean main_window_delete_cb(GtkWidget *widget, GdkEvent *event,
                                      gpointer user_data) {
...
static gboolean logview_app_set_window(LogviewApp *app) {
  LogviewWindow *window;
  ...
  if (window) {
    ...
    g_signal_connect(window, "delete-event", G_CALLBACK(main_window_delete_cb),
                     app);
    retval = TRUE;
  }
...
static GSList *parse_syslog(void) {
  char cbuf[BUFSIZ];
  char *cline, *p;
  FILE *cf;
...
static void enumerate_job_finish(EnumerateJob *job) {
  GSList *files = job->logs;
  LogviewApp *app = job->app;

  logview_manager_add_logs_from_name_list(app->priv->manager, files,
                                          files->data);
...
static void enumerate_next_files_async_cb(GObject *source, GAsyncResult *res,
                                          gpointer user_data) {
  ...
  enumerated_files =
      g_file_enumerator_next_files_finish(G_FILE_ENUMERATOR(source), res, NULL);
  if (!enumerated_files) {
  ...
  for (l = enumerated_files; l; l = l->next) {
    ...
    if (g_slist_find_custom(logs, parse_string,
                            (GCompareFunc)g_ascii_strcasecmp) == NULL) {
...
static void enumerate_children_async_cb(GObject *source, GAsyncResult *res,
                                        gpointer user_data) {
  EnumerateJob *job = user_data;
  ...
  g_file_enumerator_next_files_async(enumerator, G_MAXINT, G_PRIORITY_DEFAULT,
                                     NULL, enumerate_next_files_async_cb, job);
}

static void logview_app_first_time_initialize(LogviewApp *app) {
  GSList *logs;
  GFile *log_dir;
  EnumerateJob *job;
  ...
  g_file_enumerate_children_async(log_dir, "standard::*,access::can-read", 0,
...
static void logview_app_class_init(LogviewAppClass *klass) {
  GObjectClass *oclass = G_OBJECT_CLASS(klass);

  oclass->finalize = do_finalize;

  signals[APP_QUIT] =
      g_signal_new("app-quit", G_OBJECT_CLASS_TYPE(oclass), G_SIGNAL_RUN_LAST,
                   G_STRUCT_OFFSET(LogviewAppClass, app_quit), NULL, NULL,
                   g_cclosure_marshal_VOID__VOID, G_TYPE_NONE, 0);
}

static void logview_app_init(LogviewApp *self) {
...
static void check_error_prefs(gpointer data, gpointer user_data) {
  gchar **strings = data;
  LogviewApp *app = user_data;

...
enum { PROP_0, PROP_REGEX, PROP_NAME, PROP_TEXTTAG };
...
static void logview_filter_init(LogviewFilter *object) {
  object->priv = logview_filter_get_instance_private(object);
  object->priv->tag = NULL;
}

static void logview_filter_finalize(GObject *object) {
...
static void logview_filter_set_property(GObject *object, guint prop_id,
                                        const GValue *value,
                                        GParamSpec *pspec) {
  LogviewFilterPrivate *priv = LOGVIEW_FILTER(object)->priv;
...
static void logview_filter_get_property(GObject *object, guint prop_id,
...
static void logview_filter_class_init(LogviewFilterClass *klass) {
  ...
  g_object_class_install_property(
      object_class, PROP_REGEX,
      g_param_spec_string(
          "regex", "regular expression", "regular expression", "NULL",
  ...
  g_object_class_install_property(
      object_class, PROP_TEXTTAG,
      g_param_spec_object(
          "texttag", "texttag", "The text tag to be set on matching lines",
          GTK_TYPE_TEXT_TAG, G_PARAM_READABLE | G_PARAM_WRITABLE));
}

/* public methods */

LogviewFilter *logview_filter_new(const gchar *name, const gchar *regex) {
  return g_object_new(LOGVIEW_TYPE_FILTER, "name", name, "regex", regex, NULL);
}

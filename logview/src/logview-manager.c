...
enum { LOG_ADDED, LOG_CLOSED, ACTIVE_CHANGED, LAST_SIGNAL };
...
static void logview_manager_finalize(GObject *object) {
  LogviewManager *manager;
...
static void logview_manager_class_init(LogviewManagerClass *klass) {
  GObjectClass *object_class = G_OBJECT_CLASS(klass);

  object_class->finalize = logview_manager_finalize;

  signals[LOG_ADDED] = g_signal_new(
      "log-added", G_OBJECT_CLASS_TYPE(object_class), G_SIGNAL_RUN_LAST,
      G_STRUCT_OFFSET(LogviewManagerClass, log_added), NULL, NULL,
      g_cclosure_marshal_VOID__OBJECT, G_TYPE_NONE, 1, LOGVIEW_TYPE_LOG);

  signals[LOG_CLOSED] = g_signal_new(
      "log-closed", G_OBJECT_CLASS_TYPE(object_class), G_SIGNAL_RUN_LAST,
      G_STRUCT_OFFSET(LogviewManagerClass, log_closed), NULL, NULL,
      g_cclosure_marshal_VOID__OBJECT, G_TYPE_NONE, 1, LOGVIEW_TYPE_LOG);

  signals[ACTIVE_CHANGED] = g_signal_new(
      "active-changed", G_OBJECT_CLASS_TYPE(object_class), G_SIGNAL_RUN_LAST,
      G_STRUCT_OFFSET(LogviewManagerClass, active_changed), NULL, NULL,
      logview_marshal_VOID__OBJECT_OBJECT, G_TYPE_NONE, 2, LOGVIEW_TYPE_LOG,
      LOGVIEW_TYPE_LOG);
}

static void logview_manager_init(LogviewManager *self) {
  LogviewManagerPrivate *priv = self->priv =
      logview_manager_get_instance_private(self);
...
static MultipleCreation *multiple_creation_op_new(int total) {
...
static void multiple_creation_op_free(MultipleCreation *mc) {
...
static void create_log_cb(LogviewLog *log, GError *error, gpointer user_data) {
  ...
  if (log) {
    ...
    g_hash_table_insert(data->manager->priv->logs, log_uri, log);
    ...
    if (!data->is_multiple) {
      logview_app_add_error(logview_app_get(), path, error->message);
    } else {
...
static void add_log_from_gfile_internal(LogviewManager *manager, GFile *file,
                                        gboolean set_active,
                                        gboolean is_multiple) {
  char *file_uri;
  LogviewLog *log;
...
static void logview_manager_add_log_from_name(LogviewManager *manager,
                                              const char *filename,
                                              gboolean set_active,
                                              gboolean is_multiple) {
  GFile *file;
...
void logview_manager_add_logs_from_name_list(LogviewManager *manager,
  ...
  for (l = names; l; l = l->next) {
    logview_manager_add_log_from_name(
        manager, l->data, (g_ascii_strcasecmp(active, l->data) == 0), TRUE);
  }
}
...
void logview_manager_add_logs_from_names(LogviewManager *manager, char **names,
  ...
  for (i = 0; names[i]; i++) {
    ...
    logview_manager_add_log_from_name(manager, names[i], set_active, TRUE);
  }
}
...
LogviewLog *logview_manager_get_if_loaded(LogviewManager *manager, char *uri) {
  LogviewLog *log;

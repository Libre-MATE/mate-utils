...
enum { PREVIOUS, NEXT, CLOSE, TEXT_CHANGED, LAST_SIGNAL };

static guint signals[LAST_SIGNAL] = {0};

G_DEFINE_TYPE_WITH_PRIVATE(LogviewFindbar, logview_findbar, GTK_TYPE_TOOLBAR);

static void back_button_clicked_cb(GtkToolButton *button, gpointer user_data) {
  LogviewFindbar *findbar = user_data;

  g_signal_emit(findbar, signals[PREVIOUS], 0);
}

static void forward_button_clicked_cb(GtkToolButton *button,
                                      gpointer user_data) {
  LogviewFindbar *findbar = user_data;
...
static void clear_button_clicked_cb(GtkToolButton *button, gpointer user_data) {
  LogviewFindbar *findbar = user_data;
...
static void entry_activate_cb(GtkWidget *entry, gpointer user_data) {
  LogviewFindbar *findbar = user_data;
...
static gboolean entry_key_press_event_cb(GtkWidget *entry, GdkEventKey *event,
                                         gpointer user_data) {
  LogviewFindbar *findbar = user_data;
...
static gboolean unbold_timeout_cb(gpointer user_data) {
  LogviewFindbar *findbar = user_data;
...
static void logview_findbar_init(LogviewFindbar *findbar) {
  ...
  gtk_tool_item_set_tooltip_text(
      priv->back_button, _("Find previous occurrence of the search string"));
  ...
  gtk_tool_item_set_tooltip_text(
      priv->forward_button, _("Find next occurrence of the search string"));
  ...
  g_signal_connect(priv->entry, "activate", G_CALLBACK(entry_activate_cb),
                   findbar);
  g_signal_connect(priv->entry, "changed", G_CALLBACK(entry_changed_cb),
                   findbar);
  g_signal_connect(priv->entry, "key-press-event",
                   G_CALLBACK(entry_key_press_event_cb), findbar);
}

static void do_grab_focus(GtkWidget *widget) {
...
static void logview_findbar_class_init(LogviewFindbarClass *klass) {
  ...
  signals[PREVIOUS] =
      g_signal_new("previous", G_OBJECT_CLASS_TYPE(oclass), G_SIGNAL_RUN_LAST,
                   G_STRUCT_OFFSET(LogviewFindbarClass, previous), NULL, NULL,
                   g_cclosure_marshal_VOID__VOID, G_TYPE_NONE, 0);

  signals[NEXT] =
      g_signal_new("next", G_OBJECT_CLASS_TYPE(oclass), G_SIGNAL_RUN_LAST,
                   G_STRUCT_OFFSET(LogviewFindbarClass, next), NULL, NULL,
                   g_cclosure_marshal_VOID__VOID, G_TYPE_NONE, 0);

  signals[CLOSE] =
      g_signal_new("close", G_OBJECT_CLASS_TYPE(oclass), G_SIGNAL_RUN_LAST,
                   G_STRUCT_OFFSET(LogviewFindbarClass, close), NULL, NULL,
                   g_cclosure_marshal_VOID__VOID, G_TYPE_NONE, 0);

  signals[TEXT_CHANGED] = g_signal_new(
      "text-changed", G_OBJECT_CLASS_TYPE(oclass), G_SIGNAL_RUN_LAST,
      G_STRUCT_OFFSET(LogviewFindbarClass, text_changed), NULL, NULL,
      g_cclosure_marshal_VOID__VOID, G_TYPE_NONE, 0);
}
...
void logview_findbar_set_message(LogviewFindbar *findbar, const char *text) {
  PangoFontDescription *desc;
  ...
  if (text) {
    ...
    findbar->priv->status_bold_id =
        g_timeout_add(600, unbold_timeout_cb, findbar);
  }

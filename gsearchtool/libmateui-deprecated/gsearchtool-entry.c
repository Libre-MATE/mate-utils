...
enum { PROP_0, PROP_HISTORY_ID, PROP_HISTORY_LENGTH };
...
G_DEFINE_TYPE_WITH_PRIVATE(GsearchHistoryEntry, gsearch_history_entry,
                           GTK_TYPE_COMBO_BOX)

static void gsearch_history_entry_set_property(GObject *object, guint prop_id,
                                               const GValue *value,
                                               GParamSpec *spec) {
  ...
  switch (prop_id) {
    ...
    case PROP_HISTORY_LENGTH:
      gsearch_history_entry_set_history_length(entry, g_value_get_uint(value));
      break;
    default:
      break;
  }
...
static void gsearch_history_entry_get_property(GObject *object, guint prop_id,
                                               GValue *value,
                                               GParamSpec *spec) {
...
static void gsearch_history_entry_destroy(GtkWidget *object) {
...
static void gsearch_history_entry_finalize(GObject *object) {
  GsearchHistoryEntryPrivate *priv;
...
static void gsearch_history_entry_class_init(GsearchHistoryEntryClass *klass) {
  ...
  g_object_class_install_property(
      ...
      g_param_spec_string("history-id", "History ID", "History ID", NULL,
                          G_PARAM_READWRITE | G_PARAM_STATIC_STRINGS));

  g_object_class_install_property(
      object_class, PROP_HISTORY_LENGTH,
      g_param_spec_uint("history-length", "Max History Length",
                        "Max History Length", 0, G_MAXUINT,
...
static GtkListStore *get_history_store(GsearchHistoryEntry *entry) {
  GtkTreeModel *store;
...
static char *get_history_key(GsearchHistoryEntry *entry) {
  return g_strdup(entry->priv->history_id);
}

static GSList *get_history_list(GsearchHistoryEntry *entry) {
  GtkListStore *store;
  GtkTreeIter iter;
  gboolean valid;
  GSList *list = NULL;
...
static void gsearch_history_entry_save_history(GsearchHistoryEntry *entry) {
  GVariant *history;
  GSList *items;
  gchar *key;
  ...
  history = g_settings_get_value(entry->priv->settings, "search-history");
  ...
  if (history) {
    ...
    while ((item = g_variant_iter_next_value(iter))) {
      ...
      if (g_strcmp0(history_key, key) != 0)
        g_variant_builder_add(&history_builder, "{s@as}", history_key,
                              history_list);
      g_free(history_key);
  ...
  g_settings_set_value(entry->priv->settings, "search-history",
...
static gboolean remove_item(GtkListStore *store, const gchar *text) {
  GtkTreeIter iter;
...
static void insert_history_item(GsearchHistoryEntry *entry, const gchar *text,
                                gboolean prepend) {
  GtkListStore *store;
  GtkTreeIter iter;

  if (g_utf8_strlen(text, -1) <= MIN_ITEM_LEN) return;
...
void gsearch_history_entry_prepend_text(GsearchHistoryEntry *entry,
                                        const gchar *text) {
  g_return_if_fail(GSEARCH_IS_HISTORY_ENTRY(entry));
...
void gsearch_history_entry_append_text(GsearchHistoryEntry *entry,
                                       const gchar *text) {
  g_return_if_fail(GSEARCH_IS_HISTORY_ENTRY(entry));
  g_return_if_fail(text != NULL);
...
static void gsearch_history_entry_load_history(GsearchHistoryEntry *entry) {
  GtkListStore *store;
  GtkTreeIter iter;
  GVariant *history;
  gchar *key;
...
static void gsearch_history_entry_init(GsearchHistoryEntry *entry) {
  GsearchHistoryEntryPrivate *priv;
...
void gsearch_history_entry_set_history_length(GsearchHistoryEntry *entry,
                                              guint history_length) {
  g_return_if_fail(GSEARCH_IS_HISTORY_ENTRY(entry));
...
guint gsearch_history_entry_get_history_length(GsearchHistoryEntry *entry) {
...
void gsearch_history_entry_set_enable_completion(GsearchHistoryEntry *entry,
                                                 gboolean enable) {
  g_return_if_fail(GSEARCH_IS_HISTORY_ENTRY(entry));

  if (enable) {
    if (entry->priv->completion != NULL) return;
...
gboolean gsearch_history_entry_get_enable_completion(
    GsearchHistoryEntry *entry) {
...
GtkWidget *gsearch_history_entry_new(const gchar *history_id,
  ...
  ret =
      g_object_new(GSEARCH_TYPE_HISTORY_ENTRY, "has-entry", TRUE, "history-id",
                   history_id, "model", store, "entry-text-column", 0, NULL);
...
static void escape_cell_data_func(GtkTreeViewColumn *col,
...
void gsearch_history_entry_set_escape_func(
    GsearchHistoryEntry *entry, GsearchHistoryEntryEscapeFunc escape_func) {
  GList *cells;
  ...
  if (escape_func != NULL)
    gtk_cell_layout_set_cell_data_func(
        GTK_CELL_LAYOUT(entry), GTK_CELL_RENDERER(cells->data),
        (GtkCellLayoutDataFunc)escape_cell_data_func, escape_func, NULL);
  else
    gtk_cell_layout_set_cell_data_func(GTK_CELL_LAYOUT(entry),
                                       GTK_CELL_RENDERER(cells->data), NULL,
                                       NULL, NULL);

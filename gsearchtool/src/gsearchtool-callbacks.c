...
static void store_window_state_and_geometry(GSearchWindow *gsearch) {
...
void click_help_cb(GtkWidget *widget, gpointer data) {
  ...
  if (error) {
    ...
    dialog = gtk_message_dialog_new(
        ...
        GTK_BUTTONS_OK, _("Could not open help document."));
...
void click_expander_cb(GObject *object, GParamSpec *param_spec, gpointer data) {
  ...
  if (gtk_expander_get_expanded(GTK_EXPANDER(object)) == TRUE) {
    ...
    GdkGeometry default_geometry = {MINIMUM_WINDOW_WIDTH,
    ...
    gtk_window_set_geometry_hints(GTK_WINDOW(gsearch->window),
                                  GTK_WIDGET(gsearch->window),
                                  &default_geometry, GDK_HINT_MIN_SIZE);
  }
}
...
void add_constraint_cb(GtkWidget *widget, gpointer data) {
  ...
  idx = gtk_combo_box_get_active(
      GTK_COMBO_BOX(gsearch->available_options_combo_box));
...
void remove_constraint_cb(GtkWidget *widget, gpointer data) {
  ...
  gtk_container_remove(GTK_CONTAINER(gsearch->available_options_vbox),
...
void look_in_folder_changed_cb(GtkWidget *widget, gpointer data) {
  ...
  value = gtk_file_chooser_get_filename(
      GTK_FILE_CHOOSER(gsearch->look_in_folder_button));

  if (value != NULL) {
    g_settings_set_string(gsearch->mate_search_tool_settings, "look-in-folder",
                          value);
  }
  g_free(value);
}

static gint display_dialog_file_open_limit(GtkWidget *window, gint count) {
  GtkWidget *dialog;
  GtkWidget *button;
  gchar *primary;
  gchar *secondary;
  ...
  primary = g_strdup_printf(
      ngettext("Are you sure you want to open %d document?",
               "Are you sure you want to open %d documents?", count),
      count);

  secondary =
      g_strdup_printf(ngettext("This will open %d separate window.",
                               "This will open %d separate windows.", count),
                      count);

  dialog = gtk_message_dialog_new(
      GTK_WINDOW(window), GTK_DIALOG_DESTROY_WITH_PARENT, GTK_MESSAGE_QUESTION,
      GTK_BUTTONS_CANCEL, primary, NULL);
  gtk_message_dialog_format_secondary_text(GTK_MESSAGE_DIALOG(dialog),
                                           secondary, NULL);
  ...
  button =
      GTK_WIDGET(g_object_new(GTK_TYPE_BUTTON, "label", "gtk-open", "use-stock",
...
static void display_dialog_could_not_open_file(GtkWidget *window,
                                               const gchar *file,
                                               const gchar *message) {
  GtkWidget *dialog;
  gchar *primary;
  ...
  dialog =
      gtk_message_dialog_new(GTK_WINDOW(window), GTK_DIALOG_DESTROY_WITH_PARENT,
                             GTK_MESSAGE_ERROR, GTK_BUTTONS_OK, primary, NULL);
...
static void display_dialog_could_not_open_folder(GtkWidget *window,
                                                 const gchar *folder) {
  GtkWidget *dialog;
  gchar *primary;
  ...
  gtk_message_dialog_format_secondary_text(
      GTK_MESSAGE_DIALOG(dialog), _("The caja file manager is not running."));
...
void open_file_cb(GtkMenuItem *action, gpointer data) {
  ...
  if (gtk_tree_selection_count_selected_rows(
          GTK_TREE_SELECTION(gsearch->search_results_selection)) == 0) {
    return;
  }

  list = gtk_tree_selection_get_selected_rows(
      GTK_TREE_SELECTION(gsearch->search_results_selection), &model);

  if (g_list_length(list) > SILENT_WINDOW_OPEN_LIMIT) {
    gint response;

    response =
        display_dialog_file_open_limit(gsearch->window, g_list_length(list));
  ...
  for (idx = 0; idx < g_list_length(list); idx++) {
    ...
    gtk_tree_model_get_iter(GTK_TREE_MODEL(gsearch->search_results_list_store),
                            &iter, g_list_nth_data(list, idx));

    gtk_tree_model_get(GTK_TREE_MODEL(gsearch->search_results_list_store),
                       &iter, COLUMN_NAME, &utf8_name, COLUMN_LOCALE_FILE,
                       &locale_file, COLUMN_NO_FILES_FOUND, &no_files_found,
                       -1);
    ...
    if (!no_files_found) {
      ...
      if (!g_file_test(locale_file, G_FILE_TEST_EXISTS)) {
        gtk_tree_selection_unselect_iter(
            GTK_TREE_SELECTION(gsearch->search_results_selection), &iter);
        display_dialog_could_not_open_file(gsearch->window, utf8_name,
                                           _("The document does not exist."));

      } else if (open_file_with_application(gsearch->window, locale_file,
                                            app) == FALSE) {
        if (launch_file(locale_file) == FALSE) {
          if (g_file_test(locale_file, G_FILE_TEST_IS_DIR)) {
            if (open_file_with_filemanager(gsearch->window, locale_file) ==
                FALSE) {
              display_dialog_could_not_open_folder(gsearch->window, utf8_name);
            }
          } else {
            display_dialog_could_not_open_file(
                gsearch->window, utf8_name,
...
static gint display_dialog_folder_open_limit(GtkWidget *window, gint count) {
  GtkWidget *dialog;
  GtkWidget *button;
  gchar *primary;
  gchar *secondary;
  ...
  primary = g_strdup_printf(
      ngettext("Are you sure you want to open %d folder?",
               "Are you sure you want to open %d folders?", count),
      count);
...
void open_folder_cb(GtkAction *action, gpointer data) {
  ...
  if (g_list_length(list) > SILENT_WINDOW_OPEN_LIMIT) {
    ...
    response =
        display_dialog_folder_open_limit(gsearch->window, g_list_length(list));
  ...
  for (idx = 0; idx < g_list_length(list); idx++) {
    ...
    gtk_tree_model_get(GTK_TREE_MODEL(gsearch->search_results_list_store),
                       &iter, COLUMN_RELATIVE_PATH, &utf8_folder,
                       COLUMN_LOCALE_FILE, &locale_file, -1);
    ...
    if (idx == 0) {
      ...
      g_file_info =
          g_file_query_info(g_file, G_FILE_ATTRIBUTE_STANDARD_CONTENT_TYPE,
                            G_FILE_QUERY_INFO_NONE, NULL, NULL);
      g_app_info = g_app_info_get_default_for_type(
          g_file_info_get_content_type(g_file_info), FALSE);
    }

    if (open_file_with_application(gsearch->window, locale_folder,
                                   g_app_info) == FALSE) {
...
static void display_dialog_could_not_move_to_trash(GtkWidget *window,
                                                   const gchar *file,
                                                   const gchar *message) {
  GtkWidget *dialog;
  gchar *primary;
...
static gint display_dialog_delete_permanently(GtkWidget *window,
                                              const gchar *file) {
  GtkWidget *dialog;
  GtkWidget *button;
  gchar *primary;
  ...
  secondary = g_strdup_printf(
      _("Trash is unavailable.  Could not move \"%s\" to the trash."), file);
  ...
  button =
      GTK_WIDGET(g_object_new(GTK_TYPE_BUTTON, "label", "gtk-delete",
...
static void display_dialog_could_not_delete(GtkWidget *window,
                                            const gchar *file,
                                            const gchar *message) {
  GtkWidget *dialog;
  gchar *primary;
...
void move_to_trash_cb(GtkAction *action, gpointer data) {
  ...
  total = gtk_tree_selection_count_selected_rows(
  ...
  for (idx = 0; idx < total; idx++) {
    ...
    gtk_tree_model_get(GTK_TREE_MODEL(gsearch->search_results_list_store),
                       &iter, COLUMN_NAME, &utf8_basename, COLUMN_LOCALE_FILE,
                       &locale_filename, COLUMN_NO_FILES_FOUND, &no_files_found,
                       -1);
    ...
    if (idx + 1 == total) {
      last_selected_path = gtk_tree_model_get_path(
          GTK_TREE_MODEL(gsearch->search_results_list_store), &iter);
    }
    ...
    if (result == TRUE) {
      tree_model_iter_free_monitor(
          GTK_TREE_MODEL(gsearch->search_results_list_store), NULL, &iter,
          NULL);
      gtk_list_store_remove(GTK_LIST_STORE(gsearch->search_results_list_store),
                            &iter);
    } else {
      gint response;
      ...
      gtk_tree_selection_unselect_iter(
          GTK_TREE_SELECTION(gsearch->search_results_selection), &iter);
      response =
          display_dialog_delete_permanently(gsearch->window, utf8_filename);
      ...
      if (response == GTK_RESPONSE_OK) {
        ...
        if (result == TRUE) {
          ...
          gtk_list_store_remove(
              GTK_LIST_STORE(gsearch->search_results_list_store), &iter);
        } else {
          gchar *message;
          ...
          display_dialog_could_not_delete(gsearch->window, utf8_basename,
                                          message);
        ...
        message = g_strdup_printf(_("Moving \"%s\" failed: %s."), utf8_filename,
                                  error->message);
        display_dialog_could_not_move_to_trash(gsearch->window, utf8_basename,
                                               message);
        g_error_free(error);
...
gboolean file_button_press_event_cb(GtkWidget *widget, GdkEventButton *event,
  ...
  if (gtk_tree_view_get_path_at_pos(tree, event->x, event->y, &path, NULL, NULL,
                                    NULL)) {
    if ((event->button == 1 || event->button == 2 || event->button == 3) &&
        gtk_tree_selection_path_is_selected(gtk_tree_view_get_selection(tree),
                                            path)) {
...
static gint open_with_list_sort(gconstpointer a, gconstpointer b) {
...
static GtkWidget *mate_image_menu_item_new_from_icon(const gchar *icon_name,
...
static GtkWidget *mate_image_menu_item_new_from_gicon(GIcon *icon_img,
                                                      const gchar *label_name) {
  gchar *concat;
  GtkWidget *icon;
  GtkWidget *box = gtk_box_new(GTK_ORIENTATION_HORIZONTAL, 6);
...
static GtkWidget *mate_image_menu_item_new_from_pixbuf(
    GdkPixbuf *icon_pixbuf, const gchar *label_name) {
  gchar *concat;
  GtkWidget *icon;
  GtkWidget *box = gtk_box_new(GTK_ORIENTATION_HORIZONTAL, 6);
...
static void build_popup_menu_for_file(GSearchWindow *gsearch, gchar *file) {
  ...
  gtk_menu_set_reserve_toggle_size(GTK_MENU(gsearch->search_results_popup_menu),
                                   FALSE);
  ...
  if (file == NULL || g_file_test(file, G_FILE_TEST_IS_DIR) == TRUE) {
    ...
    g_signal_connect(new1, "activate", G_CALLBACK(open_file_cb),
                     (gpointer)gsearch);
  } else {
    GFile *g_file;
    GFileInfo *file_info;
    ...
    if (list_length <= 0) {
      /* Popup menu item: Open */
      new1 = mate_image_menu_item_new_from_icon("document-open", _("_Open"));
      gtk_container_add(GTK_CONTAINER(gsearch->search_results_popup_menu),
                        new1);
      gtk_widget_show(new1);
      ...
      str =
          g_strdup_printf(_("_Open with %s"), g_app_info_get_name(list->data));
      ...
      for (list = g_list_next(list), i = 0; list != NULL;
        ...
        if (list_length >= 4) {
          if (g_app_info_get_icon((GAppInfo *)list->data) != NULL) {
            file_icon =
                g_object_ref(g_app_info_get_icon((GAppInfo *)list->data));
          ...
          if (i == 0) {
            ...
            gtk_menu_set_reserve_toggle_size(
                GTK_MENU(gsearch->search_results_popup_submenu), FALSE);
            ...
            gtk_menu_item_set_submenu(GTK_MENU_ITEM(new2),
                                      gsearch->search_results_popup_submenu);
          }
          gtk_container_add(
              GTK_CONTAINER(gsearch->search_results_popup_submenu), new1);

          /* For submenu items, the "activate" signal is only emitted if the
             user first clicks on the parent menu item.  Since submenus in gtk+
             are automatically displayed when the user hovers over them, most
             will never click on the parent menu item. The work-around is to
             connect to "button-press-event". */
          g_signal_connect(new1, "button-press-event",
                           G_CALLBACK(open_file_event_cb), (gpointer)gsearch);
        } else {
          if (g_app_info_get_icon((GAppInfo *)list->data) != NULL) {
            file_icon =
                g_object_ref(g_app_info_get_icon((GAppInfo *)list->data));
  ...
  new1 = mate_image_menu_item_new_from_icon("document-open",
                                            _("Open Containing _Folder"));
  gtk_container_add(GTK_CONTAINER(gsearch->search_results_popup_menu), new1);
  gtk_widget_show(new1);

  g_signal_connect(new1, "activate", G_CALLBACK(open_folder_cb),
                   (gpointer)gsearch);

  /* Popup menu item: Copy Path */
  if (gtk_tree_selection_count_selected_rows(
          GTK_TREE_SELECTION(gsearch->search_results_selection)) == 1) {
    new1 = mate_image_menu_item_new_from_icon("edit-copy", _("Copy _Path"));
    gtk_container_add(GTK_CONTAINER(gsearch->search_results_popup_menu), new1);
    gtk_widget_show(new1);

    g_signal_connect(new1, "activate", G_CALLBACK(copy_path_cb),
                     (gpointer)gsearch);
  }
  ...
  pixbuf = gtk_icon_theme_load_icon(icon_theme, "user-trash",
  ...
  g_signal_connect(new1, "activate", G_CALLBACK(move_to_trash_cb),
                   (gpointer)gsearch);
  ...
  gsearch->search_results_save_results_as_item =
      mate_image_menu_item_new_from_icon("document-save",
                                         _("_Save Results As..."));
  gtk_container_add(GTK_CONTAINER(gsearch->search_results_popup_menu),
                    gsearch->search_results_save_results_as_item);
  ...
  g_signal_connect(gsearch->search_results_save_results_as_item, "activate",
                   G_CALLBACK(show_file_selector_cb), (gpointer)gsearch);
}
...
gboolean file_button_release_event_cb(GtkWidget *widget, GdkEventButton *event,
  ...
  if (event->window != gtk_tree_view_get_bin_window(
  ...
  if (event->button == 1 || event->button == 2) {
    ...
    if (gtk_tree_view_get_path_at_pos(
            GTK_TREE_VIEW(gsearch->search_results_tree_view), event->x,
  ...
  if (event->button == 3) {
    ...
    gtk_tree_model_get_iter(GTK_TREE_MODEL(gsearch->search_results_list_store),
                            &iter, g_list_first(list)->data);

    gtk_tree_model_get(GTK_TREE_MODEL(gsearch->search_results_list_store),
                       &iter, COLUMN_NAME, &utf8_name_first, COLUMN_LOCALE_FILE,
                       &locale_file_first, COLUMN_NO_FILES_FOUND,
                       &no_files_found, -1);
    ...
    if (!no_files_found) {
      ...
      if (g_list_length(list) >= 2) {
        ...
        for (tmp = g_list_first(list), idx = 0; tmp != NULL;
             tmp = g_list_next(tmp), idx++) {
          GFile *g_file;
          GAppInfo *app_info;
          ...
          gtk_tree_model_get(GTK_TREE_MODEL(gsearch->search_results_list_store),
                             &iter, COLUMN_LOCALE_FILE, &locale_file_tmp, -1);
...
gboolean file_motion_notify_cb(GtkWidget *widget, GdkEventMotion *event,
  ...
  gtk_tree_view_get_path_at_pos(GTK_TREE_VIEW(widget), event->x, event->y,
                                &gsearch->search_results_hover_path, NULL, NULL,
                                NULL);
  ...
  if (!(last_hover_path == NULL &&
        gsearch->search_results_hover_path == NULL) &&
      (!(last_hover_path != NULL &&
         gsearch->search_results_hover_path != NULL) ||
    ...
    if (last_hover_path) {
      ...
      gtk_tree_model_row_changed(
          GTK_TREE_MODEL(gsearch->search_results_list_store), last_hover_path,
          &iter);
    }
...
gboolean file_leave_notify_cb(GtkWidget *widget, GdkEventCrossing *event,
  ...
  if (gsearch->is_search_results_single_click_to_activate &&
      (gsearch->search_results_hover_path != NULL)) {
    gtk_tree_model_get_iter(GTK_TREE_MODEL(gsearch->search_results_list_store),
...
void drag_begin_file_cb(GtkWidget *widget, GdkDragContext *context,
  ...
  number_of_selected_rows = gtk_tree_selection_count_selected_rows(
      GTK_TREE_SELECTION(gsearch->search_results_selection));
...
void drag_file_cb(GtkWidget *widget, GdkDragContext *context,
                  GtkSelectionData *selection_data, guint info, guint drag_time,
                  gpointer data) {
  GSearchWindow *gsearch = data;
  gchar *uri_list = NULL;
  GList *list;
...
void show_file_selector_cb(GtkAction *action, gpointer data) {
  ...
  file_chooser = gtk_file_chooser_dialog_new(
      _("Save Search Results As..."), GTK_WINDOW(gsearch->window),
      GTK_FILE_CHOOSER_ACTION_SAVE, "gtk-cancel", GTK_RESPONSE_CANCEL,
      "gtk-save", GTK_RESPONSE_OK, NULL);
  ...
  g_signal_connect(file_chooser, "response", G_CALLBACK(save_results_cb),
                   gsearch);

  gtk_window_set_modal(GTK_WINDOW(file_chooser), TRUE);
  gtk_window_set_position(GTK_WINDOW(file_chooser),
                          GTK_WIN_POS_CENTER_ON_PARENT);

  gtk_widget_show(GTK_WIDGET(file_chooser));
}

static void display_dialog_could_not_save_no_name(GtkWidget *window) {
  GtkWidget *dialog;
  gchar *primary;
  gchar *secondary;
...
static void display_dialog_could_not_save_to(GtkWidget *window,
                                             const gchar *file,
                                             const gchar *message) {
  GtkWidget *dialog;
  gchar *primary;
  ...
  primary =
      g_strdup_printf(_("Could not save \"%s\" document to \"%s\"."),
                      g_path_get_basename(file), g_path_get_dirname(file));
...
static gint display_dialog_could_not_save_exists(GtkWidget *window,
                                                 const gchar *file) {
  GtkWidget *dialog;
  GtkWidget *button;
  gchar *primary;
  ...
  secondary =
      g_strdup(_("If you replace an existing file, "
                 "its contents will be overwritten."));

  dialog = gtk_message_dialog_new(
      GTK_WINDOW(window), GTK_DIALOG_DESTROY_WITH_PARENT, GTK_MESSAGE_QUESTION,
      GTK_BUTTONS_CANCEL, "%s", primary);
...
void save_results_cb(GtkWidget *chooser, gint response, gpointer data) {
  ...
  if (gsearch->save_results_as_default_filename != NULL) {
    utf8 = g_filename_to_utf8(gsearch->save_results_as_default_filename, -1,
                              NULL, NULL, NULL);
  }
  ...
  if (g_file_test(gsearch->save_results_as_default_filename,
                  G_FILE_TEST_IS_DIR)) {
    display_dialog_could_not_save_to(
        gsearch->window, utf8,
  ...
  if ((fp = fopen(gsearch->save_results_as_default_filename, "w")) != NULL) {
    ...
    for (idx = 0;
         idx < gtk_tree_model_iter_n_children(GTK_TREE_MODEL(store), NULL);
         idx++) {
      if (gtk_tree_model_iter_nth_child(GTK_TREE_MODEL(store), &iter, NULL,
                                        idx) == TRUE) {
        gchar *locale_file;

        gtk_tree_model_get(GTK_TREE_MODEL(store), &iter, COLUMN_LOCALE_FILE,
                           &locale_file, -1);
        fprintf(fp, "%s\n", locale_file);
        g_free(locale_file);
      }
...
gboolean key_press_cb(GtkWidget *widget, GdkEventKey *event, gpointer data) {
  ...
  if (event->keyval == GDK_KEY_Escape) {
    ...
    if (event->state & GDK_SHIFT_MASK) {
      ...
      gtk_tree_model_get(GTK_TREE_MODEL(gsearch->search_results_list_store),
                         &iter, COLUMN_NO_FILES_FOUND, &no_files_found, -1);
...
void columns_changed_cb(GtkTreeView *treeview, gpointer user_data) {
  ...
  if (g_slist_length(order) == NUM_VISIBLE_COLUMNS) {
    g_settings_set_value(gsearch->mate_search_tool_settings, "columns-order",
                         g_variant_new("ai", &array_builder));
  }
  g_slist_free(order);
}

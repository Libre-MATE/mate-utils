...
static GSearchOptionTemplate GSearchOptionTemplates[] = {
    ...
    {SEARCH_CONSTRAINT_TYPE_DATE_BEFORE, "-mtime -%d",
     N_("_Date modified less than"), N_("days"), FALSE},
    {SEARCH_CONSTRAINT_TYPE_DATE_AFTER, "\\( -mtime +%d -o -mtime %d \\)",
     N_("Date modified more than"), N_("days"), FALSE},
    {SEARCH_CONSTRAINT_TYPE_SEPARATOR, NULL, NULL, NULL, TRUE},
    {SEARCH_CONSTRAINT_TYPE_NUMERIC, "\\( -size %uc -o -size +%uc \\)",
     N_("S_ize at least"), N_("kilobytes"), FALSE},
    {SEARCH_CONSTRAINT_TYPE_NUMERIC, "\\( -size %uc -o -size -%uc \\)",
     N_("Si_ze at most"), N_("kilobytes"), FALSE},
    {SEARCH_CONSTRAINT_TYPE_BOOLEAN, "-size 0c \\( -type f -o -type d \\)",
     N_("File is empty"), NULL, FALSE},
    {SEARCH_CONSTRAINT_TYPE_SEPARATOR, NULL, NULL, NULL, TRUE},
    {SEARCH_CONSTRAINT_TYPE_TEXT, "-user '%s'", N_("Owned by _user"), NULL,
     FALSE},
    {SEARCH_CONSTRAINT_TYPE_TEXT, "-group '%s'", N_("Owned by _group"), NULL,
     FALSE},
    {SEARCH_CONSTRAINT_TYPE_BOOLEAN, "\\( -nouser -o -nogroup \\)",
     N_("Owner is unrecognized"), NULL, FALSE},
    {SEARCH_CONSTRAINT_TYPE_SEPARATOR, NULL, NULL, NULL, TRUE},
    {SEARCH_CONSTRAINT_TYPE_TEXT, "'!' -name '*%s*'",
     N_("Na_me does not contain"), NULL, FALSE},
    {SEARCH_CONSTRAINT_TYPE_TEXT, "-regex '%s'",
     N_("Name matches regular e_xpression"), NULL, FALSE},
    {SEARCH_CONSTRAINT_TYPE_SEPARATOR, NULL, NULL, NULL, TRUE},
    {SEARCH_CONSTRAINT_TYPE_BOOLEAN, "SHOW_HIDDEN_FILES",
     N_("Show hidden and backup files"), NULL, FALSE},
    {SEARCH_CONSTRAINT_TYPE_BOOLEAN, "-follow", N_("Follow symbolic links"),
     NULL, FALSE},
    {SEARCH_CONSTRAINT_TYPE_BOOLEAN, "EXCLUDE_OTHER_FILESYSTEMS",
     N_("Exclude other filesystems"), NULL, FALSE},
...
static GtkTargetEntry GSearchDndTable[] = {
    {"text/uri-list", 0, 1}, {"text/plain", 0, 0}, {"STRING", 0, 0}};

static guint GSearchTotalDnds =
    sizeof(GSearchDndTable) / sizeof(GSearchDndTable[0]);
...
static GOptionEntry GSearchGOptionEntries[] = {
    {"version", 0, G_OPTION_FLAG_NO_ARG, G_OPTION_ARG_CALLBACK, version_cb,
     N_("Show version of the application"), NULL},
    {"named", 0, 0, G_OPTION_ARG_STRING, &GSearchGOptionArguments.name, NULL,
     N_("STRING")},
    {"path", 0, 0, G_OPTION_ARG_STRING, &GSearchGOptionArguments.path, NULL,
     N_("PATH")},
    {"sortby", 0, 0, G_OPTION_ARG_STRING, &GSearchGOptionArguments.sortby, NULL,
     N_("VALUE")},
    {"descending", 0, 0, G_OPTION_ARG_NONE, &GSearchGOptionArguments.descending,
     NULL, NULL},
    {"start", 0, 0, G_OPTION_ARG_NONE, &GSearchGOptionArguments.start, NULL,
     NULL},
    {"contains", 0, 0, G_OPTION_ARG_STRING, &GSearchGOptionArguments.contains,
     NULL, N_("STRING")},
    {"mtimeless", 0, 0, G_OPTION_ARG_STRING, &GSearchGOptionArguments.mtimeless,
     NULL, N_("DAYS")},
    {"mtimemore", 0, 0, G_OPTION_ARG_STRING, &GSearchGOptionArguments.mtimemore,
     NULL, N_("DAYS")},
    {"sizemore", 0, 0, G_OPTION_ARG_STRING, &GSearchGOptionArguments.sizemore,
     NULL, N_("KILOBYTES")},
    {"sizeless", 0, 0, G_OPTION_ARG_STRING, &GSearchGOptionArguments.sizeless,
     NULL, N_("KILOBYTES")},
    {"empty", 0, 0, G_OPTION_ARG_NONE, &GSearchGOptionArguments.empty, NULL,
     NULL},
    {"user", 0, 0, G_OPTION_ARG_STRING, &GSearchGOptionArguments.user, NULL,
     N_("USER")},
    {"group", 0, 0, G_OPTION_ARG_STRING, &GSearchGOptionArguments.group, NULL,
     N_("GROUP")},
    {"nouser", 0, 0, G_OPTION_ARG_NONE, &GSearchGOptionArguments.nouser, NULL,
     NULL},
    {"notnamed", 0, 0, G_OPTION_ARG_STRING, &GSearchGOptionArguments.notnamed,
     NULL, N_("STRING")},
    {"regex", 0, 0, G_OPTION_ARG_STRING, &GSearchGOptionArguments.regex, NULL,
     N_("PATTERN")},
    {"hidden", 0, 0, G_OPTION_ARG_NONE, &GSearchGOptionArguments.hidden, NULL,
     NULL},
    {"follow", 0, 0, G_OPTION_ARG_NONE, &GSearchGOptionArguments.follow, NULL,
     NULL},
    {"mounts", 0, 0, G_OPTION_ARG_NONE, &GSearchGOptionArguments.mounts, NULL,
     NULL},
...
static gboolean handle_locate_command_stdout_io(GIOChannel *ioc,
...
static void setup_case_insensitive_arguments(GSearchWindow *gsearch) {
  ...
  g_spawn_command_line_sync("find /dev/null -iname 'string'", NULL, &cmd_stderr,
                            NULL, NULL);

  if ((cmd_stderr != NULL) && (strlen(cmd_stderr) == 0)) {
    find_command_default_name_argument = g_strdup("-iname");
    GSearchOptionTemplates[SEARCH_CONSTRAINT_FILE_IS_NOT_NAMED].option =
        g_strdup("'!' -iname '*%s*'");
  } else {
  ...
  if ((cmd_stderr != NULL) && (strlen(cmd_stderr) == 0)) {
    ...
    if ((cmd_stderr != NULL) && (strlen(cmd_stderr) == 0)) {
      GSearchOptionTemplates[SEARCH_CONSTRAINT_CONTAINS_THE_TEXT].option =
          g_strdup_printf("'!' -type p -exec %s -i -I -c '%%s' {} \\;",
                          GREP_COMMAND);
    } else {
      GSearchOptionTemplates[SEARCH_CONSTRAINT_CONTAINS_THE_TEXT].option =
          g_strdup_printf("'!' -type p -exec %s -i -c '%%s' {} \\;",
                          GREP_COMMAND);
    }
  } else {
    GSearchOptionTemplates[SEARCH_CONSTRAINT_CONTAINS_THE_TEXT].option =
  ...
  if (locate != NULL) {
    ...
    if (g_spawn_async_with_pipes(g_get_home_dir(), argv, NULL,
                                 ...
                                 &locate_database_check_command_pid, NULL,
                                 &child_stdout, NULL, NULL)) {
    ...
    if (gsearch->is_locate_database_available == TRUE) {
      ...
      /* run locate command asynchronously because on some systems it can be
       * slow */
      if (g_spawn_async_with_pipes(g_get_home_dir(), argv, NULL,
                                   G_SPAWN_SEARCH_PATH, NULL, NULL,
                                   &locate_database_check_command_pid, NULL,
...
static gchar *setup_find_name_options(gchar *file) {
...
static gboolean has_additional_constraints(GSearchWindow *gsearch) {
  GList *list;

  if (gsearch->available_options_selected_list != NULL) {
    for (list = gsearch->available_options_selected_list; list != NULL;
...
static void display_dialog_character_set_conversion_error(GtkWidget *window,
                                                          gchar *string,
                                                          GError *error) {
  GtkWidget *dialog;
  ...
  dialog = gtk_message_dialog_new(
      ...
      GTK_BUTTONS_OK, _("Character set conversion failed for \"%s\""), string);

  gtk_message_dialog_format_secondary_text(
      GTK_MESSAGE_DIALOG(dialog), (error == NULL) ? " " : error->message, NULL);
...
static void start_animation(GSearchWindow *gsearch, gboolean first_pass) {
  if (first_pass == TRUE) {
    gchar *title = NULL;
    ...
    if (g_settings_get_boolean(gsearch->mate_desktop_interface_settings,
                               "enable-animations")) {
    ...
    gtk_widget_set_sensitive(GTK_WIDGET(gsearch->search_results_tree_view),
                             TRUE);
...
static void stop_animation(GSearchWindow *gsearch) {
  ...
  if (GTK_IS_MENU_ITEM(gsearch->search_results_save_results_as_item) == TRUE) {
    gtk_widget_set_sensitive(gsearch->search_results_save_results_as_item,
                             TRUE);
  }
...
gchar *build_search_command(GSearchWindow *gsearch, gboolean first_pass) {
  GString *command;
  GError *error = NULL;
  ...
  file_is_named_utf8 = g_strdup(
      (gchar *)gtk_entry_get_text(GTK_ENTRY(gsearch_history_entry_get_entry(
  ...
  if (!file_is_named_utf8 || !*file_is_named_utf8) {
    ...
    if (locale == NULL) {
      ...
      display_dialog_character_set_conversion_error(gsearch->window,
                                                    file_is_named_utf8, error);
      g_free(file_is_named_utf8);
      g_error_free(error);
      return NULL;
    }
  ...
  file_is_named_locale =
      g_locale_from_utf8(file_is_named_utf8, -1, NULL, NULL, &error);
  ...
  look_in_folder_locale = gtk_file_chooser_get_filename(
      GTK_FILE_CHOOSER(gsearch->look_in_folder_button));

  if (look_in_folder_locale == NULL) {
    /* If for some reason a path was not returned fallback to the user's home
     * directory. */
  ...
  if (!g_str_has_suffix(look_in_folder_locale, G_DIR_SEPARATOR_S)) {
    ...
    look_in_folder_locale =
        g_strconcat(look_in_folder_locale, G_DIR_SEPARATOR_S, NULL);
    g_free(tmp);
  }
  g_free(gsearch->command_details->look_in_folder_string);

  look_in_folder_backslashed =
      backslash_backslash_characters(look_in_folder_locale);
  look_in_folder_escaped = escape_double_quotes(look_in_folder_backslashed);
  gsearch->command_details->look_in_folder_string =
      g_strdup(look_in_folder_locale);
  ...
  if ((gtk_widget_get_visible(gsearch->available_options_vbox) == FALSE) ||
    ...
    file_is_named_backslashed =
        backslash_backslash_characters(file_is_named_locale);
    file_is_named_escaped = escape_double_quotes(file_is_named_backslashed);
    gsearch->command_details->name_contains_pattern_string =
        g_strdup(file_is_named_utf8);
    ...
    if (gsearch->command_details->is_command_first_pass == TRUE) {
      ...
      locate = g_find_program_in_path("locate");
      disable_quick_search = g_settings_get_boolean(
          gsearch->mate_search_tool_settings, "disable-quick-search");
      gsearch->command_details->is_command_second_pass_enabled =
          !g_settings_get_boolean(gsearch->mate_search_tool_settings,
                                  "disable-quick-search-second-scan");

      /* Use caja settings for thumbnails if caja is installed, else fall back
       * to the caja default */
      if (gsearch->caja_schema_exists) {
        show_thumbnails_enum = g_settings_get_enum(gsearch->caja_settings,
      ...
      if (show_thumbnails_enum == SPEED_TRADEOFF_ALWAYS ||
        ...
        if (gsearch->caja_schema_exists) {
          value =
              g_settings_get_value(gsearch->caja_settings, "thumbnail-limit");
          if (value) {
      ...
      if ((disable_quick_search == FALSE) &&
          (gsearch->is_locate_database_available == TRUE) && (locate != NULL) &&
          (is_quick_search_excluded_path(look_in_folder_locale) == FALSE)) {
        g_string_append_printf(command, "%s %s \"%s*%s\"", locate,
                               locate_command_default_options,
                               look_in_folder_escaped, file_is_named_escaped);
        gsearch->command_details->is_command_using_quick_mode = TRUE;
      } else {
        g_string_append_printf(
            command, "find \"%s\" %s \"%s\" -print", look_in_folder_escaped,
            find_command_default_name_argument, file_is_named_escaped);
      }
      g_free(locate);
    } else {
      g_string_append_printf(
    ...
    g_string_append_printf(command, "find \"%s\" %s", look_in_folder_escaped,
    ...
    for (list = gsearch->available_options_selected_list; list != NULL;
      ...
      switch (GSearchOptionTemplates[constraint->constraint_id].type) {
        case SEARCH_CONSTRAINT_TYPE_BOOLEAN:
          if (strcmp(GSearchOptionTemplates[constraint->constraint_id].option,
                     "EXCLUDE_OTHER_FILESYSTEMS") == 0) {
            disable_mount_argument = FALSE;
          } else if (strcmp(GSearchOptionTemplates[constraint->constraint_id]
                                .option,
                            "SHOW_HIDDEN_FILES") == 0) {
            gsearch->command_details->is_command_show_hidden_files_enabled =
                TRUE;
          } else {
            g_string_append_printf(
                command, "%s ",
        ...
        case SEARCH_CONSTRAINT_TYPE_TEXT:
          if (strcmp(GSearchOptionTemplates[constraint->constraint_id].option,
            ...
            if (regex != NULL) {
              gsearch->command_details->is_command_regex_matching_enabled =
                  TRUE;
              gsearch->command_details->name_contains_regex_string =
                  g_locale_from_utf8(regex, -1, NULL, NULL, NULL);
            }
        ...
        case SEARCH_CONSTRAINT_TYPE_DATE_AFTER:
          g_string_append_printf(
              command, GSearchOptionTemplates[constraint->constraint_id].option,
              constraint->data.time, constraint->data.time);
          g_string_append_c(command, ' ');
          break;
        default:
          break;
...
static void add_file_to_search_results(const gchar *file, GtkListStore *store,
                                       GtkTreeIter *iter,
  ...
  if (g_hash_table_lookup_extended(gsearch->search_results_filename_hash_table,
                                   file, NULL, NULL) == TRUE) {
    return;
  }
  ...
  g_hash_table_insert(gsearch->search_results_filename_hash_table,
                      g_strdup(file), NULL);

  if (gtk_tree_view_get_headers_visible(
          GTK_TREE_VIEW(gsearch->search_results_tree_view)) == FALSE) {
    gtk_tree_view_set_headers_visible(
        GTK_TREE_VIEW(gsearch->search_results_tree_view), TRUE);
  }

  g_file = g_file_new_for_path(file);
  file_info = g_file_query_info(
      g_file, "standard::*,time::modified,thumbnail::path", 0, NULL, NULL);
  ...
  gtk_list_store_set(
      GTK_LIST_STORE(store), iter, COLUMN_ICON, pixbuf, COLUMN_NAME,
      utf8_base_name, COLUMN_RELATIVE_PATH, utf8_relative_dir_name,
      COLUMN_LOCALE_FILE, file, COLUMN_READABLE_SIZE, readable_size,
      COLUMN_SIZE, (-1) * (gdouble)file_size, COLUMN_TYPE,
  ...
  if (monitor) {
    ...
    if (error == NULL) {
      ...
      gtk_list_store_set(GTK_LIST_STORE(store), iter, COLUMN_MONITOR, monitor,
                         -1);

      g_signal_connect(handle, "changed", G_CALLBACK(file_changed_cb), monitor);
    } else {
...
static void add_no_files_found_message(GSearchWindow *gsearch) {
  /* When the list is empty append a 'No Files Found.' message. */
  gtk_widget_set_sensitive(GTK_WIDGET(gsearch->search_results_tree_view),
                           FALSE);
  gtk_tree_view_set_headers_visible(
      GTK_TREE_VIEW(gsearch->search_results_tree_view), FALSE);
  gtk_tree_view_column_set_visible(gsearch->search_results_folder_column,
                                   FALSE);
  ...
  gtk_tree_view_columns_autosize(
      GTK_TREE_VIEW(gsearch->search_results_tree_view));
  g_object_set(gsearch->search_results_name_cell_renderer, "underline",
               PANGO_UNDERLINE_NONE, "underline-set", FALSE, NULL);
  gtk_list_store_append(GTK_LIST_STORE(gsearch->search_results_list_store),
                        &gsearch->search_results_iter);
  gtk_list_store_set(GTK_LIST_STORE(gsearch->search_results_list_store),
                     &gsearch->search_results_iter, COLUMN_ICON, NULL,
                     COLUMN_NAME, _("No files found"), COLUMN_RELATIVE_PATH, "",
                     COLUMN_LOCALE_FILE, "", COLUMN_READABLE_SIZE, "",
                     COLUMN_SIZE, (gdouble)0, COLUMN_TYPE, "",
                     COLUMN_READABLE_DATE, "", COLUMN_DATE, (gdouble)0,
                     COLUMN_NO_FILES_FOUND, TRUE, -1);
}

void update_search_counts(GSearchWindow *gsearch) {
  ...
  total_files = gtk_tree_model_iter_n_children(
      GTK_TREE_MODEL(gsearch->search_results_list_store), NULL);
  ...
  if (total_files == 0) {
    ...
    title_bar_string = g_strdup_printf(
        ngettext("%'d File Found", "%'d Files Found", total_files),
        total_files);
  ...
  tmp = title_bar_string;
  title_bar_string =
      g_strconcat(title_bar_string, " - ", _("Search for Files"), NULL);
...
static void intermediate_file_count_update(GSearchWindow *gsearch) {
  gchar *string;
  gint count;
...
static GtkTreeModel *gsearch_create_list_of_templates(void) {
  GtkListStore *store;
  GtkTreeIter iter;
  gint idx;
  ...
  for (idx = 0; GSearchOptionTemplates[idx].type != SEARCH_CONSTRAINT_TYPE_NONE;
       idx++) {
    if (GSearchOptionTemplates[idx].type == SEARCH_CONSTRAINT_TYPE_SEPARATOR) {
      ...
      gchar *text =
          remove_mnemonic_character(_(GSearchOptionTemplates[idx].desc));
      gtk_list_store_append(store, &iter);
...
static void set_constraint_info_defaults(GSearchConstraint *opt) {
...
void update_constraint_info(GSearchConstraint *constraint, gchar *info) {
  switch (GSearchOptionTemplates[constraint->constraint_id].type) {
    case SEARCH_CONSTRAINT_TYPE_TEXT:
...
void set_constraint_selected_state(GSearchWindow *gsearch, gint constraint_id,
                                   gboolean state) {
  gint idx;
  ...
  for (idx = 0; GSearchOptionTemplates[idx].type != SEARCH_CONSTRAINT_TYPE_NONE;
    ...
    if (GSearchOptionTemplates[idx].is_selected == FALSE) {
      gtk_combo_box_set_active(
          GTK_COMBO_BOX(gsearch->available_options_combo_box), idx);
...
void set_constraint_gsettings_boolean(gint constraint_id, gboolean flag) {
  ...
  switch (constraint_id) {
    case SEARCH_CONSTRAINT_CONTAINS_THE_TEXT:
      g_settings_set_boolean(select_settings, "contains-the-text", flag);
      break;
    case SEARCH_CONSTRAINT_DATE_MODIFIED_BEFORE:
      g_settings_set_boolean(select_settings, "date-modified-less-than", flag);
      break;
    case SEARCH_CONSTRAINT_DATE_MODIFIED_AFTER:
      g_settings_set_boolean(select_settings, "date-modified-more-than", flag);
      break;
    case SEARCH_CONSTRAINT_SIZE_IS_MORE_THAN:
      g_settings_set_boolean(select_settings, "size-at-least", flag);
      break;
    case SEARCH_CONSTRAINT_SIZE_IS_LESS_THAN:
      g_settings_set_boolean(select_settings, "size-at-most", flag);
      break;
    case SEARCH_CONSTRAINT_FILE_IS_EMPTY:
      g_settings_set_boolean(select_settings, "file-is-empty", flag);
      break;
    case SEARCH_CONSTRAINT_OWNED_BY_USER:
      g_settings_set_boolean(select_settings, "owned-by-user", flag);
      break;
    case SEARCH_CONSTRAINT_OWNED_BY_GROUP:
      g_settings_set_boolean(select_settings, "owned-by-group", flag);
      break;
    case SEARCH_CONSTRAINT_OWNER_IS_UNRECOGNIZED:
      g_settings_set_boolean(select_settings, "owner-is-unrecognized", flag);
      break;
    case SEARCH_CONSTRAINT_FILE_IS_NOT_NAMED:
      g_settings_set_boolean(select_settings, "name-does-not-contain", flag);
      break;
    ...
    case SEARCH_CONSTRAINT_FOLLOW_SYMBOLIC_LINKS:
      g_settings_set_boolean(select_settings, "follow-symbolic-links", flag);
      break;
...
static void add_atk_namedesc(GtkWidget *widget, const gchar *name,
                             const gchar *desc) {
  ...
  if (name != NULL) atk_object_set_name(atk_widget, name);
  if (desc != NULL) atk_object_set_description(atk_widget, desc);
}
...
static void add_atk_relation(GtkWidget *obj1, GtkWidget *obj2,
...
static void gsearch_setup_goption_descriptions(void) {
  gint i = 1;
  gint j;

  GSearchGOptionEntries[i++].description =
      g_strdup(_("Set the text of \"Name contains\" search option"));
  GSearchGOptionEntries[i++].description =
      g_strdup(_("Set the text of \"Look in folder\" search option"));
  GSearchGOptionEntries[i++].description = g_strdup(_(
      "Sort files by one of the following: name, folder, size, type, or date"));
  GSearchGOptionEntries[i++].description =
      g_strdup(_("Set sort order to descending, the default is ascending"));
  GSearchGOptionEntries[i++].description =
      g_strdup(_("Automatically start a search"));

  for (j = 0; GSearchOptionTemplates[j].type != SEARCH_CONSTRAINT_TYPE_NONE;
       j++) {
    if (GSearchOptionTemplates[j].type != SEARCH_CONSTRAINT_TYPE_SEPARATOR) {
      gchar *text =
          remove_mnemonic_character(_(GSearchOptionTemplates[j].desc));
      if (GSearchOptionTemplates[j].type == SEARCH_CONSTRAINT_TYPE_BOOLEAN) {
        GSearchGOptionEntries[i++].description =
            g_strdup_printf(_("Select the \"%s\" search option"), text);
      } else {
        GSearchGOptionEntries[i++].description =
            g_strdup_printf(_("Select and set the \"%s\" search option"), text);
      }
      g_free(text);
    }
  }
...
static gboolean handle_goption_args(GSearchWindow *gsearch) {
  ...
  if (GSearchGOptionArguments.name != NULL) {
    ...
    gtk_entry_set_text(
        GTK_ENTRY(gsearch_history_entry_get_entry(
            GSEARCH_HISTORY_ENTRY(gsearch->name_contains_entry))),
  ...
  if (GSearchGOptionArguments.nouser) {
    goption_args_found = TRUE;
    add_constraint(gsearch, SEARCH_CONSTRAINT_OWNER_IS_UNRECOGNIZED, NULL,
                   TRUE);
  }
  ...
  if (GSearchGOptionArguments.hidden) {
    goption_args_found = TRUE;
    add_constraint(gsearch, SEARCH_CONSTRAINT_SHOW_HIDDEN_FILES_AND_FOLDERS,
                   NULL, TRUE);
  }
  if (GSearchGOptionArguments.follow) {
    goption_args_found = TRUE;
    add_constraint(gsearch, SEARCH_CONSTRAINT_FOLLOW_SYMBOLIC_LINKS, NULL,
                   TRUE);
  }
  if (GSearchGOptionArguments.mounts) {
    goption_args_found = TRUE;
    add_constraint(gsearch, SEARCH_CONSTRAINT_SEARCH_OTHER_FILESYSTEMS, NULL,
                   TRUE);
  }
  if (GSearchGOptionArguments.sortby != NULL) {
    ...
    if (GSearchGOptionArguments.descending) {
      gtk_tree_sortable_set_sort_column_id(
          GTK_TREE_SORTABLE(gsearch->search_results_list_store), sort_by,
          GTK_SORT_DESCENDING);
    } else {
      gtk_tree_sortable_set_sort_column_id(
          GTK_TREE_SORTABLE(gsearch->search_results_list_store), sort_by,
...
static gboolean handle_search_command_stdout_io(GIOChannel *ioc,
                                                GIOCondition condition,
                                                gpointer data) {
  GSearchWindow *gsearch = data;
  gboolean broken_pipe = FALSE;
  ...
  if (condition & G_IO_IN) {
    ...
    string = g_string_new(NULL);
    look_in_folder_string_length =
        strlen(gsearch->command_details->look_in_folder_string);
    ...
    do {
      ...
      if (strncmp(string->str, gsearch->command_details->look_in_folder_string,
                  look_in_folder_string_length) == 0) {
        if (strlen(string->str) != look_in_folder_string_length) {
          filename = g_path_get_basename(utf8);

          if (fnmatch(gsearch->command_details->name_contains_pattern_string,
                      filename, FNM_NOESCAPE | FNM_CASEFOLD) != FNM_NOMATCH) {
            if (gsearch->command_details
                    ->is_command_show_hidden_files_enabled) {
              if (gsearch->command_details->is_command_regex_matching_enabled ==
                  FALSE) {
                add_file_to_search_results(
                    string->str, gsearch->search_results_list_store,
                    &gsearch->search_results_iter, gsearch);
              } else if (compare_regex(gsearch->command_details
                                           ->name_contains_regex_string,
                                       filename)) {
                add_file_to_search_results(
                    string->str, gsearch->search_results_list_store,
                    &gsearch->search_results_iter, gsearch);
      ...
      gtk_tree_view_get_visible_rect(
          GTK_TREE_VIEW(gsearch->search_results_tree_view), &prior_rect);

      if (prior_rect.y == 0) {
        gtk_tree_view_get_visible_rect(
            GTK_TREE_VIEW(gsearch->search_results_tree_view), &after_rect);
        if (after_rect.y <= 40) { /* limit this hack to the first few pixels */
          gtk_tree_view_scroll_to_point(
              GTK_TREE_VIEW(gsearch->search_results_tree_view), -1, 0);
        }
      }
  ...
  if (!(condition & G_IO_IN) || broken_pipe == TRUE) {
    ...
    if ((gsearch->command_details->command_status != MAKE_IT_STOP) &&
        ...
        (is_second_scan_excluded_path(
             gsearch->command_details->look_in_folder_string) == FALSE)) {
      gchar *command;

      /* Free these strings now because they are reassign values during the
       * second pass. */
      ...
      gsearch->command_details->command_status =
          (gsearch->command_details->command_status == MAKE_IT_STOP) ? ABORTED
                                                                     : STOPPED;
...
static gboolean handle_search_command_stderr_io(GIOChannel *ioc,
                                                GIOCondition condition,
                                                gpointer data) {
  GSearchWindow *gsearch = data;
    ...
    if (error_msgs != NULL) {
      if (error_msgs->len > 0) {
        ...
        if (gsearch->command_details->is_command_using_quick_mode != TRUE) {
          ...
          dialog = gtk_message_dialog_new(
              GTK_WINDOW(gsearch->window), GTK_DIALOG_DESTROY_WITH_PARENT,
          ...
          gtk_box_pack_start(
              GTK_BOX(gtk_dialog_get_content_area(GTK_DIALOG(dialog))), hbox,
              FALSE, FALSE, 0);
          ...
          gtk_widget_show(dialog);
        } else if ((gsearch->command_details->is_command_second_pass_enabled ==
                    FALSE) ||
                   (is_second_scan_excluded_path(
                        gsearch->command_details->look_in_folder_string) ==
                    TRUE)) {
          ...
          button = gsearchtool_button_new_with_icon(_("Disable _Quick Search"),
                                                    "gtk-ok");
...
static void child_command_set_pgid_cb(gpointer data) {
  if (setpgid(0, 0) < 0) {
...
void spawn_search_command(GSearchWindow *gsearch, gchar *command) {
  GIOChannel *ioc_stdout;
  ...
  if (!g_shell_parse_argv(command, NULL, &argv, &error)) {
    ...
    gtk_message_dialog_format_secondary_text(
        GTK_MESSAGE_DIALOG(dialog), (error == NULL) ? " " : error->message,
        NULL);

    gtk_window_set_title(GTK_WINDOW(dialog), "");
    gtk_container_set_border_width(GTK_CONTAINER(dialog), 5);
    gtk_box_set_spacing(
        GTK_BOX(gtk_dialog_get_content_area(GTK_DIALOG(dialog))), 14);
  ...
  if (!g_spawn_async_with_pipes(g_get_home_dir(), argv, NULL,
                                G_SPAWN_SEARCH_PATH, child_command_set_pgid_cb,
                                NULL, &gsearch->command_details->command_pid,
                                NULL, &child_stdout, &child_stderr, &error)) {
    GtkWidget *dialog;
  ...
  if (gsearch->command_details->is_command_first_pass == TRUE) {
    ...
    gsearch->search_results_pixbuf_hash_table =
        g_hash_table_new_full(g_str_hash, g_str_equal, g_free, g_object_unref);
    gsearch->search_results_filename_hash_table =
        g_hash_table_new_full(g_str_hash, g_str_equal, g_free, NULL);

    /* Get the value of the caja date-format key if available. */
    if (gsearch->caja_schema_exists) {
      gsearch->search_results_date_format =
          g_settings_get_enum(gsearch->caja_settings, "date-format");
    } else {
      gsearch->search_results_date_format = CAJA_DATE_FORMAT_LOCALE;
    }

    gtk_tree_view_scroll_to_point(
        GTK_TREE_VIEW(gsearch->search_results_tree_view), 0, 0);
    gtk_tree_model_foreach(
        GTK_TREE_MODEL(gsearch->search_results_list_store),
...
static GtkWidget *create_constraint_box(GSearchWindow *gsearch,
                                        GSearchConstraint *opt, gchar *value) {
  GtkWidget *hbox;
  GtkWidget *label;
  GtkWidget *entry;
  ...
  switch (GSearchOptionTemplates[opt->constraint_id].type) {
    case SEARCH_CONSTRAINT_TYPE_BOOLEAN: {
      gchar *text = remove_mnemonic_character(
          GSearchOptionTemplates[opt->constraint_id].desc);
    ...
    case SEARCH_CONSTRAINT_TYPE_DATE_BEFORE:
    case SEARCH_CONSTRAINT_TYPE_DATE_AFTER: {
      gchar *desc = g_strconcat(
          LEFT_LABEL_SPACING,
          _(GSearchOptionTemplates[opt->constraint_id].desc), ":", NULL);
      ...
      if (GSearchOptionTemplates[opt->constraint_id].type ==
          SEARCH_CONSTRAINT_TYPE_TEXT) {
        entry = gtk_entry_new();
        if (value != NULL) {
      ...
      if (gsearch->is_window_accessible) {
        ...
        if (GSearchOptionTemplates[opt->constraint_id].units == NULL) {
          ...
          desc = g_strdup_printf(
              _("Enter a text value for the \"%s\" search option."), _(text));
        } else {
        ...
        */
          ...
          desc = g_strdup_printf(
              _("Enter a value in %s for the \"%s\" search option."),
              _(GSearchOptionTemplates[opt->constraint_id].units), _(text));
        }
      ...
      g_signal_connect(entry, "changed", G_CALLBACK(constraint_update_info_cb),
                       opt);

      g_signal_connect(entry, "activate", G_CALLBACK(constraint_activate_cb),
                       (gpointer)gsearch);
  ...
  button =
      GTK_WIDGET(g_object_new(GTK_TYPE_BUTTON, "label", "gtk-remove",
...

  {
    ...
    g_signal_connect(button, "clicked", G_CALLBACK(remove_constraint_cb),
                     (gpointer)list);
  }
  gtk_size_group_add_widget(gsearch->available_options_button_size_group,
                            button);
  ...
  if (gsearch->is_window_accessible) {
    ...
    gchar *desc = g_strdup_printf(
        _("Click to remove the \"%s\" search option."), _(text));
...
void add_constraint(GSearchWindow *gsearch, gint constraint_id, gchar *value,
  ...
  if (show_constraint) {
    if (gtk_widget_get_visible(gsearch->available_options_vbox) == FALSE) {
      gtk_expander_set_expanded(
          GTK_EXPANDER(gsearch->show_more_options_expander), TRUE);
  ...
  gtk_box_pack_start(GTK_BOX(gsearch->available_options_vbox), widget, FALSE,
                     FALSE, 0);
...
static void set_sensitive(GtkCellLayout *cell_layout, GtkCellRenderer *cell,
  ...
  g_object_set(cell, "sensitive", !(GSearchOptionTemplates[idx].is_selected),
               NULL);
}

static gboolean is_separator(GtkTreeModel *model, GtkTreeIter *iter,
                             gpointer data) {
  GtkTreePath *path;
  gint idx;
...
static void create_additional_constraint_section(GSearchWindow *gsearch) {
  ...
  gtk_box_pack_end(GTK_BOX(gsearch->available_options_vbox), hbox, FALSE, FALSE,
                   0);
  ...
  gtk_box_pack_start(GTK_BOX(hbox), gsearch->available_options_label, FALSE,
                     FALSE, 0);
  ...
  gtk_label_set_mnemonic_widget(
      GTK_LABEL(gsearch->available_options_label),
      GTK_WIDGET(gsearch->available_options_combo_box));
  gtk_combo_box_set_active(GTK_COMBO_BOX(gsearch->available_options_combo_box),
                           0);
  gtk_box_pack_start(GTK_BOX(hbox), gsearch->available_options_combo_box, TRUE,
                     TRUE, 0);

  renderer = gtk_cell_renderer_text_new();
  gtk_cell_layout_pack_start(
      GTK_CELL_LAYOUT(gsearch->available_options_combo_box), renderer, TRUE);
  gtk_cell_layout_set_attributes(
      GTK_CELL_LAYOUT(gsearch->available_options_combo_box), renderer, "text",
      0, NULL);
  gtk_cell_layout_set_cell_data_func(
      GTK_CELL_LAYOUT(gsearch->available_options_combo_box), renderer,
      set_sensitive, NULL, NULL);
  gtk_combo_box_set_row_separator_func(
      GTK_COMBO_BOX(gsearch->available_options_combo_box), is_separator, NULL,
      NULL);

  if (gsearch->is_window_accessible) {
    add_atk_namedesc(GTK_WIDGET(gsearch->available_options_combo_box),
                     _("Available options"),
                     _("Select a search option from the drop-down list."));
  }

  gsearch->available_options_add_button =
      GTK_WIDGET(g_object_new(GTK_TYPE_BUTTON, "label", "gtk-add", "use-stock",
                              TRUE, "use-underline", TRUE, NULL));

  gtk_widget_set_can_default(gsearch->available_options_add_button, FALSE);
  gsearch->available_options_button_size_group =
  ...
  if (gsearch->is_window_accessible) {
    add_atk_namedesc(GTK_WIDGET(gsearch->available_options_add_button),
                     _("Add search option"),
                     _("Click to add the selected available search option."));
  }

  gtk_box_pack_end(GTK_BOX(hbox), gsearch->available_options_add_button, FALSE,
                   FALSE, 0);
}

static void filename_cell_data_func(GtkTreeViewColumn *column,
                                    GtkCellRenderer *renderer,
  ...
  if (gsearch->is_search_results_single_click_to_activate == TRUE) {
    ...
    if ((gsearch->search_results_hover_path == NULL) ||
        (gtk_tree_path_compare(path, gsearch->search_results_hover_path) !=
         0)) {
  ...
  g_object_set(gsearch->search_results_name_cell_renderer, "underline",
               underline, "underline-set", underline_set, NULL);
}

static gboolean gsearch_equal_func(GtkTreeModel *model, gint column,
  ...
  if (name != NULL) {
    ...
    if ((casefold_key != NULL) && (casefold_name != NULL) &&
...
static GtkWidget *create_search_results_section(GSearchWindow *gsearch) {
  GtkWidget *label;
  GtkWidget *vbox;
  GtkWidget *hbox;
  GtkWidget *window;
  ...
  gtk_widget_set_size_request(gsearch->progress_spinner, GTK_ICON_SIZE_MENU,
                              GTK_ICON_SIZE_MENU);
  ...
  gsearch->search_results_list_store = gtk_list_store_new(
      NUM_COLUMNS, GDK_TYPE_PIXBUF, G_TYPE_STRING, G_TYPE_STRING, G_TYPE_STRING,
      G_TYPE_STRING, G_TYPE_DOUBLE, G_TYPE_STRING, G_TYPE_STRING, G_TYPE_DOUBLE,
      G_TYPE_POINTER, G_TYPE_BOOLEAN);

  gsearch->search_results_tree_view =
  ...
  if (gsearch->is_window_accessible) {
    add_atk_namedesc(GTK_WIDGET(gsearch->search_results_tree_view),
                     _("List View"), NULL);
  }

  gsearch->search_results_selection = gtk_tree_view_get_selection(
      GTK_TREE_VIEW(gsearch->search_results_tree_view));

  gtk_tree_selection_set_mode(
      GTK_TREE_SELECTION(gsearch->search_results_selection),
      GTK_SELECTION_MULTIPLE);

  gtk_drag_source_set(GTK_WIDGET(gsearch->search_results_tree_view),
                      GDK_BUTTON1_MASK | GDK_BUTTON2_MASK, GSearchDndTable,
                      GSearchTotalDnds,
                      GDK_ACTION_COPY | GDK_ACTION_MOVE | GDK_ACTION_ASK);

  g_signal_connect(gsearch->search_results_tree_view, "drag_data_get",
                   G_CALLBACK(drag_file_cb), (gpointer)gsearch);

  g_signal_connect(gsearch->search_results_tree_view, "drag_begin",
                   G_CALLBACK(drag_begin_file_cb), (gpointer)gsearch);

  g_signal_connect(gsearch->search_results_tree_view, "event_after",
                   G_CALLBACK(file_event_after_cb), (gpointer)gsearch);

  g_signal_connect(gsearch->search_results_tree_view, "button_release_event",
                   G_CALLBACK(file_button_release_event_cb), (gpointer)gsearch);
  ...
  g_signal_connect(gsearch->search_results_tree_view, "key_press_event",
                   G_CALLBACK(file_key_press_event_cb), (gpointer)gsearch);

  g_signal_connect(gsearch->search_results_tree_view, "motion_notify_event",
                   G_CALLBACK(file_motion_notify_cb), (gpointer)gsearch);

  g_signal_connect(gsearch->search_results_tree_view, "leave_notify_event",
                   G_CALLBACK(file_leave_notify_cb), (gpointer)gsearch);

  gtk_label_set_mnemonic_widget(GTK_LABEL(label),
                                GTK_WIDGET(gsearch->search_results_tree_view));
  ...
  gtk_tree_view_column_set_attributes(column, renderer, "pixbuf", COLUMN_ICON,
                                      NULL);

  gsearch->search_results_name_cell_renderer = gtk_cell_renderer_text_new();
  gtk_tree_view_column_pack_start(
      column, gsearch->search_results_name_cell_renderer, TRUE);
  gtk_tree_view_column_set_attributes(
      column, gsearch->search_results_name_cell_renderer, "text", COLUMN_NAME,
      NULL);
  ...
  gtk_tree_view_column_set_cell_data_func(
      column, renderer, (GtkTreeCellDataFunc)filename_cell_data_func, gsearch,
      NULL);
  gtk_tree_view_append_column(GTK_TREE_VIEW(gsearch->search_results_tree_view),
                              column);

  /* create the folder column */
  renderer = gtk_cell_renderer_text_new();
  column = gtk_tree_view_column_new_with_attributes(
      _("Folder"), renderer, "text", COLUMN_RELATIVE_PATH, NULL);
  gtk_tree_view_column_set_sizing(column, GTK_TREE_VIEW_COLUMN_AUTOSIZE);
  gtk_tree_view_column_set_resizable(column, TRUE);
  ...
  column = gtk_tree_view_column_new_with_attributes(_("Size"), renderer, "text",
                                                    COLUMN_READABLE_SIZE, NULL);
  gtk_tree_view_column_set_sizing(column, GTK_TREE_VIEW_COLUMN_AUTOSIZE);
  gtk_tree_view_column_set_resizable(column, TRUE);
  ...
  renderer = gtk_cell_renderer_text_new();
  column = gtk_tree_view_column_new_with_attributes(_("Type"), renderer, "text",
                                                    COLUMN_TYPE, NULL);
  gtk_tree_view_column_set_sizing(column, GTK_TREE_VIEW_COLUMN_AUTOSIZE);
  gtk_tree_view_column_set_resizable(column, TRUE);
  ...
  renderer = gtk_cell_renderer_text_new();
  column = gtk_tree_view_column_new_with_attributes(
      _("Date Modified"), renderer, "text", COLUMN_READABLE_DATE, NULL);
  gtk_tree_view_column_set_sizing(column, GTK_TREE_VIEW_COLUMN_AUTOSIZE);
  gtk_tree_view_column_set_resizable(column, TRUE);
  ...
  g_signal_connect(gsearch->search_results_tree_view, "columns-changed",
                   G_CALLBACK(columns_changed_cb), (gpointer)gsearch);
  return vbox;
}

static void register_gsearchtool_icon(GtkIconFactory *factory) {
...
static void gsearchtool_init_stock_icons(void) {
...
void set_clone_command(GSearchWindow *gsearch, gint *argcp, gchar ***argvp,
                       gpointer client_data, gboolean escape_values) {
  gchar **argv;
  gchar *file_is_named_utf8;
  gchar *file_is_named_locale;
  gchar *look_in_folder_locale;
  ...
  file_is_named_utf8 =
      (gchar *)gtk_entry_get_text(GTK_ENTRY(gsearch_history_entry_get_entry(
          GSEARCH_HISTORY_ENTRY(gsearch->name_contains_entry))));
  file_is_named_locale =
      g_locale_from_utf8(file_is_named_utf8 != NULL ? file_is_named_utf8 : "",
                         -1, NULL, NULL, NULL);
  if (escape_values)
  ...
  look_in_folder_locale = gtk_file_chooser_get_current_folder(
      GTK_FILE_CHOOSER(gsearch->look_in_folder_button));
  ...
  if (gtk_widget_get_visible(gsearch->available_options_vbox)) {
    for (list = gsearch->available_options_selected_list; list != NULL;
      ...
      switch (constraint->constraint_id) {
        case SEARCH_CONSTRAINT_CONTAINS_THE_TEXT:
          locale =
              g_locale_from_utf8(constraint->data.text, -1, NULL, NULL, NULL);
          if (escape_values)
...
static void handle_gsettings_settings(GSearchWindow *gsearch) {
  if (g_settings_get_boolean(gsearch->mate_search_tool_settings,
  ...
  if (g_settings_get_boolean(gsearch->mate_search_tool_select_settings,
                             "contains-the-text")) {
...
static void gsearch_window_size_allocate(GtkWidget *widget,
...
static GtkWidget *gsearch_app_create(GSearchWindow *gsearch) {
  gchar *locale_string;
  gchar *utf8_string;
  GtkWidget *hbox;
  GtkWidget *vbox;
  ...
  gsearch->mate_search_tool_select_settings =
      g_settings_new("org.mate.search-tool.select");
  gsearch->mate_desktop_interface_settings =
      g_settings_new("org.mate.interface");

  /* Check if caja schema is installed before trying to read caja settings */
  gsearch->caja_schema_exists = FALSE;
  GSettingsSchema *schema = g_settings_schema_source_lookup(
      g_settings_schema_source_get_default(), CAJA_PREFERENCES_SCHEMA, FALSE);
  ...
  gsearch->is_window_maximized = g_settings_get_boolean(
      gsearch->mate_search_tool_settings, "default-window-maximized");
  g_signal_connect(gsearch->window, "size-allocate",
                   G_CALLBACK(gsearch_window_size_allocate), gsearch);
  ...
  gtk_window_set_default_size(GTK_WINDOW(gsearch->window),
                              gsearch->window_width, gsearch->window_height);
  ...
  gsearch->name_contains_entry =
      gsearch_history_entry_new("gsearchtool-file-entry", FALSE);
  gtk_label_set_mnemonic_widget(GTK_LABEL(label), gsearch->name_contains_entry);
  gsearch_history_entry_set_history_length(
      GSEARCH_HISTORY_ENTRY(gsearch->name_contains_entry), 10);
  gtk_widget_set_hexpand(gsearch->name_contains_entry, TRUE);
  gtk_grid_attach(GTK_GRID(gsearch->name_and_folder_table),
                  gsearch->name_contains_entry, 1, 0, 1, 1);
  entry = gsearch_history_entry_get_entry(
      GSEARCH_HISTORY_ENTRY(gsearch->name_contains_entry));

  if (GTK_IS_ACCESSIBLE(
          gtk_widget_get_accessible(gsearch->name_contains_entry))) {
    gsearch->is_window_accessible = TRUE;
    add_atk_namedesc(
        gsearch->name_contains_entry, NULL,
        _("Enter a filename or partial filename with or without wildcards."));
    add_atk_namedesc(
        entry, _("Name contains"),
        _("Enter a filename or partial filename with or without wildcards."));
  }
  g_signal_connect(gsearch_history_entry_get_entry(
                       GSEARCH_HISTORY_ENTRY(gsearch->name_contains_entry)),
                   "activate", G_CALLBACK(name_contains_activate_cb),
                   (gpointer)gsearch);
  ...
  gsearch->look_in_folder_button = gtk_file_chooser_button_new(
      _("Browse"), GTK_FILE_CHOOSER_ACTION_SELECT_FOLDER);
  gtk_label_set_mnemonic_widget(GTK_LABEL(label),
                                GTK_WIDGET(gsearch->look_in_folder_button));
  gtk_widget_set_hexpand(gsearch->look_in_folder_button, TRUE);
  gtk_grid_attach(GTK_GRID(gsearch->name_and_folder_table),
                  gsearch->look_in_folder_button, 1, 1, 1, 1);

  g_signal_connect(gsearch->look_in_folder_button, "current-folder-changed",
                   G_CALLBACK(look_in_folder_changed_cb), (gpointer)gsearch);

  if (gsearch->is_window_accessible) {
    add_atk_namedesc(GTK_WIDGET(gsearch->look_in_folder_button),
                     _("Look in folder"),
                     _("Select the folder or device from which you want to "
                       "begin the search."));
  }

  locale_string = g_settings_get_string(gsearch->mate_search_tool_settings,
                                        "look-in-folder");
  ...
  gtk_file_chooser_set_current_folder(
      GTK_FILE_CHOOSER(gsearch->look_in_folder_button), utf8_string);

  g_free(locale_string);
  g_free(utf8_string);

  gsearch->show_more_options_expander =
      gtk_expander_new_with_mnemonic(_("Select more _options"));
  gtk_box_pack_start(GTK_BOX(container), gsearch->show_more_options_expander,
                     FALSE, FALSE, 0);
  ...
  gtk_box_pack_start(GTK_BOX(container),
                     GTK_WIDGET(gsearch->available_options_vbox), FALSE, FALSE,
                     0);

  if (gsearch->is_window_accessible) {
    add_atk_namedesc(
        GTK_WIDGET(gsearch->show_more_options_expander),
        _("Select more options"),
        _("Click to expand or collapse the list of available options."));
    add_atk_relation(GTK_WIDGET(gsearch->available_options_vbox),
                     GTK_WIDGET(gsearch->show_more_options_expander),
                     ATK_RELATION_CONTROLLED_BY);
    add_atk_relation(GTK_WIDGET(gsearch->show_more_options_expander),
                     GTK_WIDGET(gsearch->available_options_vbox),
  ...
  gtk_box_pack_start(GTK_BOX(vbox), gsearch->search_results_vbox, TRUE, TRUE,
                     0);
  ...
  button =
      GTK_WIDGET(g_object_new(GTK_TYPE_BUTTON, "label", "gtk-help", "use-stock",
                              TRUE, "use-underline", TRUE, NULL));

  gtk_widget_set_can_default(button, TRUE);
  gtk_box_pack_start(GTK_BOX(hbox), button, FALSE, FALSE, 0);
  gtk_button_box_set_child_secondary(GTK_BUTTON_BOX(hbox), button, TRUE);
  g_signal_connect(button, "clicked", G_CALLBACK(click_help_cb),
                   (gpointer)gsearch->window);
  if (gsearch->is_window_accessible) {
    add_atk_namedesc(GTK_WIDGET(button), NULL,
                     _("Click to display the help manual."));
  }
  ...
  gtk_widget_set_can_default(button, TRUE);
  g_signal_connect(button, "clicked", G_CALLBACK(click_close_cb),
                   (gpointer)gsearch);
  if (gsearch->is_window_accessible) {
    add_atk_namedesc(GTK_WIDGET(button), NULL,
                     _("Click to close \"Search for Files\"."));
  }
  ...
  gsearch->stop_button =
      GTK_WIDGET(g_object_new(GTK_TYPE_BUTTON, "label", "gtk-stop", "use-stock",
                              TRUE, "use-underline", TRUE, NULL));
  ...
  g_signal_connect(gsearch->find_button, "clicked", G_CALLBACK(click_find_cb),
                   (gpointer)gsearch);
  g_signal_connect(gsearch->find_button, "size_allocate",
                   G_CALLBACK(size_allocate_cb),
                   (gpointer)gsearch->available_options_add_button);
  g_signal_connect(gsearch->stop_button, "clicked", G_CALLBACK(click_stop_cb),
                   (gpointer)gsearch);

  if (gsearch->is_window_accessible) {
    add_atk_namedesc(GTK_WIDGET(gsearch->find_button), NULL,
                     _("Click to perform a search."));
    add_atk_namedesc(GTK_WIDGET(gsearch->stop_button), NULL,
                     _("Click to stop a search."));
  }
  ...
  gtk_window_set_focus(
      ...
      GTK_WIDGET(gsearch_history_entry_get_entry(
          GSEARCH_HISTORY_ENTRY(gsearch->name_contains_entry))));
...
static void gsearch_window_finalize(GObject *object) {
  parent_class->finalize(object);
}

static void gsearch_window_class_init(GSearchWindowClass *klass, void *dara) {
...
GType gsearch_window_get_type(void) {
  ...
  if (!object_type) {
    static const GTypeInfo object_info = {
        ...
        NULL};
    object_type = g_type_register_static(GTK_TYPE_WINDOW, "GSearchWindow",
                                         &object_info, 0);
  }
  return object_type;
}

static void gsearchtool_setup_gsettings_notifications(GSearchWindow *gsearch)

{
  ...
  g_signal_connect(gsearch->caja_settings, "changed::click-policy",
   ...
   */
  click_to_activate_pref =
      g_settings_get_string(gsearch->caja_settings, "click-policy");
...
int main(int argc, char *argv[]) {
  ...
  g_option_context_add_main_entries(context, GSearchGOptionEntries,
                                    GETTEXT_PACKAGE);
  ...
  g_signal_connect(gsearch->window, "delete_event", G_CALLBACK(quit_cb),
                   (gpointer)gsearch);
  g_signal_connect(gsearch->window, "key_press_event", G_CALLBACK(key_press_cb),
                   (gpointer)gsearch);
  g_signal_connect(gsearch->window, "window_state_event",
                   G_CALLBACK(window_state_event_cb), (gpointer)gsearch);

  if ((client = egg_sm_client_get()) != NULL) {
    g_signal_connect(client, "save_state", G_CALLBACK(save_session_cb),
                     (gpointer)gsearch);
    g_signal_connect(client, "quit", G_CALLBACK(quit_session_cb),
                     (gpointer)gsearch);
  }

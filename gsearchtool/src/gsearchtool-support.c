...
GtkTreeViewColumn *gsearchtool_gtk_tree_view_get_column_with_sort_column_id(
    GtkTreeView *treeview, gint id);
...
gboolean is_path_hidden(const gchar *path) {
  ...
  if (sub_str != NULL) {
    ...
    mate_desktop_str = g_strconcat(G_DIR_SEPARATOR_S, ".mate-desktop",
    ...
    if (strncmp(sub_str, mate_desktop_str, strlen(mate_desktop_str)) == 0) {
      sub_str++;
      results =
          (g_strstr_len(sub_str, strlen(sub_str), hidden_path_substr) != NULL);
    } else {
      results = TRUE;
    }
...
gboolean is_quick_search_excluded_path(const gchar *path) {
  ...
  exclude_path_list =
      g_settings_get_strv(settings, "quick-search-excluded-paths");
...
gboolean is_second_scan_excluded_path(const gchar *path) {
  ...
  /* Check path against the Quick-Search-Excluded-Paths list. */
  exclude_path_list =
      g_settings_get_strv(settings, "quick-search-second-scan-excluded-paths");
...
static gint count_of_char_in_string(const gchar *string, const gchar c) {
  int cnt = 0;
  for (; *string; string++) {
    if (*string == c) cnt++;
  }
...
gchar *get_file_type_description(const gchar *file, GFileInfo *file_info) {
  ...
  if (g_file_info_get_is_symlink(file_info) == TRUE) {
    ...
    if (g_path_is_absolute(symlink_target) != TRUE) {
      ...
      absolute_symlink =
          g_strconcat(dirname, G_DIR_SEPARATOR_S, symlink_target, NULL);
      g_free(dirname);
    } else {
    ...
    str =
        g_strdup_printf(_("link to %s"), (desc != NULL) ? desc : content_type);
    g_free(absolute_symlink);
    g_free(desc);
    return str;
  }
...
static gchar *gsearchtool_pixmap_file(const gchar *partial_path) {
  ...
  path =
      g_build_filename(DATADIR "/pixmaps/mate-search-tool", partial_path, NULL);
...
static void gsearchtool_draw_frame_row(GdkPixbuf *frame_image,
                                       gint target_width, gint source_width,
  ...
  while (remaining_width > 0) {
    slab_width =
        remaining_width > source_width ? source_width : remaining_width;
    gdk_pixbuf_copy_area(frame_image, left_offset, source_v_position,
                         slab_width, height, result_pixbuf,
...
static void gsearchtool_draw_frame_column(
    GdkPixbuf *frame_image, gint target_height, gint source_height,
  ...
  while (remaining_height > 0) {
    slab_height =
        remaining_height > source_height ? source_height : remaining_height;
    gdk_pixbuf_copy_area(frame_image, source_h_position, top_offset, width,
                         slab_height, result_pixbuf, dest_h_position,
                         top_offset + v_offset);
...
static GdkPixbuf *gsearchtool_stretch_frame_image(
    GdkPixbuf *frame_image, gint left_offset, gint top_offset,
    gint right_offset, gint bottom_offset, gint dest_width, gint dest_height,
    gboolean fill_flag) {
  GdkPixbuf *result_pixbuf;
  ...
  if (fill_flag) {
    result_pixbuf = gdk_pixbuf_scale_simple(frame_image, dest_width,
                                            dest_height, GDK_INTERP_NEAREST);
  } else {
    result_pixbuf =
        gdk_pixbuf_new(GDK_COLORSPACE_RGB, TRUE, 8, dest_width, dest_height);
  }
  ...
  gdk_pixbuf_copy_area(frame_image, 0, 0, left_offset, top_offset,
                       result_pixbuf, 0, 0);
  gsearchtool_draw_frame_row(frame_image, target_width, target_frame_width, 0,
                             0, result_pixbuf, left_offset, top_offset);

  /* Draw the right top corner and left column */
  gdk_pixbuf_copy_area(frame_image, frame_width - right_offset, 0, right_offset,
                       top_offset, result_pixbuf, dest_width - right_offset, 0);
  gsearchtool_draw_frame_column(frame_image, target_height, target_frame_height,
                                0, 0, result_pixbuf, top_offset, left_offset);

  /* Draw the bottom right corner and bottom row */
  gdk_pixbuf_copy_area(frame_image, frame_width - right_offset,
                       frame_height - bottom_offset, right_offset,
                       bottom_offset, result_pixbuf, dest_width - right_offset,
  ...
  gsearchtool_draw_frame_row(frame_image, target_width, target_frame_width,
                             ...
                             dest_height - bottom_offset, result_pixbuf,
                             left_offset, bottom_offset);

  /* Draw the bottom left corner and the right column */
  gdk_pixbuf_copy_area(frame_image, 0, frame_height - bottom_offset,
                       left_offset, bottom_offset, result_pixbuf, 0,
                       dest_height - bottom_offset);
  gsearchtool_draw_frame_column(frame_image, target_height, target_frame_height,
                                frame_width - right_offset,
                                dest_width - right_offset, result_pixbuf,
                                top_offset, right_offset);
  return result_pixbuf;
}

static GdkPixbuf *gsearchtool_embed_image_in_frame(
    GdkPixbuf *source_image, GdkPixbuf *frame_image, gint left_offset,
    gint top_offset, gint right_offset, gint bottom_offset) {
  GdkPixbuf *result_pixbuf;
  ...
  result_pixbuf = gsearchtool_stretch_frame_image(
      frame_image, left_offset, top_offset, right_offset, bottom_offset,
      dest_width, dest_height, FALSE);

  gdk_pixbuf_copy_area(source_image, 0, 0, source_width, source_height,
                       result_pixbuf, left_offset, top_offset);

  return result_pixbuf;
}

static void gsearchtool_thumbnail_frame_image(GdkPixbuf **pixbuf) {
  ...
  pixbuf_with_frame =
      gsearchtool_embed_image_in_frame(*pixbuf, frame, 3, 3, 6, 6);
...
static GdkPixbuf *gsearchtool_get_thumbnail_image(const gchar *thumbnail) {
  GdkPixbuf *pixbuf = NULL;
  ...
  if (thumbnail != NULL) {
    if (g_file_test(thumbnail, G_FILE_TEST_EXISTS)) {
      ...
      pixbuf = gdk_pixbuf_scale_simple(thumbnail_pixbuf, scale_x, scale_y,
                                       GDK_INTERP_BILINEAR);
...
static GdkPixbuf *get_themed_icon_pixbuf(GThemedIcon *icon, int size,
  ...
  icon_info = gtk_icon_theme_choose_icon(icon_theme, (const char **)icon_names,
                                         size, 0);
  if (icon_info == NULL) {
...
GdkPixbuf *get_file_pixbuf(GSearchWindow *gsearch, GFileInfo *file_info) {
  ...
  if (thumbnail_path != NULL) {
    ...
    pixbuf = (GdkPixbuf *)g_hash_table_lookup(
        gsearch->search_results_filename_hash_table, icon_string);

    if (pixbuf == NULL) {
      pixbuf = get_themed_icon_pixbuf(G_THEMED_ICON(icon), ICON_SIZE,
                                      gtk_icon_theme_get_default());
      g_hash_table_insert(gsearch->search_results_filename_hash_table,
                          g_strdup(icon_string), pixbuf);
    }
    g_free(icon_string);
  }
  return pixbuf;
...
gboolean open_file_with_filemanager(GtkWidget *window, const gchar *file) {
  ...
  if (strcmp(g_app_info_get_executable(g_app_info), "caja") == 0) {
    command = g_strconcat("caja ", "--sm-disable ", "--no-desktop ",
                          "--no-default-window ", NULL);
  } else {
    command = g_strconcat(g_app_info_get_executable(g_app_info), " ", NULL);
  }
  ...
  g_key_file_load_from_data(key_file, contents, strlen(contents),
  ...
  if (d_app_info != NULL) {
    ...
    result = g_app_info_launch_uris(G_APP_INFO(d_app_info), list,
                                    G_APP_LAUNCH_CONTEXT(ctx), NULL);
  } else {
    result = FALSE;
  }
...
gboolean open_file_with_application(GtkWidget *window, const gchar *file,
  ...
  if (app == NULL) {
    ...
    uri = g_filename_to_uri(file, NULL, NULL);
    result = g_app_info_launch_default_for_uri(
        uri, (GAppLaunchContext *)context, NULL);
    g_free(uri);
  } else {
    ...
    if (g_file == NULL) {
      ...
      result = g_app_info_launch(app, g_file_list, (GAppLaunchContext *)context,
                                 NULL);
...
gchar *gsearchtool_get_unique_filename(const gchar *path, const gchar *suffix) {
  ...
  gchar *words[] = {"foo",     "bar",    "blah",   "cranston", "frobate",
                    "hadjaha", "greasy", "hammer", "eek",      "larry",
                    "curly",   "moe",    NULL};
  gchar *retval = NULL;
  ...
  while (exists) {
    ...
    file = g_strdup_printf("%s-%010x%s", words[word], (guint)rnd, suffix);
  ...
  if (value) {
    ...
    while ((item = g_variant_iter_next_value(iter))) {
      ...
      if (id >= 0 && id < NUM_COLUMNS) {
        cur = gsearchtool_gtk_tree_view_get_column_with_sort_column_id(treeview,
                                                                       id);
...
static const char untranslated_another_copy_duplicate_tag[] =
    N_(" (another copy)");
...
static const char untranslated_first_copy_duplicate_format[] =
    N_("%s (copy)%s");
/* localizers: appended to second file copy */
static const char untranslated_second_copy_duplicate_format[] =
    N_("%s (another copy)%s");

/* localizers: appended to x11th file copy */
static const char untranslated_x11th_copy_duplicate_format[] =
    N_("%s (%dth copy)%s");
/* localizers: appended to x12th file copy */
static const char untranslated_x12th_copy_duplicate_format[] =
    N_("%s (%dth copy)%s");
/* localizers: appended to x13th file copy */
static const char untranslated_x13th_copy_duplicate_format[] =
    N_("%s (%dth copy)%s");

/* localizers: appended to x1st file copy */
static const char untranslated_st_copy_duplicate_format[] =
    N_("%s (%dst copy)%s");
/* localizers: appended to x2nd file copy */
static const char untranslated_nd_copy_duplicate_format[] =
    N_("%s (%dnd copy)%s");
/* localizers: appended to x3rd file copy */
static const char untranslated_rd_copy_duplicate_format[] =
    N_("%s (%drd copy)%s");
/* localizers: appended to xxth file copy */
static const char untranslated_th_copy_duplicate_format[] =
    N_("%s (%dth copy)%s");

#define FIRST_COPY_DUPLICATE_FORMAT _(untranslated_first_copy_duplicate_format)
#define SECOND_COPY_DUPLICATE_FORMAT \
  _(untranslated_second_copy_duplicate_format)
...
static gchar *extract_string_until(const gchar *original,
...
/* Dismantle a file name, separating the base name, the file suffix and removing
 * any (xxxcopy), etc. string. Figure out the count that corresponds to the
 * given (xxxcopy) substring.
 */
static void parse_previous_duplicate_name(const gchar *name, gchar **name_base,
                                          const gchar **suffix, gint *count) {
  const gchar *tag;
...
static gchar *make_next_duplicate_name(const gchar *base, const gchar *suffix,
                                       gint count) {
  const gchar *format;
  gchar *result;
...
static gchar *get_duplicate_name(const gchar *name) {
  const gchar *suffix;
  gchar *name_base;
  gchar *result;
  gint count;
...
gchar *gsearchtool_get_next_duplicate_name(const gchar *basename) {
  gchar *utf8_name;
  gchar *utf8_result;
  gchar *result;

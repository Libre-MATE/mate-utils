...
static void baobab_treemap_draw_rectangle(GtkWidget *chart, cairo_t *cr,
                                          gdouble x, gdouble y, gdouble width,
                                          gdouble height,
                                          BaobabChartColor fill_color,
                                          const char *text, gboolean show_text);
static void baobab_treemap_draw_item(GtkWidget *chart, cairo_t *cr,
...
static void baobab_treemap_init(BaobabTreemap *chart) {
...
static void baobab_treemap_draw_rectangle(GtkWidget *chart, cairo_t *cr,
  ...
  cairo_rectangle(cr, x + border, y + border, width - border * 2,
                  height - border * 2);
...
static void baobab_treemap_draw_item(GtkWidget *chart, cairo_t *cr,
  ...
  baobab_treemap_draw_rectangle(chart, cr, rect->x, rect->y, width, height,
                                fill_color, item->name,
...
static void baobab_treemap_calculate_item_geometry(GtkWidget *chart,
  ...
  if (item->data == NULL) item->data = g_new(cairo_rectangle_t, 1);
  ...
  if (parent != NULL) parent->has_visible_children = TRUE;

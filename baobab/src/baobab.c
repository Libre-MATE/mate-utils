...
#define GET_TOGGLE_ACTION(x) \
  (GTK_TOGGLE_ACTION(gtk_builder_get_object(baobab.main_ui, (x))))
...
static void set_drop_target(GtkWidget *target, gboolean active) {
  if (active) {
    gtk_drag_dest_set(GTK_WIDGET(target),
                      GTK_DEST_DEFAULT_DROP | GTK_DEST_DEFAULT_MOTION |
                          GTK_DEST_DEFAULT_HIGHLIGHT,
                      dnd_target_list, G_N_ELEMENTS(dnd_target_list),
                      GDK_ACTION_COPY);
  } else {
...
static void check_menu_sens(gboolean scanning) {
  ...
  gtk_action_set_sensitive(GET_ACTION("menurescan"),
  ...
  gtk_action_set_sensitive(GET_ACTION("ck_allocated"),
...
static void update_scan_label(void) {
  ...
  markup =
      g_markup_printf_escaped("<small>%s <b>%s</b> (%s %s %s %s)</small>",
                              _("Total filesystem capacity:"), total,
                              _("used:"), used, _("available:"), available);
...
void baobab_scan_location(GFile *file) {
  GtkToggleAction *ck_allocated;

  if (!baobab_check_dir(file)) return;

  if (iterstack != NULL) return;

  if (baobab.current_location) g_object_unref(baobab.current_location);
 ...
 */
static void prefill_model(struct chan_data *data) {
  ...
  if (currentdepth == -1) {
    ...
    gtk_tree_view_expand_row(GTK_TREE_VIEW(baobab.tree_view), path, FALSE);
    gtk_tree_path_free(path);
  } else if (data->depth > currentdepth) {
    gtk_tree_store_append(baobab.model, &iter, &currentiter);
  } else if (data->depth == currentdepth) {
    gtk_tree_model_iter_parent((GtkTreeModel *)baobab.model, &iterparent,
                               &currentiter);
    ...
    for (i = 0; i <= (currentdepth - data->depth); i++) {
      gtk_tree_model_iter_parent((GtkTreeModel *)baobab.model, &tempiter,
                                 &iter);
      iter = tempiter;
    }
  ...
  gtk_tree_store_set(baobab.model, &iter, COL_DIR_NAME, name, COL_H_PARSENAME,
                     "", COL_H_ELEMENTS, -1, COL_H_PERC, -1.0, COL_DIR_SIZE,
                     str, COL_ELEMENTS, str, -1);
...
static void first_row(void) {
  ...
  gtk_tree_store_set(baobab.model, &root_iter, COL_DIR_NAME, capacity_label,
                     COL_H_PARSENAME, "", COL_H_PERC, 100.0, COL_DIR_SIZE,
                     capacity_size, COL_H_SIZE, baobab.fs.total,
                     COL_H_ALLOCSIZE, baobab.fs.total, COL_H_ELEMENTS, -1, -1);
  ...
  gtk_tree_store_set(baobab.model, &firstiter, COL_DIR_NAME, label,
                     COL_H_PARSENAME, "", COL_H_PERC, perc, COL_DIR_SIZE, size,
                     COL_H_SIZE, baobab.fs.used, COL_H_ALLOCSIZE,
                     baobab.fs.used, COL_H_ELEMENTS, -1, -1);
...
void baobab_fill_model(struct chan_data *data) {
  GtkTreeIter iter;
  GString *hardlinks;
  GString *elements;
  char *name;
  ...
  g_string_printf(elements, ngettext("%5d item", "%5d items", data->elements),
                  data->elements);
  ...
  gtk_tree_store_set(baobab.model, &iter, COL_DIR_NAME, name, COL_H_PARSENAME,
                     data->parse_name, COL_H_PERC, -1.0, COL_DIR_SIZE,
                     baobab.show_allocated ? alloc_size : size, COL_H_SIZE,
                     data->size, COL_ELEMENTS, elements->str, COL_H_ELEMENTS,
                     data->elements, COL_HARDLINK, hardlinks->str,
                     COL_H_HARDLINK, data->tempHLsize, COL_H_ALLOCSIZE,
                     data->alloc_size, -1);
...
void push_iter_in_stack(GtkTreeIter *iter) {
...
GtkTreeIter pop_iter_from_stack(void) {
  GtkTreeIter iter;
...
gboolean baobab_is_excluded_location(GFile *file) {
  gboolean ret = FALSE;
  GSList *l;
...
static void volume_changed(GVolumeMonitor *volume_monitor, GVolume *volume,
                           gpointer user_data) {
  /* filesystem has changed (mounted or unmounted device) */
  baobab_update_filesystem();
}

static void home_contents_changed(GFileMonitor *file_monitor, GFile *child,
                                  GFile *other_file,
...

{
  ...
  if (baobab.CONTENTS_CHANGED_DELAYED) return;
...
static void monitor_volume(void) {
...
static void monitor_home(gboolean enable) {
  if (enable && baobab.monitor_home == NULL) {
    ...
    if (!baobab.monitor_home) {
      ...
      g_signal_connect(baobab.monitor_home, "changed",
                       G_CALLBACK(home_contents_changed), NULL);
    }
...
void baobab_set_statusbar(const gchar *text) {
...
static void toolbar_reconfigured_cb(GtkToolItem *item, GtkWidget *spinner) {
...
static void baobab_create_toolbar(void) {
  GtkWidget *toolbar;
  GtkToolItem *item;
  ...
  g_settings_bind(baobab.ui_settings, BAOBAB_SETTINGS_TOOLBAR_VISIBLE,
                  baobab.toolbar, "visible", G_SETTINGS_BIND_DEFAULT);
  g_settings_bind(baobab.ui_settings, BAOBAB_SETTINGS_TOOLBAR_VISIBLE,
                  GET_TOGGLE_ACTION("view_tb"), "active",
                  G_SETTINGS_BIND_DEFAULT);
}

static void baobab_create_statusbar(void) {
  ...
  g_settings_bind(baobab.ui_settings, BAOBAB_SETTINGS_STATUSBAR_VISIBLE,
                  baobab.statusbar, "visible", G_SETTINGS_BIND_DEFAULT);
  g_settings_bind(baobab.ui_settings, BAOBAB_SETTINGS_STATUSBAR_VISIBLE,
                  GET_TOGGLE_ACTION("view_sb"), "active",
                  G_SETTINGS_BIND_DEFAULT);
}

static void baobab_settings_subfoldertips_changed(GSettings *settings,
                                                  const gchar *key,
                                                  gpointer user_data) {
  gboolean visible;
...
static void baobab_set_excluded_locations(gchar **uris) {
  gint i;

  g_slist_free_full(baobab.excluded_locations, g_object_unref);
  baobab.excluded_locations = NULL;
  for (i = 0; uris[i] != NULL; ++i) {
    baobab.excluded_locations =
        g_slist_prepend(baobab.excluded_locations, g_file_new_for_uri(uris[i]));
  }
}

static void store_excluded_locations(void) {
  GSList *l;
  GPtrArray *uris;
...
static void sanity_check_excluded_locations(void) {
  GFile *root;
  GSList *l;
  ...
  for (l = baobab.excluded_locations; l != NULL; l = l->next) {
    if (g_file_equal(l->data, root)) {
      baobab.excluded_locations =
          g_slist_delete_link(baobab.excluded_locations, l);
...
static void excluded_uris_changed(GSettings *settings, const gchar *key,
                                  gpointer user_data) {
  gchar **uris;
...
static void baobab_setup_excluded_locations(void) {
  gchar **uris;

  g_signal_connect(baobab.prefs_settings,
                   "changed::" BAOBAB_SETTINGS_EXCLUDED_URIS,
                   G_CALLBACK(excluded_uris_changed), NULL);

  uris =
      g_settings_get_strv(baobab.prefs_settings, BAOBAB_SETTINGS_EXCLUDED_URIS);
  baobab_set_excluded_locations(uris);
  g_strfreev(uris);
...
static void baobab_settings_monitor_home_changed(GSettings *settings,
                                                 const gchar *key,
                                                 gpointer user_data) {
  gboolean enable;
...
static void baobab_setup_monitors(void) {
  gboolean enable;
  ...
  g_signal_connect(baobab.prefs_settings,
                   ...
                   G_CALLBACK(baobab_settings_monitor_home_changed), NULL);
...
static void baobab_init(void) {
  ...
  if (gtk_builder_add_from_resource(baobab.main_ui, BAOBAB_UI_RESOURCE,
                                    &error) == 0) {
  ...
  g_signal_connect(baobab.ui_settings,
                   "changed::" BAOBAB_SETTINGS_SUBFLSTIPS_VISIBLE,
                   (GCallback)baobab_settings_subfoldertips_changed, NULL);
}

static void baobab_shutdown(void) {
...
static BaobabChartMenu *create_context_menu(void) {
  ...
  menu->up_item =
      gtk_image_menu_item_new_with_label(_("Move to parent folder"));
  gtk_image_menu_item_set_image(
      GTK_IMAGE_MENU_ITEM(menu->up_item),
      gtk_image_new_from_icon_name("go-up", GTK_ICON_SIZE_MENU));

  menu->zoom_in_item = gtk_image_menu_item_new_with_label(_("Zoom in"));
  gtk_image_menu_item_set_image(
      GTK_IMAGE_MENU_ITEM(menu->zoom_in_item),
      gtk_image_new_from_icon_name("list-add", GTK_ICON_SIZE_MENU));

  menu->zoom_out_item = gtk_image_menu_item_new_with_label(_("Zoom out"));
  gtk_image_menu_item_set_image(
      GTK_IMAGE_MENU_ITEM(menu->zoom_out_item),
      gtk_image_new_from_icon_name("list-remove", GTK_ICON_SIZE_MENU));

  menu->snapshot_item =
      gtk_image_menu_item_new_with_label(_("Save screenshot"));
  gtk_image_menu_item_set_image(
      GTK_IMAGE_MENU_ITEM(menu->snapshot_item),
      gtk_image_new_from_icon_name("applets-screenshooter",
                                   GTK_ICON_SIZE_MENU));

  gtk_menu_shell_append(GTK_MENU_SHELL(menu->widget), menu->up_item);
  gtk_menu_shell_append(GTK_MENU_SHELL(menu->widget),
                        gtk_separator_menu_item_new());
  gtk_menu_shell_append(GTK_MENU_SHELL(menu->widget), menu->zoom_in_item);
  gtk_menu_shell_append(GTK_MENU_SHELL(menu->widget), menu->zoom_out_item);
  gtk_menu_shell_append(GTK_MENU_SHELL(menu->widget),
                        gtk_separator_menu_item_new());
  gtk_menu_shell_append(GTK_MENU_SHELL(menu->widget), menu->snapshot_item);

  /* connect signals */
  g_signal_connect(menu->up_item, "activate", G_CALLBACK(on_move_upwards_cb),
                   NULL);
  g_signal_connect(menu->zoom_in_item, "activate", G_CALLBACK(on_zoom_in_cb),
                   NULL);
  g_signal_connect(menu->zoom_out_item, "activate", G_CALLBACK(on_zoom_out_cb),
                   NULL);
...
static void on_chart_item_activated(BaobabChart *chart, GtkTreeIter *iter) {
  GtkTreePath *path;
  ...
  gtk_tree_view_set_cursor(GTK_TREE_VIEW(baobab.tree_view), path, NULL, FALSE);
  gtk_tree_path_free(path);
}

static gboolean on_chart_button_release(BaobabChart *chart,
                                        GdkEventButton *event, gpointer data) {
  if (baobab_chart_is_frozen(baobab.current_chart)) return FALSE;
  ...
  if (event->button == 3) /* right button */
  {
    ...
    gtk_widget_set_sensitive(
        menu->up_item,
        ((root_path != NULL) && (gtk_tree_path_get_depth(root_path) > 1)));
    ...
    /* show the menu */
    gtk_menu_popup_at_pointer(GTK_MENU(menu->widget), (const GdkEvent *)event);
...
static void drag_data_received_handl(GtkWidget *widget, GdkDragContext *context,
  ...
  if ((selection_data != NULL) &&
    ...
    gchar **uri_list;
    uri_list = g_uri_list_extract_uris(
...
static void set_active_chart(GtkWidget *chart) {
  if (baobab.current_chart != chart) {
    ...
    g_settings_set_string(
        baobab.ui_settings, BAOBAB_SETTINGS_ACTIVE_CHART,
        baobab.current_chart == baobab.rings_chart ? "rings" : "treemap");
  }
}

static void on_chart_type_change(GtkWidget *combo, gpointer user_data) {
  GtkWidget *chart;
  gint active;
...
static void initialize_charts(void) {
  ...
  gtk_paned_pack2(GTK_PANED(hpaned_main), baobab.chart_frame, TRUE, TRUE);
  ...
  g_signal_connect(baobab.chart_type_combo, "changed",
  ...
  gtk_box_set_child_packing(GTK_BOX(hbox1), baobab.chart_type_combo, FALSE,
                            TRUE, 0, GTK_PACK_END);
  ...
  baobab_chart_set_model_with_columns(
      baobab.treemap_chart, GTK_TREE_MODEL(baobab.model), COL_DIR_NAME,
      COL_DIR_SIZE, COL_H_PARSENAME, COL_H_PERC, COL_H_ELEMENTS, NULL);
  ...
  baobab_chart_set_model_with_columns(
      baobab.rings_chart, GTK_TREE_MODEL(baobab.model), COL_DIR_NAME,
      COL_DIR_SIZE, COL_H_PARSENAME, COL_H_PERC, COL_H_ELEMENTS, NULL);
  ...
  saved_chart =
      g_settings_get_string(baobab.ui_settings, BAOBAB_SETTINGS_ACTIVE_CHART);
...
static gboolean start_proc_on_command_line(GFile *file) {
  baobab_scan_location(file);
...
int main(int argc, char *argv[]) {
  ...
  const GOptionEntry options[] = {
      {"version", 'V', G_OPTION_FLAG_NO_ARG, G_OPTION_ARG_CALLBACK,
       show_version, N_("Show version"), NULL},
      {G_OPTION_REMAINING, 0, 0, G_OPTION_ARG_FILENAME_ARRAY, &directories,
       NULL, N_("[DIRECTORY]")},
  ...
  if (directories && directories[0]) {
    ...
    g_idle_add_full(G_PRIORITY_DEFAULT_IDLE,
                    (GSourceFunc)start_proc_on_command_line, file,
                    g_object_unref);
  }

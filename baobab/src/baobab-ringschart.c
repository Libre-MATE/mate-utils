...
G_DEFINE_TYPE_WITH_PRIVATE(BaobabRingschart, baobab_ringschart,
                           BAOBAB_CHART_TYPE);

static void baobab_ringschart_draw_sector(cairo_t *cr, gdouble center_x,
                                          gdouble center_y, gdouble radius,
                                          gdouble thickness, gdouble init_angle,
                                          gdouble final_angle,
                                          BaobabChartColor fill_color,
                                          gboolean continued, guint border);
static void baobab_ringschart_draw_item(GtkWidget *chart, cairo_t *cr,
...
static void baobab_ringschart_class_init(BaobabRingschartClass *class) {
  ...
  chart_class->calculate_item_geometry =
      baobab_ringschart_calculate_item_geometry;
...
static void baobab_ringschart_init(BaobabRingschart *chart) {
...
static void baobab_ringschart_draw_sector(cairo_t *cr, gdouble center_x,
  ...
  cairo_arc_negative(cr, center_x, center_y, radius + thickness, final_angle,
                     init_angle);
  cairo_close_path(cr);
...
static void baobab_ringschart_draw_item(GtkWidget *chart, cairo_t *cr,
  ...
  if (priv->drawing_subtips)
    if ((priv->highlighted_item) && (item->parent) &&
      ...
      if (priv->subtip_items) priv->subtip_items->prev = node;
      priv->subtip_items = node;
    }

  baobab_chart_get_item_color(&fill_color, data->start_angle / M_PI * 99,
                              item->depth, highlighted);

  gtk_widget_get_allocation(chart, &allocation);
  baobab_ringschart_draw_sector(
      cr, allocation.width / 2, allocation.height / 2, data->min_radius,
      data->max_radius - data->min_radius, data->start_angle,
      data->start_angle + data->angle, fill_color, data->continued,
      ITEM_BORDER_WIDTH);
}
...
static void baobab_ringschart_calculate_item_geometry(GtkWidget *chart,
  ...
  if (item->data == NULL) item->data = g_new(BaobabRingschartItem, 1);
  ...
  if (item->parent == NULL) {
    ...
    if (data->angle < ITEM_MIN_ANGLE) return;

    data->start_angle =
        p_data.start_angle + p_data.angle * item->rel_start / 100;
...
static gboolean baobab_ringschart_is_point_over_item(GtkWidget *chart,
  ...
  return (radius >= data->min_radius) && (radius <= data->max_radius) &&
...
static gboolean baobab_ringschart_subfolder_tips_timeout(gpointer data) {
  BaobabRingschartPrivate *priv;
...
static void baobab_ringschart_clean_subforlder_tips_state(GtkWidget *chart) {
  BaobabRingschartPrivate *priv;
  GList *node;

  priv = baobab_ringschart_get_instance_private(BAOBAB_RINGSCHART(chart));

  if (priv->drawing_subtips) gtk_widget_queue_draw(chart);

  priv->drawing_subtips = FALSE;

  if (priv->highlighted_item == NULL) return;
...
static void baobab_ringschart_draw_subfolder_tips(GtkWidget *chart,
                                                  cairo_t *cr) {
  BaobabRingschartPrivate *priv;
  GList *node;
  BaobabChartItem *item;
  ...
  while (node != NULL) {
    ...
    while (middle_angle_n > M_PI / 2) middle_angle_n -= M_PI;
    ...
    tooltip_rect.x =
        q_width + tip_x - layout_rect.width / 2 - SUBFOLDER_TIP_PADDING;
    tooltip_rect.y =
        q_height + tip_y - layout_rect.height / 2 - SUBFOLDER_TIP_PADDING;
    ...
    while (a > M_PI / 2) {
      if (i % 2 == 0)
        ...
        tooltip_rect.y =
            allocation.height - tooltip_rect.y - tooltip_rect.height;
    ...
    if (!gdk_rectangle_intersect(&_rect, &last_rect, NULL)) {
      ...
      middle_radius =
          data->min_radius + (data->max_radius - data->min_radius) / 2;
      ...
      cairo_rectangle(cr, tooltip_rect.x, tooltip_rect.y, tooltip_rect.width,
                      tooltip_rect.height);
      cairo_set_source_rgb(cr, 0.8275, 0.8431, 0.8118); /* tango: #d3d7cf */
      cairo_fill_preserve(cr);
...
static void baobab_ringschart_pre_draw(GtkWidget *chart, cairo_t *cr) {
  ...
  if (hl_item != priv->highlighted_item) {
    ...
    priv->tips_timeout_event = g_timeout_add(
        priv->subtip_timeout,
        (GSourceFunc)baobab_ringschart_subfolder_tips_timeout, (gpointer)chart);
  }
}

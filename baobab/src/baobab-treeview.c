...
static GtkTreeStore *create_model(void) {
  GtkTreeStore *mdl =
      gtk_tree_store_new(NUM_TREE_COLUMNS, G_TYPE_STRING, /* COL_DIR_NAME */
...
static void on_tv_row_expanded(GtkTreeView *treeview, GtkTreeIter *arg1,
...
static void on_tv_cur_changed(GtkTreeView *treeview, gpointer data) {
  ...
  gtk_tree_selection_get_selected(gtk_tree_view_get_selection(treeview), NULL,
                                  &iter);

  if (gtk_tree_store_iter_is_valid(baobab.model, &iter)) {
    gtk_tree_model_get(GTK_TREE_MODEL(baobab.model), &iter, COL_H_PARSENAME,
                       &parsename, -1);
  }
}

static void contents_changed(void) {
  if (messageyesno(_("Rescan your home folder?"),
                   _("The content of your home folder has changed. Select "
                     "rescan to update the disk usage details."),
                   GTK_MESSAGE_QUESTION, _("_Rescan"),
                   baobab.window) == GTK_RESPONSE_OK) {
...
static gboolean on_tv_button_press(GtkWidget *widget, GdkEventButton *event,
                                   gpointer data) {
  GtkTreePath *path;
  GtkTreeIter iter;
  GFile *file;

  gtk_tree_view_get_path_at_pos(GTK_TREE_VIEW(widget), (gint)event->x,
...
static gboolean baobab_treeview_equal_func(GtkTreeModel *model, gint column,
...
static void perc_cell_data_func(GtkTreeViewColumn *col,
...
GtkWidget *create_directory_treeview(void) {
  ...
  g_signal_connect(tvw, "row-expanded", G_CALLBACK(on_tv_row_expanded), NULL);
  g_signal_connect(tvw, "cursor-changed", G_CALLBACK(on_tv_cur_changed), NULL);
  g_signal_connect(tvw, "button-press-event", G_CALLBACK(on_tv_button_press),
                   NULL);
  ...
  col = gtk_tree_view_column_new_with_attributes(
      NULL, cell, "markup", COL_DIR_NAME, "text", COL_DIR_NAME, NULL);
  ...
  gtk_tree_view_column_set_attributes(col, cell, "perc", COL_H_PERC, NULL);

  cell = gtk_cell_renderer_text_new();
  gtk_tree_view_column_pack_start(col, cell, TRUE);
  gtk_tree_view_column_set_cell_data_func(col, cell, perc_cell_data_func, NULL,
                                          NULL);
  ...
  cell = gtk_cell_renderer_text_new();
  col = gtk_tree_view_column_new_with_attributes(
      NULL, cell, "markup", COL_DIR_SIZE, "text", COL_DIR_SIZE, NULL);
  g_object_set(G_OBJECT(cell), "xalign", (gfloat)1.0, NULL);
  gtk_tree_view_column_set_sort_column_id(
      col, baobab.show_allocated ? COL_H_ALLOCSIZE : COL_H_SIZE);
  gtk_tree_view_column_set_reorderable(col, TRUE);
  ...
  cell = gtk_cell_renderer_text_new();
  col = gtk_tree_view_column_new_with_attributes(
      NULL, cell, "markup", COL_ELEMENTS, "text", COL_ELEMENTS, NULL);
  g_object_set(G_OBJECT(cell), "xalign", (gfloat)1.0, NULL);

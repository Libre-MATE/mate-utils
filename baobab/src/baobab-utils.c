...
static void filechooser_cb(GtkWidget *chooser, gint response, gpointer data) {
 ...
 */
gchar *dir_select(gboolean SEARCH, GtkWidget *parent) {
  static GtkWidget *file_chooser = NULL;
  GtkWidget *toggle;

  if (file_chooser == NULL) {
    file_chooser = gtk_file_chooser_dialog_new(
        _("Select Folder"), GTK_WINDOW(parent),
        GTK_FILE_CHOOSER_ACTION_SELECT_FOLDER, "gtk-cancel",
        GTK_RESPONSE_CANCEL, "gtk-open", GTK_RESPONSE_OK, NULL);
    ...
    g_signal_connect((gpointer)toggle, "toggled", G_CALLBACK(on_toggled),
                     file_chooser);
    gtk_file_chooser_set_extra_widget(GTK_FILE_CHOOSER(file_chooser), toggle);

    g_signal_connect(file_chooser, "response", G_CALLBACK(filechooser_cb),
                     NULL);
    g_signal_connect(file_chooser, "destroy", G_CALLBACK(gtk_widget_destroyed),
                     &file_chooser);
...
void on_toggled(GtkToggleButton *togglebutton, gpointer dialog) {
  gtk_file_chooser_set_show_hidden(
      GTK_FILE_CHOOSER(dialog),
      !gtk_file_chooser_get_show_hidden(GTK_FILE_CHOOSER(dialog)));
}

gboolean show_bars(GtkTreeModel *mdl, GtkTreePath *path, GtkTreeIter *iter,
                   gpointer data) {
  GtkTreeIter parent;
  gdouble perc;
  ...
  if (gtk_tree_model_iter_parent(mdl, &parent, iter)) {
    ...
    if (readelements == -1) {
      gtk_tree_store_set(GTK_TREE_STORE(mdl), iter, COL_DIR_SIZE, "--",
                         COL_ELEMENTS, "--", -1);
      return FALSE;
    }
    ...
    if (readelements == -1) {
      gtk_tree_store_set(GTK_TREE_STORE(mdl), iter, COL_DIR_SIZE, sizecstr, -1);
    ...
    gtk_tree_store_set(GTK_TREE_STORE(mdl), iter, COL_DIR_SIZE, sizecstr,
                       COL_H_PERC, perc, -1);
    ...
    if (readelements != -1) {
      ...
      gtk_tree_store_set(GTK_TREE_STORE(mdl), iter, COL_H_PERC, 100.0,
...
void message(const gchar *primary_msg, const gchar *secondary_msg,
  ...
  dialog =
      gtk_message_dialog_new(GTK_WINDOW(parent), GTK_DIALOG_DESTROY_WITH_PARENT,
                             type, GTK_BUTTONS_OK, "%s", primary_msg);
...
gint messageyesno(const gchar *primary_msg, const gchar *secondary_msg,
  ...
  dialog =
      gtk_message_dialog_new(GTK_WINDOW(parent), GTK_DIALOG_DESTROY_WITH_PARENT,
                             type, GTK_BUTTONS_CANCEL, "%s", primary_msg);
  gtk_message_dialog_format_secondary_text(GTK_MESSAGE_DIALOG(dialog), "%s",
                                           secondary_msg);
...
gboolean baobab_check_dir(GFile *file) {
  GFileInfo *info;
  GError *error = NULL;
  gboolean ret = TRUE;

  if (baobab_is_excluded_location(file)) {
    message("", _("Cannot check an excluded folder!"), GTK_MESSAGE_INFO,
            baobab.window);
    return FALSE;
  }
  ...
  if ((g_file_info_get_file_type(info) != G_FILE_TYPE_DIRECTORY) ||
    ...
    error_msg = g_strdup_printf(_("\"%s\" is not a valid folder"), name);

    message(error_msg, _("Could not analyze disk usage."), GTK_MESSAGE_ERROR,
            baobab.window);
...
static void add_popupmenu_item(GtkMenu *pmenu, const gchar *label,
                               const gchar *icon_name, GCallback item_cb) {
  GtkWidget *item;
  GtkWidget *image;
...
void popupmenu_list(GtkTreePath *path, GdkEventButton *event,
  ...
  add_popupmenu_item(GTK_MENU(pmenu), _("_Open Folder"), "document-open",
                     G_CALLBACK(open_file_cb));

  if (baobab.is_local && can_trash) {
    add_popupmenu_item(GTK_MENU(pmenu), _("Mo_ve to Trash"), "edit-delete",
                       G_CALLBACK(trash_dir_cb));
  }

  gtk_widget_show_all(pmenu);
  gtk_menu_popup_at_pointer(GTK_MENU(pmenu), (const GdkEvent *)event);
}
...
gboolean can_trash_file(GFile *file) {
  ...
  info = g_file_query_info(file, G_FILE_ATTRIBUTE_ACCESS_CAN_TRASH,
                           G_FILE_QUERY_INFO_NONE, NULL, NULL);

  if (info) {
    if (g_file_info_has_attribute(info, G_FILE_ATTRIBUTE_ACCESS_CAN_TRASH)) {
      can_trash = g_file_info_get_attribute_boolean(
          info, G_FILE_ATTRIBUTE_ACCESS_CAN_TRASH);
    }
...
gboolean trash_file(GFile *file) {
  ...
  if (!g_file_trash(file, NULL, &error)) {
    ...
    /* fallback */
    if (str == NULL) str = g_strdup(_("Could not move file to the Trash"));
...
gboolean baobab_help_display(GtkWindow *parent, const gchar *file_name,
  ...
  uri = (link_id) ? g_strdup_printf("help:%s/%s", file_name, link_id)
...
gboolean is_virtual_filesystem(GFile *file) {
  ...
  if (path != NULL) {
    if ((strcmp(path, "/proc") == 0) || (strcmp(path, "/sys") == 0)) ret = TRUE;
  }

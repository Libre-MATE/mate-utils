...
gboolean screenshot_grab_lock(void) {
  ...
  if (gdk_selection_owner_get(selection_atom) != NULL) goto out;
...
static GdkWindow *screen_get_active_window(GdkScreen *screen) {
  GdkWindow *ret = NULL;
  Atom type_return;
  gint format_return;
  ...
  if (!gdk_x11_screen_supports_net_wm_hint(
          screen, gdk_atom_intern_static_string("_NET_ACTIVE_WINDOW")))
    return NULL;
  ...
  if (XGetWindowProperty(
          ...
          gdk_x11_get_xatom_by_name_for_display(gdk_screen_get_display(screen),
                                                "_NET_ACTIVE_WINDOW"),
          0, 1, False, XA_WINDOW, &type_return, &format_return, &nitems_return,
          &bytes_after_return, &data) == Success) {
    if ((type_return == XA_WINDOW) && (format_return == 32) && (data)) {
      Window window = *(Window *)data;

      if (window != None) {
        ret = gdk_x11_window_foreign_new_for_display(
            gdk_screen_get_display(screen), window);
      }
    }
  }
...
static GdkWindow *screenshot_find_active_window(void) {
  GdkWindow *window;
...
static gboolean screenshot_window_is_desktop(GdkWindow *window) {
  GdkWindow *root_window = gdk_get_default_root_window();
  GdkWindowTypeHint window_type_hint;

  if (window == root_window) return TRUE;

  window_type_hint = gdk_window_get_type_hint(window);
  if (window_type_hint == GDK_WINDOW_TYPE_HINT_DESKTOP) return TRUE;

  return FALSE;
}

GdkWindow *screenshot_find_current_window() {
...
static gboolean select_area_button_press(GtkWidget *window,
                                         GdkEventButton *event,
                                         select_area_filter_data *data) {
  if (data->button_pressed) return TRUE;
...
static gboolean select_area_motion_notify(GtkWidget *window,
  ...
  if (!gtk_widget_get_app_paintable(window)) {
    ...
    if (draw_rect.width > 2 && draw_rect.height > 2) {
      ...
      cairo_rectangle_int_t region_rect = {0, 0, draw_rect.width - 2,
                                           draw_rect.height - 2};
...
static gboolean select_area_button_release(GtkWidget *window,
                                           GdkEventButton *event,
                                           select_area_filter_data *data) {
  if (!data->button_pressed) return TRUE;
...
static gboolean select_area_key_press(GtkWidget *window, GdkEventKey *event,
                                      select_area_filter_data *data) {
...
static gboolean draw(GtkWidget *window, cairo_t *cr, gpointer unused) {
...
static gboolean emit_select_callback_in_idle(gpointer user_data) {
...
void screenshot_select_area_async(SelectAreaCallback callback) {
  ...
  g_signal_connect(data.window, "key-press-event",
                   G_CALLBACK(select_area_key_press), &data);
  g_signal_connect(data.window, "button-press-event",
                   G_CALLBACK(select_area_button_press), &data);
  g_signal_connect(data.window, "button-release-event",
                   G_CALLBACK(select_area_button_release), &data);
  g_signal_connect(data.window, "motion-notify-event",
                   G_CALLBACK(select_area_motion_notify), &data);
  ...
  res = gdk_seat_grab(seat, gtk_widget_get_window(data.window),
                      GDK_SEAT_CAPABILITY_ALL, FALSE, cursor, NULL, NULL, NULL);
...
static Window find_wm_window(Window xid) {
  ...
  do {
    ...
    if (root == parent) return xid;

    xid = parent;
  } while (TRUE);
}

static cairo_region_t *make_region_with_monitors(GdkScreen *screen) {
  GdkDisplay *display;
...
static void blank_rectangle_in_pixbuf(GdkPixbuf *pixbuf, GdkRectangle *rect) {
  int x, y;
  int x2, y2;
  guchar *pixels;
  int rowstride;
  ...
  for (y = rect->y; y < y2; y++) {
    ...
    for (x = rect->x; x < x2; x++) {
      ...
      *p++ = 0;
      ...
      if (has_alpha) *p++ = 255; /* opaque black */
    }
  }
}

static void blank_region_in_pixbuf(GdkPixbuf *pixbuf, cairo_region_t *region) {
  int n_rects;
  int i;
  int width, height;
 ...
 */
static void mask_monitors(GdkPixbuf *pixbuf, GdkWindow *root_window) {
  GdkScreen *screen;
...
GdkPixbuf *screenshot_get_pixbuf(GdkWindow *window, GdkRectangle *rectangle,
  ...
  if (include_border) {
    ...
    if (wm != None)
      window =
          gdk_x11_window_foreign_new_for_display(gdk_display_get_default(), wm);
  ...
  if (x_orig + width > screen_width) width = screen_width - x_orig;

  if (y_orig + height > screen_height) height = screen_height - y_orig;
   ...
   */
  if (include_mask) mask_monitors(screenshot, root);
  ...
  if (include_border) {
    ...
    rectangles = XShapeGetRectangles(
        GDK_DISPLAY_XDISPLAY(gdk_display_get_default()), GDK_WINDOW_XID(window),
      ...
      for (i = 0; i < rectangle_count; i++) {
        ...
        for (y = rec_y; y < rec_y + rec_height; y++) {
          ...
          dest_pixels = gdk_pixbuf_get_pixels(tmp) +
                        y * gdk_pixbuf_get_rowstride(tmp) + rec_x * 4;
  ...
  if (include_pointer && !rectangle) {
    ...
    if (cursor_pixbuf != NULL) {
      ...
      if (gdk_rectangle_intersect(&r1, &r2, &r2)) {
        gdk_pixbuf_composite(cursor_pixbuf, screenshot, cx - xhot, cy - yhot,
                             r2.width, r2.height, cx - xhot, cy - yhot, 1.0,

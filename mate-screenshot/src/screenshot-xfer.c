...
static gboolean do_run_overwrite_confirm_dialog(gpointer _data) {
  ...
  dialog = gtk_message_dialog_new(
      GTK_WINDOW(data->parent), GTK_DIALOG_DESTROY_WITH_PARENT,
      GTK_MESSAGE_QUESTION, GTK_BUTTONS_NONE, _("File already exists"));
  ...
  gtk_dialog_add_button(GTK_DIALOG(dialog), _("_Replace"), GTK_RESPONSE_OK);
...
static void transfer_dialog_response_cb(GtkDialog *d, gint response,
                                        GCancellable *cancellable) {
...
static gboolean transfer_progress_dialog_new(TransferJob *job) {
  TransferDialog *dialog;
  GtkWidget *gdialog;
  GtkWidget *widget;
  ...
  gdialog = gtk_message_dialog_new(
      GTK_WINDOW(job->parent), GTK_DIALOG_DESTROY_WITH_PARENT,
      GTK_MESSAGE_OTHER, GTK_BUTTONS_CANCEL, _("Saving file..."));
  widget = gtk_progress_bar_new();
  gtk_box_pack_start(
      GTK_BOX(gtk_message_dialog_get_message_area GTK_MESSAGE_DIALOG(gdialog)),
      widget, FALSE, 0, 0);
  gtk_widget_show(widget);
  dialog->progress_bar = widget;
  dialog->dialog = gdialog;

  g_signal_connect(gdialog, "response", G_CALLBACK(transfer_dialog_response_cb),
                   job->cancellable);
...
static void transfer_progress_dialog_start(TransferJob *job) {
  /* sends to the mainloop and schedules show */
  if (job->dialog_timeout_id == 0)
    job->dialog_timeout_id = g_timeout_add_seconds(
        1, (GSourceFunc)transfer_progress_dialog_new, job);
}

static int run_overwrite_confirm_dialog(TransferJob *job) {
  ...
  g_io_scheduler_job_send_to_mainloop(
      job->io_job, do_run_overwrite_confirm_dialog, data, NULL);
  response = data->resp;

  if (need_timeout) transfer_progress_dialog_start(job);
...
static gboolean transfer_progress_dialog_update(TransferJob *job) {
...
static gboolean transfer_job_done(gpointer user_data) {
  ...
  if (dialog) gtk_widget_destroy(dialog->dialog);

  if (job->callback) {
    (job->callback)(job->result, job->error, job->callback_data);
  }
...
static void transfer_progress_cb(goffset current_num_bytes,
                                 goffset total_num_bytes, TransferJob *job) {
  job->current_bytes = current_num_bytes;

  if (!job->dialog) return;

  g_io_scheduler_job_send_to_mainloop_async(
      job->io_job, (GSourceFunc)transfer_progress_dialog_update, job, NULL);
}
...
static goffset get_file_size(GFile *file) {
  ...
  file_info =
      g_file_query_info(file, G_FILE_ATTRIBUTE_STANDARD_SIZE, 0, NULL, NULL);
  if (file_info != NULL) {
...
static gboolean transfer_file(GIOSchedulerJob *io_job,
...
retry:
  error = NULL;
  if (!g_file_copy(job->source, job->dest, job->flags, job->cancellable,
                   (GFileProgressCallback)transfer_progress_cb, job, &error)) {
...
out:
  ...
  g_io_scheduler_job_send_to_mainloop_async(io_job, transfer_job_done, job,
                                            NULL);
  return FALSE;
}
...
void screenshot_xfer_uri(GFile *source_file, GFile *target_file,
  ...
  g_io_scheduler_push_job(transfer_file, job, NULL, 0, job->cancellable);
}

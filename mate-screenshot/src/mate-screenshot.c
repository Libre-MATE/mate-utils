...
static void display_help(GtkWindow *parent) {
  ...
  gtk_show_uri_on_window(parent, "help:mate-user-guide/goseditmainmenu-53",
                         gtk_get_current_event_time(), &error);

  if (error) {
    screenshot_show_gerror_dialog(parent, _("Error loading the help page"),
                                  error);
    g_error_free(error);
  }
}

static void interactive_dialog_response_cb(GtkDialog *dialog, gint response,
                                           gpointer user_data) {
  switch (response) {
...
static void target_toggled_cb(GtkToggleButton *button, gpointer data) {
...
static void delay_spin_value_changed_cb(GtkSpinButton *button) {
  delay = gtk_spin_button_get_value_as_int(button);
}

static void include_border_toggled_cb(GtkToggleButton *button, gpointer data) {
  include_border = gtk_toggle_button_get_active(button);
}

static void include_pointer_toggled_cb(GtkToggleButton *button, gpointer data) {
  include_pointer = gtk_toggle_button_get_active(button);
}

static void effect_combo_changed_cb(GtkComboBox *combo, gpointer user_data) {
  GtkTreeIter iter;
...
static GtkWidget *create_effects_combo(void) {
  GtkWidget *retval;
  GtkListStore *model;
  ...
  model =
      gtk_list_store_new(N_COLUMNS, G_TYPE_STRING, G_TYPE_STRING, G_TYPE_UINT);

  for (i = 0; i < n_effects; i++) {
    GtkTreeIter iter;

    gtk_list_store_insert(model, &iter, i);
    gtk_list_store_set(model, &iter, COLUMN_ID, effects[i].id, COLUMN_LABEL,
                       gettext(effects[i].label), COLUMN_NICK, effects[i].nick,
                       -1);
  }

  retval = gtk_combo_box_new();
  gtk_combo_box_set_model(GTK_COMBO_BOX(retval), GTK_TREE_MODEL(model));
  g_object_unref(model);

  switch (border_effect[0]) {
    case 's': /* shadow */
      gtk_combo_box_set_active(GTK_COMBO_BOX(retval), SCREENSHOT_EFFECT_SHADOW);
      break;
    case 'b': /* border */
      gtk_combo_box_set_active(GTK_COMBO_BOX(retval), SCREENSHOT_EFFECT_BORDER);
      break;
    case 'n': /* none */
      gtk_combo_box_set_active(GTK_COMBO_BOX(retval), SCREENSHOT_EFFECT_NONE);
      break;
    default:
      break;
  }
  ...
  gtk_cell_layout_set_attributes(GTK_CELL_LAYOUT(retval), renderer, "text",
                                 COLUMN_LABEL, NULL);

  g_signal_connect(retval, "changed", G_CALLBACK(effect_combo_changed_cb),
                   NULL);

  return retval;
}

static void create_effects_frame(GtkWidget *outer_vbox,
  ...
  g_signal_connect(check, "toggled", G_CALLBACK(include_pointer_toggled_cb),
                   NULL);
  ...
  g_signal_connect(check, "toggled", G_CALLBACK(include_border_toggled_cb),
                   NULL);
  gtk_box_pack_start(GTK_BOX(vbox), check, FALSE, FALSE, 0);
  gtk_widget_show(check);
  border_check = check;
...
static void create_screenshot_frame(GtkWidget *outer_vbox,
                                    const gchar *frame_title) {
  GtkWidget *main_vbox, *vbox, *hbox;
  GtkWidget *radio;
  GtkWidget *image;
  ...
  image =
      gtk_image_new_from_icon_name(SCREENSHOOTER_ICON, GTK_ICON_SIZE_DIALOG);
  ...
  group = NULL;
  radio =
      gtk_radio_button_new_with_mnemonic(group, _("Grab the whole _desktop"));
  if (take_window_shot || take_area_shot)
    gtk_toggle_button_set_active(GTK_TOGGLE_BUTTON(radio), FALSE);
  g_signal_connect(radio, "toggled", G_CALLBACK(target_toggled_cb),
  ...
  /** Grab current window **/
  radio =
      gtk_radio_button_new_with_mnemonic(group, _("Grab the current _window"));
  if (take_window_shot)
  ...
  radio = gtk_radio_button_new_with_mnemonic(group, _("Select _area to grab"));
  if (take_area_shot)
    gtk_toggle_button_set_active(GTK_TOGGLE_BUTTON(radio), TRUE);
  g_signal_connect(radio, "toggled", G_CALLBACK(target_toggled_cb),
  ...
  adjust = GTK_ADJUSTMENT(
      gtk_adjustment_new((gdouble)delay, 0.0, 99.0, 1.0, 1.0, 0.0));
  spin = gtk_spin_button_new(adjust, 1.0, 0);
  g_signal_connect(spin, "value-changed",
                   G_CALLBACK(delay_spin_value_changed_cb), NULL);
...
static GtkWidget *create_interactive_dialog(void) {
  GtkWidget *retval;
  GtkWidget *main_vbox;
  ...
  gtk_dialog_add_buttons(GTK_DIALOG(retval), "gtk-help", GTK_RESPONSE_HELP,
                         "gtk-cancel", GTK_RESPONSE_CANCEL,
                         _("Take _Screenshot"), GTK_RESPONSE_OK, NULL);
  ...
  g_signal_connect(retval, "key-press-event", G_CALLBACK(key_press_cb), NULL);

  return retval;
}

static void save_folder_to_settings(ScreenshotDialog *dialog) {
  char *folder;

  folder = screenshot_dialog_get_folder(dialog);
  g_settings_set_string(settings, LAST_SAVE_DIRECTORY_KEY, folder);

  g_free(folder);
}

static void set_recent_entry(ScreenshotDialog *dialog) {
...
static void error_dialog_response_cb(GtkDialog *d, gint response,
...
static void save_callback(TransferResult result, char *error_message,
                          gpointer data) {
  ...
  if (result == TRANSFER_OK) {
    ...
    gtk_main_quit();
  } else if (result == TRANSFER_OVERWRITE || result == TRANSFER_CANCELLED) {
  ...
  } else /* result == TRANSFER_ERROR */
  {
    ...
    error_dialog = gtk_message_dialog_new(
        GTK_WINDOW(toplevel), GTK_DIALOG_DESTROY_WITH_PARENT, GTK_MESSAGE_ERROR,
        GTK_BUTTONS_OK, _("Error while saving screenshot"));
    ...
    g_signal_connect(error_dialog, "response",
                     G_CALLBACK(error_dialog_response_cb), dialog);

    g_free(uri);
  }
}

static void try_to_save(ScreenshotDialog *dialog, const char *target) {
  ...
  screenshot_xfer_uri(source_file, target_file,
                      screenshot_dialog_get_toplevel(dialog), save_callback,
                      dialog);
...
static void save_screenshot_in_clipboard(GdkDisplay *display,
...
static void run_dialog(ScreenshotDialog *dialog) {
  GtkWidget *toplevel;
  ...
  g_signal_connect(toplevel, "response",
                   G_CALLBACK(screenshot_dialog_response_cb), dialog);
}

static void play_sound_effect(GdkWindow *window) {
  ca_context *c;
  ca_proplist *p = NULL;
  int res;
  ...
  if (window != NULL) {
    res = ca_proplist_setf(p, CA_PROP_WINDOW_X11_XID, "%lu",
  ...
  if (p != NULL) ca_proplist_destroy(p);
}

static void finish_prepare_screenshot(char *initial_uri, GdkWindow *window,
                                      GdkRectangle *rectangle) {
  ScreenshotDialog *dialog;
  ...
  if (screenshot == NULL) {
    screenshot_show_error_dialog(
        NULL, _("Unable to take a screenshot of the current window"), NULL);
    exit(1);
  }
...
static void async_existence_job_free(AsyncExistenceJob *job) {
  if (!job) return;

  g_free(job->base_uris[1]);
  g_free(job->base_uris[2]);

  if (job->rectangle != NULL) g_slice_free(GdkRectangle, job->rectangle);

  g_slice_free(AsyncExistenceJob, job);
}

static gboolean check_file_done(gpointer user_data) {
...
static char *build_uri(AsyncExistenceJob *job) {
  ...
  if (job->iteration == 0) {
     ...
     * taken */
    file_name = g_strdup_printf(_("Screenshot at %s - %d.png"), timestamp,
                                job->iteration);
  }
...
static gboolean try_check_file(GIOSchedulerJob *io_job,
                               GCancellable *cancellable, gpointer data) {
...
out:
  ...
  g_io_scheduler_job_send_to_mainloop_async(io_job, check_file_done, job, NULL);
  return FALSE;
}

static GdkWindow *find_current_window(void) {
  GdkWindow *window;

  if (!screenshot_grab_lock()) exit(0);
...
static void push_check_file_job(GdkRectangle *rectangle) {
  ...
  g_io_scheduler_push_job(try_check_file, job, NULL, 0, NULL);
}

static void rectangle_found_cb(GdkRectangle *rectangle) {
  push_check_file_job(rectangle);
}

static void prepare_screenshot(void) {
  if (take_area_shot)
...
static gboolean prepare_screenshot_timeout(gpointer data) {
  prepare_screenshot();
  save_options();
...
static gchar *get_desktop_dir(void) {
  ...
  if (schema != NULL) {
    ...
    desktop_is_home_dir =
        g_settings_get_boolean(caja_prefs, "desktop-is-home-dir");
  ...
  if (desktop_is_home_dir)
    ...
    desktop_dir = g_strconcat(
        "file://", g_get_user_special_dir(G_USER_DIRECTORY_DESKTOP), NULL);

  return desktop_dir;
}

/* Taken from mate-vfs-utils.c */
static char *expand_initial_tilde(const char *path) {
...
static void load_options(void) {
  /* Find various dirs */
  last_save_dir = g_settings_get_string(settings, LAST_SAVE_DIRECTORY_KEY);
  ...
  include_border = g_settings_get_boolean(settings, INCLUDE_BORDER_KEY);

  include_pointer = g_settings_get_boolean(settings, INCLUDE_POINTER_KEY);

  border_effect = g_settings_get_string(settings, BORDER_EFFECT_KEY);
  if (!border_effect) border_effect = g_strdup("none");

  delay = g_settings_get_int(settings, DELAY_KEY);
}

static void save_options(void) {
  g_settings_set_boolean(settings, INCLUDE_BORDER_KEY, include_border);
  g_settings_set_boolean(settings, INCLUDE_POINTER_KEY, include_pointer);
  g_settings_set_int(settings, DELAY_KEY, delay);
  g_settings_set_string(settings, BORDER_EFFECT_KEY, border_effect);
}

static void register_screenshooter_icon(GtkIconFactory *factory) {
...
static void screenshooter_init_stock_icons(void) {
...
void loop_dialog_screenshot(void) {
  ...
  if (((delay > 0 && interactive_arg) || delay_arg > 0) && !take_area_shot) {
    g_timeout_add(delay * 1000, prepare_screenshot_timeout, NULL);
  } else {
    if (interactive_arg) {
       ...
       * re-drawing.
       */
      g_timeout_add(200, prepare_screenshot_timeout, NULL);
    } else
...
int main(int argc, char *argv[]) {
  ...
  const GOptionEntry entries[] = {
      {"window", 'w', 0, G_OPTION_ARG_NONE, &window_arg,
       N_("Grab a window instead of the entire screen"), NULL},
      {"area", 'a', 0, G_OPTION_ARG_NONE, &area_arg,
       N_("Grab an area of the screen instead of the entire screen"), NULL},
      {"clipboard", 'c', 0, G_OPTION_ARG_NONE, &noninteractive_clipboard_arg,
       N_("Send grabbed area directly to the clipboard"), NULL},
      {"include-border", 'b', 0, G_OPTION_ARG_NONE, &include_border_arg,
       N_("Include the window border with the screenshot"), NULL},
      {"remove-border", 'B', 0, G_OPTION_ARG_NONE, &disable_border_arg,
       N_("Remove the window border from the screenshot"), NULL},
      {"delay", 'd', 0, G_OPTION_ARG_INT, &delay_arg,
       N_("Take screenshot after specified delay [in seconds]"), N_("seconds")},
      {"border-effect", 'e', 0, G_OPTION_ARG_STRING, &border_effect_arg,
       N_("Effect to add to the border (shadow, border or none)"),
       N_("effect")},
      {"interactive", 'i', 0, G_OPTION_ARG_NONE, &interactive_arg,
       N_("Interactively set options"), NULL},
      {"version", 0, 0, G_OPTION_ARG_NONE, &version_arg,
  ...
  if (interactive_arg && noninteractive_clipboard_arg) {
    g_printerr(
        _("Conflicting options: --clipboard and --interactive should not be "
  ...
  if (window_arg && area_arg) {
    g_printerr(
        _("Conflicting options: --window and --area should not be "
          "used at the same time.\n"));
    exit(1);
  }
  ...
  if (window_arg) take_window_shot = TRUE;

  if (area_arg) take_area_shot = TRUE;

  if (include_border_arg) include_border = TRUE;

  if (disable_border_arg) include_border = FALSE;
  ...
  if (delay_arg > 0) delay = delay_arg;

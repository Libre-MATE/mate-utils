...
static gboolean on_toplevel_key_press_event(GtkWidget *widget,
                                            GdkEventKey *key) {
...
static void on_preview_draw(GtkWidget *drawing_area, cairo_t *cr,
                            gpointer data) {
  ...
  if (!dialog->preview_image ||
    ...
    dialog->preview_image = gdk_pixbuf_scale_simple(
        dialog->screenshot, width, height, GDK_INTERP_BILINEAR);
  }
  ...
  gtk_style_context_set_state(context,
                              gtk_widget_get_state_flags(drawing_area));
  gtk_render_icon(context, cr, dialog->preview_image, 0, 0);

  gtk_style_context_restore(context);
}

static gboolean on_preview_button_press_event(GtkWidget *drawing_area,
...
static gboolean on_preview_button_release_event(GtkWidget *drawing_area,
                                                GdkEventButton *event,
                                                gpointer data) {
  ScreenshotDialog *dialog = data;
...
static void drag_data_get(GtkWidget *widget, GdkDragContext *context,
                          ...
                          guint time, ScreenshotDialog *dialog) {
  if (info == TYPE_TEXT_URI_LIST) {
    gchar **uris;

    uris = g_new(gchar *, 2);
    uris[0] = g_strconcat("file://", screenshot_save_get_filename(), NULL);
    uris[1] = NULL;
...
static void drag_begin(GtkWidget *widget, GdkDragContext *context,
  ...
  gtk_drag_set_icon_pixbuf(context, dialog->preview_image, dialog->drag_x,
                           dialog->drag_y);
}
...
ScreenshotDialog *screenshot_dialog_new(GdkPixbuf *screenshot,
  ...
  if (gtk_builder_add_from_resource(dialog->builder,
                                    "/org/mate/screenshot/mate-screenshot.ui",
                                    &error) == 0) {
    GtkWidget *dialog;
  ...
  dialog->save_widget = gtk_file_chooser_button_new(
  ...
  gtk_file_chooser_set_current_folder_uri(GTK_FILE_CHOOSER(dialog->save_widget),
                                          current_folder);
  gtk_entry_set_text(GTK_ENTRY(dialog->filename_entry), current_name);

  gtk_box_pack_start(GTK_BOX(file_chooser_box), dialog->save_widget, TRUE, TRUE,
                     0);
  ...
  g_signal_connect(toplevel, "key_press_event",
                   G_CALLBACK(on_toplevel_key_press_event), dialog);
  g_signal_connect(preview_darea, "draw", G_CALLBACK(on_preview_draw), dialog);
  g_signal_connect(preview_darea, "button_press_event",
                   G_CALLBACK(on_preview_button_press_event), dialog);
  g_signal_connect(preview_darea, "button_release_event",
                   G_CALLBACK(on_preview_button_release_event), dialog);
  ...
  /* setup dnd */
  g_signal_connect(preview_darea, "drag_begin", G_CALLBACK(drag_begin), dialog);
  g_signal_connect(preview_darea, "drag_data_get", G_CALLBACK(drag_data_get),
                   dialog);
  ...
  gtk_editable_select_region(GTK_EDITABLE(dialog->filename_entry), 0, pos);
...
void screenshot_dialog_enable_dnd(ScreenshotDialog *dialog) {
  ...
  preview_darea = GET_WIDGET("preview_darea");
  gtk_drag_source_set(preview_darea, GDK_BUTTON1_MASK | GDK_BUTTON3_MASK,
...
void screenshot_dialog_set_busy(ScreenshotDialog *dialog, gboolean busy) {
  GtkWidget *toplevel;
  GdkDisplay *display;

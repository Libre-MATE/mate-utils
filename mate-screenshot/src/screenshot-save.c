 ...
 */
static void clean_up_temporary_dir(gboolean gui_on_error) {
  char *message;
...
static void child_done_notification(GPid pid, gint status, gpointer data) {
  ...
  dialog = gtk_message_dialog_new(
      ...
      _("The child save process unexpectedly exited.  We are unable to write "
...
static gboolean read_pipe_from_child(GIOChannel *source, GIOCondition condition,
                                     gpointer data) {
...
static char *make_temp_directory(void) {
  ...
  do {
    gchar *tmp_dir =
        g_strdup_printf("mate-screenshot.%u.%d", (unsigned int)getpid(), i++);

    dir_name = g_build_filename(g_get_tmp_dir(), tmp_dir, NULL);
    g_free(tmp_dir);
...
void screenshot_save_start(GdkPixbuf *pixbuf, SaveFunction callback,
                           gpointer user_data) {
  GPid pid;
  int parent_exit_notification[2];
  int pipe_from_child[2];

  if (pipe(parent_exit_notification) == -1) perror("pipe error");

  if (pipe(pipe_from_child) == -1) perror("pipe error");

  parent_dir = make_temp_directory();
  if (parent_dir == NULL) return;

  tmp_filename = g_build_filename(parent_dir, _("Screenshot.png"), NULL);
  ...
  if (pid == 0) {
    ...
    if (!gdk_pixbuf_save(pixbuf, tmp_filename, "png", &error, "tEXt::Software",
                         "mate-screenshot", NULL)) {
      if (error && error->message) {
        if (write(pipe_from_child[1], error->message, strlen(error->message)) ==
            -1)
          perror("write error");
      } else {
#define ERROR_MESSAGE _("Unknown error saving screenshot to disk")
        if (write(pipe_from_child[1], ERROR_MESSAGE, strlen(ERROR_MESSAGE)) ==
            -1)
          perror("write error");
      }
    }
    ...
    if (read(parent_exit_notification[0], &c, 1) == -1) perror("read error");
...
const char *screenshot_save_get_filename(void) { return tmp_filename; }
